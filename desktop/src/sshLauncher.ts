...
export function watchTunnel(
  ...
  // Bruit normal d'une connexion ssh saine : ni erreur, ni raison de refuser
  // l'onglet -- juste a tracer dans le log.
  ...
  const onErrData = (buf: Buffer) => {
    for (const raw of buf.toString().split(/\r?\n/)) {
      const text = raw.trim()
      if (!text) continue
      if (BENIGN.test(text) || !FATAL.test(text)) { onEvent(`[tunnel] ${text}`, false); continue }
  ...
  tunnel.on('exit', (code) => {
    ...
    // n'a pas pu etre etabli (le message precis est deja passe par stderr).
    if (code !== 0 && code !== null && !fatalReported) {
      fatalReported = true
      onEvent(`Tunnel SSH termine prematurement (code ${code}) : forward non etabli.`, true)
    }
  })
}
