...
import { launchApp, waitForPorts, openTunnel, watchTunnel, waitUntilReady, waitUntilBackendReady } from './sshLauncher'
...
let contentBounds = { x: 0, y: 98, width: 1260, height: 622 }
...
function applyLayout(shouldBroadcast = true): void {
  ...
  for (const { view } of dockedTabs.values()) {
    try { catalogWindow.contentView.removeChildView(view) } catch { /* deja retiree */ }
  }
  if (activeTabId === null) { if (shouldBroadcast) broadcastTabs(); return }   // accueil : catalog seul
...
function createAppTab(
  ...
  void view.webContents.loadURL(`http://127.0.0.1:${frontendPort}`)
...
ipcMain.handle('cv:set-subapps-flyout', (_e, _open?: boolean) => {
  ...
  try { tab?.view.setBounds(contentBounds); tab?.view.setVisible(true) } catch { /* vue deja retiree */ }
})
...
ipcMain.handle('cv:show-subapps-menu', () => {
  if (!catalogWindow) return
