...
test('forward refuse au demarrage : transitoire, jamais fatal', () => {
  // La ligne exacte vue sur hudson quand le backend charge encore ses modeles.
...
test('hote injoignable : fatal', () => {
  const v = classifyTunnelLine('ssh: connect to host hudson port 22: Connection refused')
...
test('resolution DNS impossible : fatal', () => {
  assert.equal(classifyTunnelLine('ssh: Could not resolve hostname hudson: Name or service not known').fatal, true)
})

test('bruit benin : ni fatal ni transitoire', () => {
  const known = classifyTunnelLine('Warning: Permanently added \'hudson\' to the list of known hosts.')
