...
export default function DiffPage() {
  ...
  const handleDiff = async () => {
    if (!revA.trim() || !revB.trim()) {
      toast.error('Entrez deux révisions')
      return
    }
    setLoading(true)
    try {
      ...
      toast.error('Erreur lors du diff')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-semibold text-white">Diff de versions</h1>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">Révision A (base)</label>
            <div className="flex gap-2">
              <input
                value={revA}
                onChange={e => setRevA(e.target.value)}
                placeholder="hash, HEAD~1, tag…"
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">Révision B (cible)</label>
            <div className="flex gap-2">
              <input
                value={revB}
                onChange={e => setRevB(e.target.value)}
                placeholder="hash, HEAD, tag…"
                className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 font-mono"
              />
              {commits && commits.length > 0 && (
                <select
        ...
        <div className="flex justify-end">
          <button
            ...
            {loading ? 'Calcul…' : 'Calculer le diff'}
          </button>
        </div>
      </div>
      ...
      {hasSummary && (
        <div className="bg-gray-900 border border-indigo-800/40 rounded-xl p-4 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-white">Résumé métier</h2>
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            ...
            <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-900/30 border border-amber-700/30 text-amber-300">
              <FileText size={13} /> ~ {lblChanged} annotation(s)
            </span>
          </div>
          {(imgAdded + imgDeleted + lblChanged) === 0 && (
            <p className="text-[11px] text-gray-500">
              Ce diff ne touche pas d'images/annotations directement (le contenu est
              agrégé dans un dossier DVC — voir le détail fichier ci-dessous).
            </p>
          )}
          ...
          <div className="border-t border-gray-800 pt-2.5 text-[11px] text-gray-400">
            {usedByRun ? (
              <p>
                Utilisé par : <span className="font-mono text-amber-300">Run {usedByRun}</span>
                {usedByMlflow.length > 0 && (
                  <> · run(s) MLflow <span className="font-mono text-purple-300">{usedByMlflow.join(', ')}</span></>
                )}
                <span className="text-gray-600"> — détails et liens dans le Run Insight de l'Orchestrator.</span>
              </p>
            ) : (
              <p className="text-gray-500">
                Aucun run associé à cette version dans les trailers du commit (info non
                disponible — non inventée).
              </p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-900/30 text-emerald-400 border border-emerald-700/30 rounded-lg text-sm">
              <Plus size={13} /> {added.length} ajouté(s)
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-red-900/30 text-red-400 border border-red-700/30 rounded-lg text-sm">
              <Minus size={13} /> {deleted.length} supprimé(s)
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-900/30 text-amber-400 border border-amber-700/30 rounded-lg text-sm">
              <RefreshCw size={13} /> {modified.length} modifié(s)
            </span>
            {renamed.length > 0 && (
          ...
          {added.length + deleted.length + modified.length + renamed.length === 0 ? (
            <div className="text-center py-8 text-gray-500 text-sm">
              Aucune différence DVC entre ces deux révisions
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium uppercase">Changement</th>
                ...
                <tbody>
                  {added.map((f, i) => (
                    <tr key={`add-${i}`} className={i > 0 ? 'border-t border-gray-800/40' : ''}>
                      <td className="px-4 py-2.5"><span className="text-emerald-400 text-xs font-medium">+ ajouté</span></td>
                  ...
                  {deleted.map((f, i) => (
                    <tr key={`del-${i}`} className="border-t border-gray-800/40">
                      <td className="px-4 py-2.5"><span className="text-red-400 text-xs font-medium">− supprimé</span></td>
                      <td className="px-4 py-2.5 font-mono text-xs text-gray-200">{f.path}</td>
                      <td className="px-4 py-2.5 font-mono text-xs text-gray-500">{f.hash?.slice(0, 10)}</td>
                    </tr>
                  ))}
                  {modified.map((f, i) => (
                    <tr key={`mod-${i}`} className="border-t border-gray-800/40">
                      <td className="px-4 py-2.5"><span className="text-amber-400 text-xs font-medium">~ modifié</span></td>
                      <td className="px-4 py-2.5 font-mono text-xs text-gray-200">{f.path}</td>
                      <td className="px-4 py-2.5 font-mono text-xs text-gray-500">{f.hash?.slice(0, 10)}</td>
                    </tr>
                  ))}
