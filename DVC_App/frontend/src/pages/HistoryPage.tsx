...
// "il y a X" (FR) devient "X ago" (EN) : ordre des mots different, un simple
// t() par sous-chaine ne peut pas inverser -> on branche sur la langue ici.
function timeAgo(ts: number): string {
  const diff = Math.floor(Date.now() / 1000 - ts)
  const en = getLang() === 'en'
  if (diff < 60)       return en ? `${diff}s ago`      : `il y a ${diff}s`
  if (diff < 3600)     return en ? `${Math.floor(diff / 60)}min ago`   : `il y a ${Math.floor(diff / 60)}min`
  if (diff < 86400)    return en ? `${Math.floor(diff / 3600)}h ago`   : `il y a ${Math.floor(diff / 3600)}h`
  if (diff < 2592000)  return en ? `${Math.floor(diff / 86400)}d ago`  : `il y a ${Math.floor(diff / 86400)}j`
  return formatTs(ts)
}
...
export default function HistoryPage() {
  ...
  const restoreMut = useMutation({
    ...
    onSuccess: r => {
      toast.success(`${t('Version')} ${r.rev?.slice(0, 8) ?? ''} ${t('restaurée')}`)
      qc.invalidateQueries({ queryKey: ['commits'] })
      qc.invalidateQueries({ queryKey: ['datasets'] })
    },
    onError: (e: Error) => toast.error(`${t('Restauration échouée')} : ${e.message}`),
  })
  const restore = (rev: string, subject: string) => {
    if (confirm(`${t('Restaurer cette version ?')}\n\n"${subject}"\n\n${t("Le dossier de travail reviendra exactement à l'état de ce commit (dataset + modèle). Réversible en restaurant une version plus récente.")}`))
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-white">{t('Historique des versions')}</h1>
          {commits && (
      ...
      <div className="flex items-start gap-2 bg-gray-900/60 border border-gray-800 rounded-lg px-3 py-2.5 text-[11px] text-gray-400 leading-relaxed">
        ...
        <p>
          {t('Chaque commit est')} <b className="text-gray-300">{t('une version')}</b> {t('de vos données/modèles. Le hash (ex.')} <span className="font-mono text-indigo-300">41ef68b3</span>{t(') identifie la version du code + config. Un fichier')} <span className="font-mono">.dvc</span> {t("modifié = nouvelle version du dataset ou du modèle qu'il pointe. Les puces")}
          <span className="text-emerald-300"> Dataset</span> /
          <span className="text-amber-300"> Run</span> /
          <span className="text-blue-300"> mAP</span> {t("proviennent des trailers posés par l'Orchestrator.")}
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-500">
          ...
          <p className="text-sm">{t('Aucun commit DVC trouvé')}</p>
        </div>
      ) : (
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {commit.dvc_files.length > 0 && (
                        <span className="text-xs text-indigo-300 bg-indigo-900/30 border border-indigo-700/30 px-2 py-0.5 rounded">
                          {commit.dvc_files.length} {t('fichier(s) DVC')}
                        </span>
                      )}
                      <button
                        onClick={e => {
                      ...
                      <button
                        ...
                        title={t("Restaurer cette version (checkout) — ramène le dossier de travail à cet état, sans ligne de commande")}
                  ...
                  {expanded === commit.hash && commit.dvc_files.length > 0 && (
                    <div className="border-t border-gray-800 px-4 py-3 bg-gray-950/50">
                      <p className="text-xs text-gray-500 mb-2">
                        {t('Fichiers DVC modifiés (chacun = une nouvelle version du dataset/modèle pointé) :')}
                      </p>
