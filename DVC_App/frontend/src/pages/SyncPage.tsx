...
export default function SyncPage() {
  ...
  const relinkMut = useMutation({
    ...
    onSuccess: r => {
      if (r.ok) { toast.success(t('Working dir re-lié au cache')); refetchDisk() }
      else toast.error(r.error ?? t('Relink échoué'))
    },
  ...
  const addRemoteMut = useMutation({
    ...
    onSuccess: r => {
      if (r.ok) { toast.success(`${t('Remote')} « ${r.name} » ${t('ajouté')}`); setRemoteUrl(''); qc.invalidateQueries({ queryKey: ['remotes'] }) }
      else toast.error(r.error ?? t('Ajout du remote échoué'))
    },
    onError: (e: Error) => toast.error(e.message),
  })
    ...
    const cancel = startSync(
      ...
      (evt: SyncEvent) => {
        if (evt.type === 'start') {
          ...
          setLogs(p => [...p, { text: `${t('Erreur')}: ${evt.message}`, type: 'error' }])
          setState('error')
        } else if (evt.type === 'done') {
          setLogs(p => [...p, { text: `${t('Terminé')} (code ${evt.returncode})`, type: evt.returncode === 0 ? 'success' : 'error' }])
          setState(evt.returncode === 0 ? 'done' : 'error')
          if (evt.returncode === 0) {
            toast.success(`DVC ${action} ${t('terminé')}`)
            invalidate()
          } else {
            toast.error(`DVC ${action} ${t('échoué')}`)
          }
        }
      },
      () => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                {isPush
                  ? t('Envoyer les fichiers trackés vers le remote')
                  : t('Récupérer les fichiers trackés depuis le remote')
                }
              </p>
            </div>
          </div>
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-semibold text-white">{t('Synchronisation')}</h1>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-2 text-[12px] text-gray-400 leading-relaxed">
        <p>
          <b className="text-blue-300">Push</b> {t("= j'envoie les données versionnées (contenu réel pointé par les")} <span className="font-mono">.dvc</span>{t(") vers le remote, pour qu'elles soient récupérables ailleurs.")} <b className="text-emerald-300">Pull</b> {t('= je récupère depuis le remote le contenu des versions suivies par le repo.')}
        </p>
        <p>
          « <b className="text-gray-200">Sync</b> » {t('ici = cette page (Push et Pull DVC) — pas d\'abstraction cachée : rien n\'est envoyé ni récupéré sans que vous cliquiez.')}
        </p>
        {/* Source -> destination reelle */}
        {hasRemote ? (
          <div className="flex flex-wrap items-center gap-2 pt-1">
            <Server size={13} className="text-gray-500" />
            <span className="text-gray-300">{t('Remote :')}</span>
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-2 bg-amber-900/20 border border-amber-700/30 rounded-lg px-3 py-2 text-amber-300">
              ...
              <span>{t("Aucun remote DVC configuré : Push/Pull n'ont aucune destination. Ajoutez-en un ci-dessous (0 CLI).")}</span>
            </div>
            <div className="flex flex-wrap items-end gap-2">
              ...
              <label className="text-[11px] text-gray-400 flex-1 min-w-[220px]">
                {t('Destination (dossier local ou url s3://, ssh://…)')}
                <input value={remoteUrl} onChange={e => setRemoteUrl(e.target.value)}
                  placeholder={t('ex. D:\\dvc_remote  ou  s3://bucket/path')}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white">{t('Stockage (dé-duplication)')}</h2>
          {!showDisk && (
            <button onClick={() => setShowDisk(true)}
              ...
              {t("Calculer l'usage disque")}
            </button>
          )}
          {showDisk && (
            <button onClick={() => refetchDisk()} disabled={diskLoading}
              className="ml-auto text-xs px-2.5 py-1 rounded-lg border border-gray-700 text-gray-300 hover:bg-gray-800">
              {diskLoading ? <Loader2 size={12} className="animate-spin inline" /> : t('Recalculer')}
            </button>
          )}
        </div>
        <p className="text-[11px] text-gray-500 leading-relaxed">
          {t('Le cache DVC stocke chaque fichier une fois (par empreinte md5) : deux versions qui partagent des images ne les stockent qu\'une fois. Avec le cache')} <b className="text-gray-300">{t('en liens')}</b> {t('(hardlink/reflink), le working dir ne fait que pointer vers le cache — pas de 2e copie physique.')}
        </p>
        {showDisk && (
          diskLoading && !disk ? (
            <div className="text-xs text-gray-500 flex items-center gap-2"><Loader2 size={13} className="animate-spin" /> {t('Calcul…')}</div>
          ) : disk?.repo_exists ? (
            <div className="space-y-2">
              <div className="grid grid-cols-3 gap-2 text-center">
                ...
                <div className="bg-gray-950 border border-gray-800 rounded-lg p-2.5">
                  <p className="text-[10px] text-gray-500 uppercase">{t('Type cache')}</p>
                  <p className={`text-sm font-mono mt-0.5 ${disk.linked ? 'text-emerald-300' : 'text-amber-300'}`}>
                    {disk.linked ? t('liens') : 'copy'}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[11px] text-gray-500">
                  {disk.cache_type}
                  {disk.linked
                    ? t(' — working dir partagé avec le cache (pas de doublon)')
                    : t(' — le working dir est une 2e copie ; relie-le au cache :')}
                </span>
                {!disk.linked && (
            ...
            <p className="text-xs text-gray-500">{t('Repo DVC introuvable — rien à mesurer.')}</p>
          )
        )}
      </div>
    </div>
