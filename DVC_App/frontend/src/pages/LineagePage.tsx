      ...
      <div className="h-full grid place-items-center bg-gray-950 text-center">
        <div>
          <p className="text-red-300 font-semibold">
            Le lineage n’a pas pu être affiché.
          </p>
...
function buildFlow(
  ...
  runs.forEach((r, i) => {
    ...
    subsets.forEach((d, j) => {
      ...
      nodes.push({
        ...
        data: {
          kind: "dataset",
          label: d.label,
          sub: `${String(d.data.dataset || "")} · ${String(d.data.dvc_version || "non versionné")}`,
          central: d,
        },
      });
    });
    ...
    nodes.unshift({
      ...
      type: "groupbox",
      position: { x: x0, y: 220 },
      data: { label: `${fork ? "Run fork" : "Run mère"} · vue DVC`, fork },
...
function LineageInner() {
  ...
  const flow = useMemo(
    () => buildFlow(payload, commits, search, collapsed, orchUrl),
    [payload, commits, search, collapsed, orchUrl],
  );
  ...
  return (
    <div className="h-full min-h-[600px] flex flex-col bg-gray-950">
      <header className="px-5 py-3 border-b border-gray-800 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-semibold text-white">Lineage DVC</h1>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Nom, Run ID, dataset…"
                      ...
                      >
                        <div className="flex">
                          <div className="flex-1">
                            ...
                            >
                              {r.data.is_fork ? "Run fork" : "Run mère"}
                            </p>
                          ...
                          >
                            {commit ? "versionné" : "non versionné"}
                          </span>
                        </div>
                        <div className="mt-4 grid gap-2 sm:grid-cols-2">
                          <div className="rounded-lg border border-amber-800/40 bg-amber-950/10 p-3">
                            ...
                            <p className="font-mono text-[10px] text-gray-500">
                              DVC {String(r.data.dvc_version || "non committé")}
                            </p>
                          </div>
                          <div className="rounded-lg border border-indigo-800/40 bg-indigo-950/10 p-3">
                            ...
                            <p className="text-xs text-white">
                              {commit?.subject || "Aucun commit produit"}
                            </p>
                            <p className="font-mono text-[10px] text-gray-500">
                              {commit?.short || "—"}
                            </p>
          ...
          <aside className="w-80 border-l border-gray-800 bg-gray-900 p-4 space-y-3 overflow-y-auto">
            ...
            {selected.runId && (
              <div className="rounded-lg border border-violet-700/30 p-2">
                <p className="text-[10px] text-violet-400">Run ID MLOps</p>
