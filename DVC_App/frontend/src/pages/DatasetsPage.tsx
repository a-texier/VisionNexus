    ...
    <div className="p-6 space-y-5">
      ...
      {statusData?.error ? (
        ...
        <div className="flex items-center gap-3 bg-amber-900/20 border border-amber-700/30 rounded-xl px-4 py-3 text-sm text-amber-400">
          <AlertCircle size={16} className="flex-shrink-0" />
          <span>
            {modifiedCount > 0 && `${modifiedCount} fichier(s) modifié(s)`}
            {modifiedCount > 0 && missingCount > 0 && ' · '}
            {missingCount > 0 && `${missingCount} fichier(s) manquant(s)`}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 bg-emerald-900/20 border border-emerald-700/30 rounded-xl px-4 py-3 text-sm text-emerald-400">
          Tous les datasets sont synchronisés
        </div>
      ) : null}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-500">
          ...
          <p className="text-sm">Aucun fichier DVC trouvé</p>
          <p className="text-xs text-gray-600">Configurez DVC_REPO_PATH vers un repo git+dvc</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium uppercase">Fichier DVC</th>
