...
export const AnnotationPage: React.FC = () => {
  ...
  const handleImportSeqAnnotations = async () => {
    ...
    if (!seq || seq.id == null) { toast.error('Sélectionnez une séquence'); return }
    const path = window.prompt(
      `Importer des annotations sur « ${seq.name} »\n\n` +
      `Chemin serveur d'un fichier .ver OU d'un dossier YOLO (.txt) :`
    )
    if (!path || !path.trim()) return
    const replace = window.confirm('Remplacer les annotations existantes de cette séquence ?\n(OK = remplacer, Annuler = ajouter)')
    try {
      const res = await datasetAPI.importSequenceAnnotations(numericProjectId, seq.id, { path: path.trim(), replace })
      toast.success(`${res.annotations_created} annotations importées (${res.format}) sur ${res.frames_annotated} frames`)
      ...
      toast.error(`Import annotations : ${msg}`)
    }
  }
  ...
  useEffect(() => {
    if (propagationTaskId) {
      ...
      localStorage.setItem(propLabelKey, propagationLabel || 'Traitement')
    } else {
  ...
  const handleStopPropagation = useCallback(async () => {
    ...
    try {
      ...
      setPropagationStatus('Arrêt demandé…')
      toast('Arrêt du process demandé')
    } catch {
      toast.error('Impossible d\'arrêter le process')
    }
  }, [propagationTaskId])
    ...
    const finishRun = async (status: 'completed' | 'error' | 'gone', message: string) => {
      ...
      if (status === 'completed') {
        ...
        if (!propagationLabel.toLowerCase().includes('detection')) {
          toast.success(`${propagationLabel} terminé !`)
        }
        setTimeout(() => {
        ...
        toast.error(`Erreur propagation : ${message}`)
  ...
  const handleApplyNMS = async (iouThreshold: number) => {
    const frame = getCurrentFrame()
    if (!frame) { toast.error('Aucune frame sélectionnée'); return }
    try {
      const result = await annotationsAPI.applyNMS(frame.id, iouThreshold)
      toast.success(`NMS : ${result.deleted_count} doublon${result.deleted_count !== 1 ? 's' : ''} supprimé${result.deleted_count !== 1 ? 's' : ''}`)
      const anns = await annotationsAPI.list(frame.id)
      annotationsCacheRef.current.set(frame.id, anns)
      loadAnnotations(frame.id, anns)
    } catch {
      toast.error('Erreur lors du NMS')
    }
  }
  ...
  const handleAssignTrack = useCallback(async (
    ...
    try {
      ...
      toast.error('Erreur assignation de track')
    }
  ...
  const handleDeleteAnnotationsForFrames = async (frameIndices: number[]) => {
    ...
    try {
      ...
      toast.error('Erreur suppression des annotations')
      return
    }
    await fetchFrames(numericProjectId)
    await loadTracks()
    ...
    toast.success(
      `Annotations supprimées sur ${selectedFrames.length} frame${selectedFrames.length > 1 ? 's' : ''} — Ctrl+Z pour annuler`
    )
  }
  ...
  const handleBulkUndo = useCallback(async () => {
    try {
      ...
      toast.success(`Suppression annulée (${res.restored ?? 0} annotations restaurées)`)
    } catch {
  ...
  const handleBulkRedo = useCallback(async () => {
    try {
      ...
      await refreshAfterBulk()
      useBulkUndoStore.getState().setFlags(res.can_undo_more, res.can_redo_more)
      toast.success('Suppression refaite')
    } catch {
      toast.error('Impossible de refaire la suppression')
    }
  }, [numericProjectId, refreshAfterBulk])
  ...
  const handleDeleteTrack = useCallback(async (trackId: number) => {
    try {
      ...
      toast.success(`Track supprimé (${res.deleted_annotations} annotation${res.deleted_annotations !== 1 ? 's' : ''})`)
    } catch (e) {
      console.error('[AnnotationPage] Erreur suppression track:', e)
      toast.error('Erreur suppression du track')
      return
    }
    await refreshTracksAndFrame()
  ...
  const handleDeleteTrackBlock = useCallback(async (trackId: number, startFrame: number, endFrame: number) => {
    try {
      ...
      toast.success(
        res.track_removed
          ? `Bloc supprimé — piste vidée (${res.deleted_annotations} annotation${res.deleted_annotations !== 1 ? 's' : ''})`
          : `Bloc supprimé (${res.deleted_annotations} annotation${res.deleted_annotations !== 1 ? 's' : ''})`
      )
    } catch (e) {
      console.error('[AnnotationPage] Erreur suppression bloc:', e)
      toast.error('Erreur suppression du bloc')
      return
    }
    await refreshTracksAndFrame()
  }, [refreshTracksAndFrame])
  ...
  const handleTextSegment = async (frameId?: number, silent = false) => {
    ...
    try {
      ...
      if (!currentProject?.classes?.length) {
        if (!silent) toast.error('Créez d abord une classe dans le panneau Classes avant de lancer GD ou SAM3.')
        return 0
      }
      const classId = activeClassId
      if (!classId) {
        if (!silent) toast.error('Sélectionnez une classe active avant de lancer GD ou SAM3.')
        return 0
      }
      ...
      if (textModel === 'sam3') {
        ...
        if (result.count === 0) {
          if (!silent) toast('Aucun objet détecté avec SAM3.', { icon: '🔍' })
          return 0
        }
        ...
        if (!silent) toast.success(`SAM3 : ${result.count} objet${result.count > 1 ? 's' : ''} détecté${result.count > 1 ? 's' : ''}`)
        return result.count
      } else {
        // Grounding DINO
        ...
        if (result.count === 0) {
          if (!silent) toast('Aucun objet détecté avec ce prompt.', { icon: '🔍' })
          return 0
        }
        // Bulk create : 1 appel réseau au lieu de result.count appels séquentiels
        const bulk = result.detections.map((det) => {
        ...
        await bulkAddAnnotations(frame.id, bulk)
        if (!silent) toast.success(`${result.count} objet${result.count > 1 ? 's' : ''} détecté${result.count > 1 ? 's' : ''}`)
        return result.count
      }
    } catch (e) {
      ...
      if (!silent) {
        toast.error(typeof detail === 'string'
          ? `Detection par texte : ${detail}`
          : 'Erreur lors de la segmentation par texte', { duration: 8000 })
      }
      return 0
    } finally {
  ...
  const handleSAMAuto = async () => {
    ...
    try {
      ...
      toast.success(`SAM Auto terminé : ${autoCountRef.value} objet${autoCountRef.value !== 1 ? 's' : ''}`)
    } catch {
      toast.error('Erreur SAM Auto')
  ...
  const handleValidateSAMMask = useCallback((maskIndex: number) => {
    ...
    if (!resolvedClassId) { toast.error('Sélectionnez une classe avant de valider'); return }
  ...
  const handleAutoModeStart = async () => {
    if (!textPrompt.trim()) {
      toast.error('Entrez un prompt texte pour le mode auto')
    ...
    if (!currentProject?.classes?.length) {
      toast.error('Créez d abord une classe dans le panneau Classes avant de lancer GD ou SAM3.')
      return
    }
    if (!initialClassId) {
      toast.error('Sélectionnez une classe active avant de lancer GD ou SAM3.')
      return
    }
      ...
      try {
        ...
        if (firstError === null) {
          ...
          firstError = typeof detail === 'string'
            ? detail
            : (e as Error)?.message ?? 'echec de la detection'
        }
        handleFrameSelect(i)
      }
    ...
    toast.success(
      `${modelLabel} batch terminé : ${successCount} frame${successCount !== 1 ? 's' : ''} traitée${successCount !== 1 ? 's' : ''}${errorCount > 0 ? ` (${errorCount} erreur${errorCount !== 1 ? 's' : ''})` : ''}`
    )
  }
  ...
  const handleDropAnnotations = useCallback(
    async (e: React.DragEvent) => {
      ...
      if (files.length === 0) { toast.error('Aucun fichier détecté'); return }
      ...
      if (jsonFile && files.length === 1) {
        try {
          ...
          if (data.success) {
            toast.success(`${data.restored_count} annotations restaurées`)
            const frame = getCurrentFrame()
          ...
          toast.error('Fichier JSON invalide ou erreur réseau')
        }
        return
      }
      ...
      if (verFiles.length === 0 && txtFiles.length === 0) {
        toast.error('Glissez un fichier .ver, un dossier YOLO (.txt), ou un backup .json')
        return
      }
      const seq = sequences.find(
        (s) => currentFrameIndex >= s.start_index && currentFrameIndex < s.start_index + s.frame_count
      )
      if (!seq || seq.id == null) { toast.error('Sélectionnez une séquence pour l\'import'); return }
      const replace = window.confirm(
        `Importer sur « ${seq.name} ».\nRemplacer les annotations existantes de cette séquence ?\n(OK = remplacer, Annuler = ajouter)`
      )
  ...
  if (!currentProject) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <LoadingSpinner size="lg" label="Chargement du projet..." />
      </div>
    )
  }
    ...
    >
      ...
      {isDragOver && (
        <div className="fixed inset-0 z-50 bg-blue-900/60 border-2 border-blue-400 border-dashed flex items-center justify-center pointer-events-none">
          <div className="text-center text-blue-200">
            ...
            <p className="text-lg font-semibold">Déposer des annotations</p>
            <p className="text-sm opacity-70">.ver ou dossier YOLO (.txt) → séquence courante · .json → restauration</p>
          </div>
        </div>
      )}
      ...
      <header className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 border-b border-slate-700 flex-shrink-0 overflow-x-auto">
        ...
        <div className="flex items-center gap-0.5 bg-slate-800 border border-slate-600 rounded p-0.5 flex-shrink-0"
             ...
             title="Mode de sortie des annotations automatiques (SAM Auto, Grounding DINO, SAM3)">
          <button
            ...
            title="Boîtes englobantes (SAM Auto, Grounding DINO, SAM3)"
          >
            BBox
          </button>
          <button
            ...
            title="Polygones de segmentation (SAM Auto, Grounding DINO avec raffinement SAM2, SAM3)"
          >
            Seg
          </button>
        </div>
        ...
        <button
          ...
          title="Annoter par texte (GD / SAM3)"
          ...
          <>
            ...
            <div className="flex rounded overflow-hidden border border-slate-600 flex-shrink-0">
              ...
              <button
                ...
                title="SAM3 — détection par texte open-vocabulary"
              >SAM3</button>
            </div>
            ...
            {textModel === 'gd' && (
              <div className="flex items-center gap-1 bg-slate-700 rounded px-2 py-0.5 flex-shrink-0">
                ...
                <input type="number" min="0.05" max="0.95" step="0.05" value={boxThreshold}
                  ...
                  className="w-12 bg-transparent text-white text-xs outline-none" title="Seuil boîte GD" />
                ...
                <input type="number" min="0.05" max="0.95" step="0.05" value={textThreshold}
                  ...
                  className="w-12 bg-transparent text-white text-xs outline-none" title="Seuil texte GD" />
              </div>
            )}
            ...
            {textModel === 'sam3' && (
              <div className="flex items-center gap-1 bg-slate-700 rounded px-2 py-0.5 flex-shrink-0">
                ...
                <input type="number" min="0.05" max="0.95" step="0.05" value={sam3BoxThreshold}
                  ...
                  className="w-12 bg-transparent text-white text-xs outline-none" title="Seuil score SAM3" />
                <span className="text-slate-400 text-xs">Txt:</span>
                <input type="number" min="0.05" max="0.95" step="0.05" value={sam3TextThreshold}
                  ...
                  className="w-12 bg-transparent text-white text-xs outline-none" title="Seuil texte SAM3" />
              </div>
            )}
          </>
        )}
        ...
        {showTextPrompt && (
          <div className="flex items-center gap-1.5 flex-shrink-0 flex-wrap">
            ...
            <input
              ...
              placeholder="voiture. personne. vélo."
            ...
            {totalFrameCount > 1 && (
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <button
                  ...
                  title="Configurer la plage de frames"
                >
                ...
                {!autoModeRunning ? (
                  <button
                    ...
                    title="Lancer le batch sur la plage sélectionnée"
                  >
                    <Play size={11} /> Batch
                  </button>
                ) : (
            ...
            {showBatchRange && (
              <div className="flex items-center gap-1.5 bg-slate-700/80 rounded px-2 py-1 flex-shrink-0">
                <span className="text-slate-400 text-xs">De F</span>
                <input
                ...
                />
                <span className="text-slate-400 text-xs">à F</span>
                <input
                  type="number" min={0} max={totalFrameCount - 1}
                  value={batchEndIndex}
        ...
        <div className="flex items-center gap-1 flex-shrink-0" data-tour="zoom-controls">
          <button onClick={handleZoomOut} className="p-1.5 rounded hover:bg-slate-700 text-slate-400 hover:text-white transition-colors" title="Dézoomer">
            <ZoomOut size={13} />
          </button>
          <button
          ...
          <button onClick={handleZoomIn} className="p-1.5 rounded hover:bg-slate-700 text-slate-400 hover:text-white transition-colors" title="Zoomer">
            <ZoomIn size={13} />
          </button>
          <button
            onClick={handleZoomReset}
            className="p-1.5 rounded hover:bg-slate-700 text-slate-400 hover:text-white transition-colors"
            title="Centrer la vue (reset zoom + pan)"
          >
            <Maximize2 size={13} />
          </button>
        </div>
        ...
        <button
          ...
          title="Télécharger un backup JSON des annotations (glissez-déposez pour restaurer)"
        >
          <Upload size={12} />
        </button>
        <button
      ...
      {extractionTaskId && (
        <div className="flex items-center gap-3 px-4 py-1.5 bg-orange-900/40 border-b border-orange-800/50 text-xs text-orange-200 flex-shrink-0">
          ...
          <span className="truncate max-w-xs">Extraction frames — {extractionMessage || 'En cours...'}</span>
      ...
      {propagationTaskId && (
        <div data-tour="propagation-bar"
          ...
          <span className="truncate max-w-xs">{propagationLabel} — {propagationStatus || 'Démarrage...'}</span>
          <div className="flex-1 h-1 bg-slate-700 rounded-full overflow-hidden">
            <div
        ...
        <div className="px-4 py-1.5 bg-blue-900/30 border-b border-blue-800/40 flex-shrink-0 space-y-1">
          <div className="flex items-center gap-2 text-xs text-blue-200">
            ...
            <span className="font-medium">
              Import de {importJobs.length} séquence{importJobs.length > 1 ? 's' : ''}
              {importRunning ? ' en cours…' : ' terminé'}
            </span>
            {!importRunning && (
              <button
      ...
      {autoModeRunning && (
        <div data-tour="batch-bar"
          ...
          <span>
            ...
            {autoModePaused ? ' (pausé)' : ''} : frame {autoModeProgress.current}/{autoModeProgress.total}
          </span>
          <div className="flex-1 h-1 bg-slate-700 rounded-full overflow-hidden">
            <div
      ...
      {samAutoStatus !== 'idle' && (
        <div className="flex items-center gap-3 px-4 py-1.5 bg-emerald-900/40 border-b border-emerald-800/50 text-xs text-emerald-200 flex-shrink-0">
          ...
          <span>
            {samAutoStatus === 'connecting' ? 'Connexion SAM...'
              : samAutoStatus === 'running' ? `SAM Auto en cours — ${samAutoCount} masque${samAutoCount !== 1 ? 's' : ''}`
              : `SAM Auto terminé — ${samAutoCount} masque${samAutoCount !== 1 ? 's' : ''}`}
          </span>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {currentProject.project_type === 'video' && (
          <Sidebar
            ...
            onPropagationStarted={(taskId, label) => {
              ...
              setPropagationStatus('Démarrage...')
            }}
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col min-w-0" data-tour="canvas-area">
          ...
          <button onClick={toggleLut} title="LUT / Affichage (remap 16/8 bits, histogramme)"
            data-tour="lut-btn"
          ...
          {currentFrame ? (
            ...
            <div className="flex-1 flex items-center justify-center h-full text-slate-600">
              <div className="text-center">
                <p className="text-sm">Aucune frame disponible</p>
                <button
                  onClick={() => setShowImport(true)}
                  className="mt-3 text-xs text-blue-400 hover:text-blue-300 underline"
                >
                  Importer des images ou une vidéo
                </button>
              </div>
            </div>
          )}
              ...
              <div className="flex items-center gap-2 px-3 pt-1.5 pb-1" data-tour="frame-nav">
                ...
                {sequences.length > 0 && (
                  <select
                    ...
                    title={currentSequence
                      ? `${currentSequence.name} — ${currentSequence.annotated_frames}/${currentSequence.frame_count} frames annotées · ${currentSequence.annotation_count} annotations`
                      : 'Séquences du projet'}
                  ...
                  >
                    {!currentSequence && <option value="">Séquence…</option>}
                    {sequences.map((s) => (
                      <option key={String(s.id ?? 'legacy')} value={String(s.id ?? 'legacy')}>
                        {s.source_type === 'video' ? '🎬' : s.source_type === 'images' ? '🖼' : '📦'} {s.name} · {s.annotated_frames}/{s.frame_count} annotées · {s.annotation_count} annot.
                      </option>
                    ))}
                  </select>
                )}
                ...
                {currentSequence && (
                  <button
                    ...
                    title="Importer des annotations (.ver ou dossier YOLO) sur cette séquence"
                ...
                <input
                  ...
                  title={currentSequence
                    ? `${currentSequence.name} — frame ${seqLocalIndex + 1} / ${seqLocalCount}`
                    : `Frame ${currentFrameIndex + 1} / ${totalFrameCount}`}
                />
                ...
                <div className="flex flex-col items-end flex-shrink-0 w-24">
                  ...
                  >
                    <span className="text-[10px] text-slate-600">aller à</span>
                    <input
                      ...
                      title={`Aller à la frame (1–${seqLocalCount})`}
                    />
                  </form>
                </div>
              </div>
      ...
      <ImportModal
        ...
        onImported={async (taskId?: string) => {
          ...
          if (taskId) {
            ...
            setExtractionProgress(0)
            setExtractionMessage('Extraction en cours...')
          }
        }}
