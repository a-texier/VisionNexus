...
export function ConvertPage() {
  ...
  const runVerToYolo = async () => {
    if (!verPath.trim() || !verOut.trim()) return toast.error('Chemin .ver et dossier de sortie requis')
    setVerBusy(true)
    try {
      const r = await postJson('/api/convert/ver-to-yolo', { ver_path: verPath.trim(), out_dir: verOut.trim(), width: verW, height: verH })
      toast.success(`YOLO écrit : ${r.frames} frames, ${r.classes} classes`)
    } catch (e) { toast.error(`Erreur : ${e instanceof Error ? e.message : e}`) }
    finally { setVerBusy(false) }
  }

  const runYoloToVer = async () => {
    if (!yoloDir.trim() || !yoloOut.trim()) return toast.error('Dossier YOLO et fichier .ver de sortie requis')
    setYoloBusy(true)
    try {
      const r = await postJson('/api/convert/yolo-to-ver', { yolo_dir: yoloDir.trim(), out_path: yoloOut.trim(), width: yoloW, height: yoloH })
      toast.success(`.ver écrit : ${r.boxes} boîtes`)
    } catch (e) { toast.error(`Erreur : ${e instanceof Error ? e.message : e}`) }
    ...
    <div className="min-h-screen bg-slate-900 text-slate-200">
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold">Convert — conversions rapides</h1>
        </div>
        <p className="text-xs text-slate-500">
          Utilitaire indépendant des projets. Tous les chemins sont des <strong>chemins serveur</strong>.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card icon={<FileText size={16} />} title=".ver → YOLO"
            desc="Convertit un .ver (pixels) en dossier YOLO normalisé. La résolution image est requise (pixels → [0,1]).">
            <TextInput value={verPath} onChange={setVerPath} placeholder="Chemin du fichier .ver" />
            <TextInput value={verOut} onChange={setVerOut} placeholder="Dossier YOLO de sortie" />
            <div className="flex items-center gap-2">
              ...
              <RunButton onClick={runVerToYolo} busy={verBusy} label="Convertir" />
            </div>
          </Card>

          <Card icon={<FileText size={16} />} title="YOLO → .ver"
            desc="Convertit un dossier YOLO en .ver. YOLO n'a pas de sous-classe ni de track : classe = sous-classe = sous-sous-classe, track_id = -1.">
            <TextInput value={yoloDir} onChange={setYoloDir} placeholder="Dossier YOLO (.txt)" />
            <TextInput value={yoloOut} onChange={setYoloOut} placeholder="Fichier .ver de sortie" />
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-500">W×H</span>
              ...
              <RunButton onClick={runYoloToVer} busy={yoloBusy} label="Convertir" />
            </div>
          </Card>
        </div>
      </div>
