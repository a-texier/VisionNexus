    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <header className="sticky top-0 z-20 bg-slate-950/95 backdrop-blur border-b border-slate-800 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2" data-tour="home-tools">
          ...
          <button
            ...
            title={t("Convertir les formats d'annotations (.ver ↔ YOLO)")}
            className="flex items-center gap-2 text-slate-400 hover:text-white text-sm px-3 py-2 rounded-lg hover:bg-slate-800 transition-colors"
          >
            <Wand2 size={15} />
            {t('Convert')}
          ...
          <button
            ...
            title={t('Usage : automatique vs manuel, reprise humaine')}
            className="flex items-center gap-2 text-slate-400 hover:text-white text-sm px-3 py-2 rounded-lg hover:bg-slate-800 transition-colors"
          >
            <Activity size={15} />
            {t('Monitoring')}
      ...
      <main className="px-6 py-8 max-w-6xl mx-auto pb-24">
        <div className="flex items-baseline gap-3 mb-6">
          <h2 className="text-xl font-semibold text-slate-100">{t('Projets')}</h2>
          {projects.length > 0 && (
            <span className="text-xs text-slate-500">
              {projects.length} {t(projects.length > 1 ? 'projets' : 'projet')}
            </span>
          )}
        </div>
        ...
        {isLoading ? (
          <div className="flex justify-center py-16">
            <LoadingSpinner size="lg" label={t('Chargement des projets...')} />
          </div>
        ) : projects.length === 0 ? (
          <div className="text-center py-16 border-2 border-dashed border-slate-700 rounded-xl">
            ...
            <p className="text-slate-400 text-sm">{t('Aucun projet.')}</p>
            <p className="text-slate-600 text-xs mt-1">
              {t("Créez votre premier projet d'annotation.")}
            </p>
            <button
              onClick={() => setShowCreate(true)}
      ...
      <ConfirmDialog
        ...
        message={`${t('Supprimer')} "${deleteTarget?.name}" ${t('et toutes ses annotations ?')}`}
      ...
      <div ref={workspaceMenuRef} data-tour="workspace-badge"
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            title={workspacePath ? `${t('Workspace')} : ${workspacePath}` : t('Workspace')}
