...
const labelOf = (s: string) => SOURCE_LABELS[s] ?? s
...
const Pie: React.FC<{
  ...
  const name = colorFn ? (k: string) => k : labelOf
  const total = data.reduce((a, [, n]) => a + n, 0)
  if (!total) return <p className="text-xs text-slate-500 italic py-6">Aucune annotation</p>
      ...
      <header className="border-b border-slate-800 px-6 py-4 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500">
            Repartition automatique / manuel et reprise humaine
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 bg-slate-800 rounded-lg p-0.5" data-tour="monitoring-scope">
          {(['me', 'all'] as const).map((s) => (
            <button key={s} onClick={() => setScope(s)}
              ...
              {s === 'me' ? 'Moi' : 'Tous les utilisateurs'}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-1 bg-slate-800 rounded-lg p-0.5" data-tour="monitoring-view">
          {(['detail', 'global'] as const).map((v) => (
            <button key={v} onClick={() => setView(v)}
              title={v === 'global'
                ? 'Repartition entre utilisateurs, tous projets sommes'
                : 'Detail par sequence et par provenance'}
              className={`px-3 py-1.5 text-xs rounded-md transition-colors ${
                view === v ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>
              {v === 'detail' ? 'Detail' : 'Global'}
            </button>
          ))}
        </div>
        ...
        <a href={`/api/monitoring/report?scope=${scope}`}
           ...
           title="Exporter ce rapport en HTML autonome (ouvrable hors ligne)"
        ...
        <button onClick={() => void load()} title="Rafraichir"
      ...
      <main className="p-6 max-w-[1400px] mx-auto" data-tour="monitoring-main">
        {loading ? (
          ...
          <p className="text-sm text-slate-500 italic py-20 text-center">
            Aucune annotation dans ce perimetre.
          </p>
        ) : view === 'global' ? (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
              {[
                [fmt(byUser.length), 'Utilisateurs'],
                [fmt(byRoot.length), 'Racines de workspace'],
                [fmt(globalTotals.total), 'Annotations'],
                [fmt(globalTotals.auto), 'Automatiques'],
                [fmt(globalTotals.manual), 'Manuelles'],
                [`${fmt(globalTotals.done)} / ${fmt(globalTotals.seqs)}`, 'Sequences exportees'],
              ].map(([v, l]) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <Card title="Part de chaque utilisateur (toutes annotations)">
                <Pie data={byUser.map((u) => [u.user, u.annotations_total])
                     ...
                     detailFor={(k) => {
                       ...
                       if (!u) return ''
                       return `auto ${fmt(u.annotations_auto)} · manuel ${fmt(u.annotations_manual)}`
                         + ` · retouchees ${fmt(u.auto_edited)} · supprimees ${fmt(u.auto_deleted)}`
                     }} />
              </Card>

              <Card title="Part de chaque utilisateur (annotations manuelles)">
                <Pie data={byUser.map((u) => [u.user, u.annotations_manual])
                              .filter(([, n]) => (n as number) > 0) as [string, number][]}
                     colorFn={userColorOf}
                     detailFor={(k) => {
                       const u = byUser.find((x) => x.user === k)
                       return u ? `${fmt(u.annotations_manual)} manuelles sur ${fmt(u.annotations_total)} au total` : ''
                     }} />
              </Card>

              <Card title="Detail par utilisateur" wide>
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-slate-400">
                        <th className="text-left py-1.5 px-2 font-medium">Utilisateur</th>
                        <th className="text-left py-1.5 px-2 font-medium">Racines</th>
                        {['Projets', 'Sequences', 'Exportees', 'Total', 'Auto', 'Manuel',
                          'Retouchees', 'Supprimees', 'Runs'].map((h) => (
                          <th key={h} className="text-right py-1.5 px-2 font-medium">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
              ...
              {byRoot.length > 1 && (
                <Card title="Par racine de workspace" wide>
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="text-slate-400">
                          <th className="text-left py-1.5 px-2 font-medium">Racine</th>
                          <th className="text-left py-1.5 px-2 font-medium">Utilisateurs</th>
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
              {[
                [fmt(total), 'Annotations'],
                [pctLabel(agg.auto, total), `Auto (${fmt(agg.auto)})`],
                [pctLabel(agg.manual, total), `Manuel (${fmt(agg.manual)})`],
                [agg.auto ? `${Math.round(agg.edited / agg.auto * 100)} %` : '0 %',
                 `Auto retouchees (${fmt(agg.edited)})`],
                [fmt(agg.deleted), 'Auto supprimees'],
                [fmt(agg.framesMulti), 'Frames reprises 2 fois+'],
              ].map(([v, l]) => (
                <div key={l} className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-3">
                  <div className="text-2xl font-semibold">{v}</div>
                  <div className="text-[11px] text-slate-500 mt-0.5">{l}</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <Card title="Repartition par provenance">
                <Pie data={agg.totals} />
              </Card>

              <Card title="Devenir des sorties automatiques">
                {agg.auto === 0 ? (
                  <p className="text-xs text-slate-500 italic py-6">Aucune sortie automatique</p>
                ) : (
                  <>
                    <div className="flex h-7 rounded overflow-hidden bg-slate-700">
                      {([['Conservees', kept, '#10b981'],
                         ['Retouchees', agg.edited, '#f59e0b'],
                         ['Supprimees', agg.deleted, '#ef4444']] as [string, number, string][])
                        .filter(([, v]) => v > 0)
                        .map(([n, v, c]) => (
                    ...
                    <div className="flex flex-wrap gap-3 mt-2">
                      {([['Conservees telles quelles', kept, '#10b981'],
                         ['Retouchees a la main', agg.edited, '#f59e0b'],
                         ['Supprimees', agg.deleted, '#ef4444']] as [string, number, string][])
                        .map(([n, v, c]) => (
                    ...
                    {!hasEvents && (
                      <p className="text-[11px] text-amber-400/80 mt-3">
                        Le journal d'evenements est vide : retouches et suppressions ne
                        sont comptees que depuis l'activation du monitoring. Les
                        annotations deja presentes apparaissent comme conservees.
                      </p>
                    )}
                  </>
                )}
              ...
              <Card title="Sequences (dataset)" wide>
                <div className="space-y-3">
                  {[...agg.rows].sort((a, b) => b.total - a.total).slice(0, 25).map((r, i) => {
                    ...
                    const detail = [
                      `${r.user} · ${r.project}`,
                      r.path || r.seq,
                      `${fmt(r.total)} annotations sur ${fmt(r.framesAnnotated)} frames`,
                      `Séquence : ${fmt(r.frameCount)} images (${pct} % couvertes)`,
                      ...Object.entries(r.by).sort((a, b) => b[1] - a[1])
                        .map(([s, n]) => `  ${labelOf(s)} : ${fmt(n)}`),
                      r.isDone
                        ? `Exporté ${r.lastExportFormat ?? ''} le ${(r.lastExportAt ?? '').slice(0, 10)}`
                        : 'Jamais exporté',
                    ].join('\n')
                ...
                <p className="text-[11px] text-slate-600 mt-3">
                  Coche verte = séquence exportée (YOLO, COCO ou .ver). Une séquence
                  annotée mais jamais exportée n'est pas considérée terminée.
                </p>
              </Card>

              {agg.rework.length > 0 && (
                <Card title="Reprise humaine par dataset" wide>
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="text-slate-400">
                          <th className="text-left py-1.5 px-2 font-medium">Dataset</th>
                          <th className="text-left py-1.5 px-2 font-medium">Projet</th>
                          <th className="text-right py-1.5 px-2 font-medium">Frames touchees</th>
                          <th className="text-right py-1.5 px-2 font-medium">Reprises 2 fois+</th>
                          <th className="text-right py-1.5 px-2 font-medium">Interventions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {agg.rework.map((r, i) => (
              ...
              <Card title="Runs automatiques" wide>
                {agg.runs.length === 0 ? (
                  <p className="text-xs text-slate-500 italic py-3">
                    Aucun run enregistre. Le journal se remplit a partir du prochain
                        ...
                        <tr className="text-slate-400">
                          {['Date', 'Utilisateur', 'Algorithme', 'Mode', 'Cibles',
                            ...
                            <th key={h} className={`py-1.5 px-2 font-medium ${
                              i >= 4 && i <= 7 ? 'text-right' : 'text-left'}`}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
            ...
            <p className="text-[11px] text-slate-600 mt-5">
              Rapport hors ligne multi-workspaces :
              <code className="ml-1 text-slate-500">
                python tools/monitoring_report.py &lt;workspace&gt; [...] --outdir &lt;dossier&gt;
              </code>
            </p>
          </>
        )}
