...
export const trackingAPI = {
  ...
  runGuidedTracking: (
    ...
    data: {
      ...
      start_frame_index: number
      end_frame_index: number
      algorithm: 'grounding_dino' | 'sam3' | 'yolo'
      text_prompt: string
  ...
  // Statut du détecteur YOLO custom (ultralytics dispo, modèle configuré trouvé + classes).
  getYoloStatus: (modelPath?: string) =>
    apiClient
      .get<YoloStatus>('/api/yolo/status', { params: modelPath ? { model_path: modelPath } : {} })
      .then((r) => r.data),
...
export interface YoloStatus {
  available: boolean          // ultralytics installé
  device: string
  model_path: string | null
  model_resolved: string | null
  model_found: boolean        // fichier .pt résolu et présent
  class_names: Record<string, string>  // {index: nom}
  loaded: boolean
}
