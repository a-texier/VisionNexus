...
export const LanguageToggle: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    <button
      ...
      className={
        'flex items-center justify-center text-slate-400 hover:text-white text-xs font-semibold ' +
        'w-9 h-9 rounded-lg hover:bg-slate-800 transition-colors border border-slate-700 ' +
        (className ?? '')
      }
    >
      {lang === 'en' ? 'EN' : 'FR'}
