...
const slotKind = (slot: ImportSlot, formats: SpecificFormatCapability[]): { icon: React.ReactNode; label: string } | null => {
  if (slot.serverPath.trim()) {
    const p = slot.serverPath.trim().toLowerCase()
    const specific = findSpecificFormat(p, formats)
    if (specific) return { icon: <Film size={14} className="text-violet-400" />, label: `${specific.label} serveur` }
    if (VIDEO_EXTS.some((e) => p.endsWith(`.${e}`))) return { icon: <Film size={14} className="text-emerald-400" />, label: 'Vidéo serveur' }
    return { icon: <FolderOpen size={14} className="text-emerald-400" />, label: 'Dossier images serveur (symlink)' }
  }
  if (slot.files.length > 0) {
    const first = slot.files[0].name.toLowerCase()
    const specific = findSpecificFormat(first, formats)
    if (specific) return { icon: <Film size={14} className="text-violet-400" />, label: `${specific.label} (upload)` }
    if (VIDEO_EXTS.some((e) => first.endsWith(`.${e}`))) return { icon: <Film size={14} className="text-blue-400" />, label: 'Vidéo (upload)' }
    return { icon: <Image size={14} className="text-blue-400" />, label: `${slot.files.length} image${slot.files.length > 1 ? 's' : ''} (upload)` }
  }
  return null
}
...
export const ImportModal: React.FC<ImportModalProps> = ({
  ...
  const fillSlotPath = (id: number, path: string) => {
    ...
    if (path.trim().toLowerCase().endsWith('.txt')) {
      datasetAPI.parseSequenceManifest(path.trim())
        .then((res) => {
          if (res.sequences.length === 0) { setError('Manifeste .txt vide.'); return }
          setSlots(() => [
            ...res.sequences.map((s) => ({
        ...
        .catch(() => setError('Manifeste .txt introuvable ou illisible côté serveur.'))
      return
    }
    setSlots((prev) => {
  ...
  const applySequenceManifest = (text: string) => {
    const rows = text.split(/\r?\n/).map((l) => l.trim())
      ...
      .filter((r) => r.path)
    if (rows.length === 0) { setError('Fichier .txt de séquences vide ou invalide.'); return }
    setSlots(() => [
  ...
  const handleDrop = async (e: React.DragEvent, slotId: number) => {
    ...
    if (items?.length > 0) {
      ...
      if (entry && (entry as { isDirectory?: boolean }).isDirectory) {
        ...
        else setError('Aucune image dans ce dossier.')
        return
      }
    }
  ...
  const handleImportAll = () => {
    ...
    if (readySlots.length === 0) { setError('Ajoutez au moins une séquence (glisser-déposer ou chemin serveur).'); return }
  ...
  return (
    <>
        ...
        <div className="relative bg-slate-800 border border-slate-700 rounded-xl p-5 w-[640px] max-h-[90vh] overflow-y-auto shadow-2xl">
          ...
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base font-semibold text-slate-100">
              Importer des séquences — {isVideoProject ? 'Séquence Image' : 'Image Random'}
            </h2>
          ...
          <div className="mb-4 rounded-lg border border-slate-700 bg-slate-900/40 px-3 py-2.5 space-y-1.5">
            <p className="text-xs text-slate-300 leading-relaxed">
              ...
              <strong className="text-emerald-300">Chemin serveur</strong> = zéro copie (symlink) —
              recommandé pour les gros datasets et les disques <strong>montés sur la machine du backend</strong>.
            </p>
            <p className="text-xs text-slate-400 leading-relaxed">
              <Upload size={11} className="inline mr-1 text-blue-400" />
              <strong className="text-blue-300">Glisser-déposer</strong> = upload via le navigateur.
              Un dossier glissé depuis un montage réseau Windows est bien lu, mais son <em>contenu est
              uploadé</em> (le navigateur ne transmet jamais un chemin). Pour référencer les fichiers sans
              copie, utilisez le chemin tel que vu par le serveur (ex : <code className="text-slate-300">/mnt/…</code>).
            </p>
            <p className="text-xs text-slate-500 leading-relaxed">
              Chaque ajout crée une <strong>séquence</strong> du projet (formats mélangeables).
              Import en série, une barre de progression par séquence.
            </p>
          </div>
          ...
          <div className="rounded-lg border border-slate-700 bg-slate-800/40 px-3 py-2 text-[11px] leading-relaxed text-slate-400">
            <span className="font-semibold text-slate-300">Formats importables</span>
            <ul className="mt-1 space-y-0.5">
              <li>
                <span className="text-emerald-400 font-medium">Dossier d'images</span> —
                {' '}JPG · PNG · BMP · WebP · <strong>TIFF/TIF</strong>. 8 bits, ou{' '}
                <strong className="text-slate-300">16 bits (PNG/TIFF, RGB ou IR mono)</strong> :
                source 16 bits conservée, seul l'affichage est remappé par la LUT (réglable par séquence).
              </li>
              <li>
                <span className="text-blue-400 font-medium">Vidéo</span> —
                {' '}MP4 · AVI · MOV · MKV · WebM (extraction en JPEG, décimation possible).
              </li>
              {specificFormats.map((format) => (
                <li key={format.id}>
                  <span className="text-violet-400 font-medium">{format.label}</span> —{' '}
                  {format.extensions.join(' · ')} (adaptateur optionnel détecté côté backend).
                </li>
              ))}
              <li className="text-slate-500">
                Source : <strong className="text-slate-400">chemin serveur</strong> (zéro copie / symlink)
                ou <strong className="text-slate-400">glisser-déposer</strong> local (upload navigateur).
              </li>
            </ul>
          </div>
          ...
          <div className="space-y-2.5">
            {slots.map((slot, i) => {
              const kind = slotKind(slot, specificFormats)
                  ...
                  <div className="px-3 py-2.5">
                      ...
                      {kind ? (
                        ...
                        <div className="flex items-center gap-2 flex-1 text-slate-500">
                          ...
                          <span className="text-xs">
                            Glissez un dossier d'images, une vidéo
                            {specificFormats.length > 0 ? ' ou un format spécifique détecté' : ''} ici…
                          </span>
                          <button
                    ...
                    {!locked && (
                      <div className="flex gap-1.5 mt-2">
                        <input
                          ...
                          placeholder="…ou chemin serveur : /mnt/datasets/frames · /data/video.mp4"
                    ...
                    {!locked && kind && (
                      <div className="flex items-center gap-1.5 mt-1.5">
                        <span className="text-[10px] text-slate-500 flex-shrink-0">Nom séquence</span>
                        <input
                          type="text"
                          value={slot.name}
          ...
          <div className="mt-3 border border-slate-700 rounded-lg overflow-hidden">
            <button onClick={() => setShowAdvanced(!showAdvanced)}
              ...
              <span className="flex items-center gap-1.5"><Settings2 size={11} /> Options d'optimisation (appliquées à chaque séquence)</span>
            ...
            {showAdvanced && (
              <div className="px-3 pb-3 pt-1 bg-slate-900/30 space-y-3">
                <div className="rounded border border-slate-700/70 bg-slate-800/40 px-2 py-1.5 text-xs text-slate-500 leading-relaxed">
                  <strong className="text-slate-400">Dossiers images serveur</strong> : symlink = zéro copie, démarrage immédiat.<br />
                  <strong className="text-slate-400">MP4</strong> : décimation 1/N + qualité JPEG = volume disque et vitesse d'extraction.<br />
                  <strong className="text-slate-400">16-bit</strong> : PNG/TIFF 16 bits — source conservée en 16 bits ; l'affichage ET l'IA passent par la LUT (défaut 3-sigma, réglable par séquence via le bouton LUT).
                </div>

                {/* Décimation vidéo */}
                <div>
                  <label className="text-xs text-slate-400 block mb-1.5">Décimation des frames vidéo</label>
                ...
                <label className="block">
                  <div className="flex justify-between text-xs text-slate-400 mb-1">
                    <span>Qualité JPEG frames extraites (MP4)</span>
                ...
                <label className="flex items-start gap-2 cursor-pointer">
                  ...
                  <div>
                    <span className="text-xs text-slate-300">PNG sans perte pour les MP4 (ignore la qualité JPEG)</span>
                    <p className="text-xs text-slate-600 mt-0.5">Idéal pour XFeat et flux optique. Plus lourd sur disque.</p>
                  </div>
                </label>
                ...
                <label className="flex items-start gap-2 cursor-pointer">
                  ...
                  <div>
                    <span className="text-xs text-slate-300">Liens symboliques pour les dossiers serveur (recommandé)</span>
                    <p className="text-xs text-slate-600 mt-0.5">Aucune copie des images. Si les symlinks sont interdits, l'app copie automatiquement.</p>
                  </div>
                </label>
                ...
                <label className="block">
                  <div className="flex justify-between text-xs text-slate-400 mb-1">
                    <span>Frames par batch (extraction arrière-plan)</span>
          ...
          <p className="mt-3 text-xs text-slate-500 text-center">
            L'import se fait en tâche de fond : le modal se ferme et vous pouvez
            annoter pendant le chargement (barre de progression par séquence en bas d'écran).
          </p>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button onClick={handleImportAll} disabled={readyCount === 0}
              ...
              Importer {readyCount > 0 ? `${readyCount} séquence${readyCount > 1 ? 's' : ''}` : ''} en fond
            </button>
          </div>
        </div>
      </div>
      ...
      {browserSlotId !== null && (
        <FileBrowserModal
          isOpen={showBrowser}
          title="Choisir un dossier d'images ou une vidéo (serveur)"
          filterType="all"
          selectDir={true}
