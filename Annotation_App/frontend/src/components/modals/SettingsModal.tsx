...
import { settingsAPI, storageAPI, trackingAPI } from '../../services/api'
import type { StorageStats, YoloStatus } from '../../services/api'
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-1.5 border-b border-slate-700/50 last:border-0">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <span className={`text-xs font-semibold ${color}`}>{mb.toFixed(1)} Mo</span>
        {onClear && (
          <button
            onClick={onClear}
            disabled={clearing || mb === 0}
            title="Vider ce dossier"
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const [yoloCheck, setYoloCheck] = useState<YoloStatus | null>(null)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-slate-800 border border-slate-700 rounded-xl shadow-2xl w-[580px] max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          ...
          {/* ---- Interface ---- */}
          <Section title="Interface" id="interface" open={openSections.has('interface')} onToggle={toggleSection}>
            <ParamRow label="Couleur de fond canvas" hint="Hex ou nom CSS">
              <div className="flex items-center gap-2">
                <input
                  type="color"
            ...
            <ParamRow label="Outil par défaut" hint="Astuce : clic molette = auto-ajustement (fit), molette = zoom">
              <select
              ...
              >
                ...
                <option value="pan">Déplacement</option>
              </select>
            </ParamRow>

            <ParamRow label="Opacité des annotations" hint="[0 = invisible, 1 = plein]">
              <SliderInput
            ...
            <ParamRow label="Hauteur timeline (px)">
              <NumberInput
            ...
            <ParamRow label="Afficher les étiquettes" hint="Nom de classe sur chaque annotation">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-xs text-slate-400">{settings.interface.show_labels ? 'Oui' : 'Non'}</span>
              </label>
            </ParamRow>

            <ParamRow label="Afficher le score" hint="Confiance en % sur les annotations IA">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="accent-blue-500"
                />
                <span className="text-xs text-slate-400">{settings.interface.show_confidence ? 'Oui' : 'Non'}</span>
              </label>
            </ParamRow>

            <ParamRow label="Épaisseur des bordures" hint="Épaisseur du contour des annotations (1–4 px)">
              <SliderInput
            ...
            <ParamRow label="Réduction preview (480/1600px)" hint="Désactiver si votre connexion est bonne : sert la source pleine résolution pour le scrub/l'affichage, sans écriture disque à résolution réduite">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="accent-blue-500"
                />
                <span className="text-xs text-slate-400">{settings.interface.preview_downscale_enabled ? 'Activée' : 'Désactivée (pleine résolution)'}</span>
              </label>
            </ParamRow>

            <ParamRow
              label="Live temps réel par défaut"
              hint="Activé : pendant SAMURAI/SAM2, le canvas suit la propagation, lit l'image via le chemin natif SMB/app-image quand il est disponible et applique les annotations poussées par WebSocket."
            >
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
            ...
            <ParamRow
              label="Suivi propagation : cadence du canvas"
              hint="Intervalle minimal entre deux sauts d'image pendant une propagation. 150 ms suit la boucle WebSocket et convient au chemin SMB natif ; 700 ms économise le réseau en repli HTTP ; 0 suit chaque résultat GPU. Chaque image affichée reçoit toujours les annotations de la même frame."
            >
              <SliderInput
          ...
          {/* ---- Import ---- */}
          <Section title="Import" id="import" open={openSections.has('import')} onToggle={toggleSection}>
            <p className="text-[11px] text-slate-500 pb-2 leading-relaxed border-b border-slate-700/50 mb-1">
              Ces réglages s'appliquent selon le TYPE d'import :
              <br />• <strong className="text-slate-400">Vidéo / format spécifique</strong> : décimation et upload par chunks si l'adaptateur le permet.
              <br />• <strong className="text-slate-400">Dossier d'images</strong> : copie/symlink sans ré-encodage (qualité source préservée).
              <br />Le <strong className="text-slate-400">nom de séquence</strong> par défaut = nom du dossier/fichier, modifiable dans la fenêtre d'import.
            </p>
            <ParamRow label="Qualité JPEG" hint="Extraction vidéo uniquement (50–95). Dossiers d'images : non ré-encodés">
              <SliderInput
            ...
            <ParamRow label="Chunk source (MB)" hint="Upload d'une source monofichier — RAM max pendant l'envoi">
              <SliderInput
            ...
            <ParamRow label="Décimation frames (frame_keep)" hint="Vidéo ou format séquentiel — 0=tout, 2=1/2, 3=1/3, 4=1/4...">
              <NumberInput
            ...
            <ParamRow label="Taille batch images" hint="Upload dossier d'images — envoi séquentiel par batchs">
              <NumberInput
          ...
          {/* ---- Algorithmes ---- */}
          <Section title="Algorithmes" id="algorithms" open={openSections.has('algorithms')} onToggle={toggleSection}>
            <p className="text-xs text-slate-500 pb-2">NMS</p>
            <ParamRow label="Seuil IoU NMS" hint="[0–1] — plus bas = plus agressif">
              <SliderInput
            ...
            <ParamRow
              label="Sortie segmentation par défaut"
              hint="Valeur de départ du sélecteur BBox / Seg de la barre d'outils. Seg = polygones (GD raffiné par SAM2, SAM3, SAM Auto) ; BBox = boîtes brutes."
            >
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                ...
                <span className="text-xs text-slate-400">
                  {settings.algorithms.grounding_dino_use_sam_refine ? 'Oui' : 'Non'}
                </span>
              </label>
            </ParamRow>

            <p className="text-xs text-slate-500 pb-2 pt-3">SAM2 Auto</p>
            <ParamRow label="Points par côté" hint="Grille de points SAM2 auto">
              <NumberInput
            ...
            <p className="text-xs text-slate-500 pb-2 pt-3">Homographie (XFeat / SIFT)</p>
            ...
            <ParamRow label="Min ratio inliers">
              <SliderInput
            ...
            <p className="text-xs text-slate-500 pb-2 pt-3">Flux optique (Lucas-Kanade)</p>
            <ParamRow label="Fenêtre win_size (px)">
              <NumberInput
            ...
            <ParamRow label="Niveaux pyramide max_level">
              <NumberInput
            ...
            <ParamRow label="Points min trackés">
              <NumberInput
            ...
            <p className="text-xs text-slate-600 pb-2">
              Toolbar SAM3 et tracking guidé utilisent les mêmes valeurs. La sortie
              (boîtes ou polygones) suit le sélecteur BBox / Seg de la barre d'outils.
            </p>
            <ParamRow label="Box threshold">
              <SliderInput
            ...
            <p className="text-xs text-slate-500 pb-2 pt-3">Detect. — Matching géométrique</p>
            <ParamRow label="Max distance centroïde" hint="Normalisée [0–1]">
              <SliderInput
            ...
            <ParamRow label="Variation de taille max" hint="Ratio relatif [0–1]">
              <SliderInput
            ...
            <p className="text-xs text-slate-500 pb-2 pt-3">YOLO custom (Detect.)</p>
            <p className="text-xs text-slate-600 pb-2">
              Chemin d'un modèle <code>.pt</code> (absolu, ou relatif au workspace). Utilisé dans
              l'onglet Detect. comme détecteur en mode tracking (matching centroïde → seuil bas OK).
            </p>
            <div className="py-2 border-b border-slate-700/50 space-y-1.5">
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  value={settings.algorithms.yolo_model_path ?? ''}
                  onChange={(e) => { patch('algorithms', 'yolo_model_path', e.target.value); setYoloCheck(null) }}
                  placeholder="ex: models/best.pt  ou  D:\\runs\\detect\\train\\weights\\best.pt"
                  className="flex-1 min-w-0 bg-slate-700 border border-slate-600 text-white text-xs px-2 py-1 rounded outline-none placeholder-slate-500"
                />
                <button
                  onClick={() => {
                    setYoloChecking(true)
                    trackingAPI.getYoloStatus(settings.algorithms.yolo_model_path || undefined)
                      .then(setYoloCheck)
                      .catch(() => setYoloCheck(null))
                      .finally(() => setYoloChecking(false))
                  }}
                  className="flex-shrink-0 text-xs px-2 py-1 rounded bg-purple-600 hover:bg-purple-500 text-white transition-colors"
                >
                  {yoloChecking ? '…' : 'Vérifier'}
                </button>
              </div>
              {yoloCheck && (
                <div className="text-[11px] rounded border border-slate-700 bg-slate-800/50 px-2 py-1">
                  {!yoloCheck.available ? (
                    <span className="text-red-400">ultralytics non installé côté serveur.</span>
                  ) : !yoloCheck.model_found ? (
                    <span className="text-amber-400">Modèle introuvable à ce chemin.</span>
                  ) : (
                    <span className="text-emerald-400">
                      ✓ {Object.keys(yoloCheck.class_names).length} classe(s) · {yoloCheck.device}
                      {' — '}{Object.values(yoloCheck.class_names).slice(0, 6).join(', ')}
                      {Object.keys(yoloCheck.class_names).length > 6 ? '…' : ''}
                    </span>
                  )}
                </div>
              )}
            </div>
            <ParamRow label="Confiance YOLO par défaut" hint="Bas = plus de rappel (le tracking filtre les fausses alarmes)">
              <SliderInput
                value={settings.algorithms.yolo_conf_threshold ?? 0.15}
                min={0.01} max={0.95} step={0.01}
                onChange={(v) => patch('algorithms', 'yolo_conf_threshold', v)}
              />
            </ParamRow>
            <ParamRow label="IoU NMS YOLO" hint="Seuil NMS interne du modèle">
              <SliderInput
                value={settings.algorithms.yolo_iou_threshold ?? 0.7}
                min={0.1} max={0.95} step={0.05}
                onChange={(v) => patch('algorithms', 'yolo_iou_threshold', v)}
              />
            </ParamRow>

            <p className="text-xs text-slate-500 pb-2 pt-3">SAMURAI / SAM2 vidéo</p>
            <ParamRow
              label="Mode GPU rapide"
              hint="Coché (défaut) = frames sur le GPU → 1.5–3x plus rapide, mais limité par la VRAM (~350–450 frames @1024² sur 10 Go). Décoché = offload CPU : VRAM mini, séquences longues, mais plus lent"
            >
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                ...
                />
                <span className="text-xs text-slate-400">
                  {!(settings.algorithms.sam2_offload_video_to_cpu ?? false) ? 'GPU (rapide)' : 'CPU (VRAM mini)'}
                </span>
              </label>
            </ParamRow>

            <p className="text-xs text-slate-500 pb-2 pt-3">Auto-stop global (tous les trackers)</p>
            <ParamRow label="Activer auto-stop" hint="Arrêt si trop d'objets perdus">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                ...
                />
                <span className="text-xs text-slate-400">
                  {settings.algorithms.auto_stop_enabled ? 'Activé' : 'Désactivé'}
                </span>
              </label>
            </ParamRow>
            {settings.algorithms.auto_stop_enabled && (<>
              <ParamRow label="% d'objets perdus max" hint="0.5 = stop si > 50% perdus">
                <SliderInput
              ...
              <ParamRow label="Frames consécutives" hint="Nb frames avant arrêt">
                <SliderInput
          ...
          {/* ---- Stockage ---- */}
          <Section title="Stockage workspace" id="storage" open={openSections.has('storage')} onToggle={toggleSection}>
            {storageLoading ? (
              <p className="text-xs text-slate-500 py-2">Calcul en cours…</p>
            ) : storageStats ? (
              <div className="space-y-2 py-1">
                {/* Projets (lecture seule) */}
                <StorageRow
                  label="Projets (frames + thumbnails)"
                  mb={storageStats.projects_mb}
                  path={storageStats.projects_path}
                />
                {/* Backup */}
                <StorageRow
                  label="Sauvegardes JSON"
              ...
              <p className="text-xs text-slate-500 py-2">Impossible de lire les tailles.</p>
            )}
          </Section>

          {/* ---- Export ---- */}
          <Section title="Export YOLO" id="export" open={openSections.has('export')} onToggle={toggleSection}>
            <ParamRow label="Ratio Train" hint={`Somme : ${exportSum}${exportSum !== 1 ? ' ⚠ doit valoir 1.0' : ' ✓'}`}>
              <SliderInput
            ...
            <ParamRow label="Inclure non-annotées">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="accent-blue-500"
                />
                <span className="text-xs text-slate-400">{settings.export.include_unannotated ? 'Oui' : 'Non'}</span>
              </label>
            </ParamRow>
            <ParamRow label="Liens symboliques images" hint="Dossier local (pas de ZIP) si actif">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="accent-blue-500"
                />
                <span className="text-xs text-slate-400">{settings.export.symlink_images ? 'Oui' : 'Non'}</span>
              </label>
            </ParamRow>
