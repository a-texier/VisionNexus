      ...
      <div className="relative bg-slate-800 border border-slate-700 rounded-xl shadow-2xl w-[680px] max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700">
          <h2 className="text-base font-semibold text-slate-100 flex items-center gap-2">
            <Keyboard size={16} className="text-blue-400" /> {t('Aide — raccourcis & modes')}
          </h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          {tab === 'modes' && (
            <div className="space-y-5">
              <ModeGrid title={t("Modes d'annotation")} items={TOOL_MODES} />
              <ModeGrid title={t('Modes de suivi (Tracks)')} items={TRACKING_MODES} />
              <ModeGrid title={t('Fonctions')} items={FEATURES} />
            </div>
          )}
          ...
          {tab === 'workflow' && (
            <div className="space-y-2">
              {onStartTour && (
                ...
                >
                  <span className="flex items-center gap-1.5"><Video size={13} /> {t('Lancer le tutoriel interactif')}</span>
                  <span className="text-[10px] font-normal text-orange-100/80">
                    {t('Crée un projet démo « Template Cars Annotation » et déroule tout le workflow')}
                  </span>
                </button>
              )}
