        ...
        {!taskId ? (
          <>
            <p className="text-[11px] text-slate-400 mb-3 bg-slate-800/60 border border-slate-700 rounded px-2.5 py-1.5 leading-relaxed">
              Exporte <strong>tout le projet en une fois</strong> — toutes les séquences,
              chacune dans son propre sous-dossier (YOLO) ou fichier <code className="text-orange-400">.ver</code>,
              nommé d'après la séquence.
            </p>

            {/* Format de sortie */}
            <div className="mb-4">
              <span className="text-xs font-medium text-slate-300 block mb-1.5">Format de sortie</span>
              ...
              {outputFormat === 'ver' && (
                <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
                  Format .ver : <code className="text-orange-400">frame_id vis x1 y1 x2 y2 track_id classe sous-classe sous-sous-classe</code> —
                  un fichier .ver par séquence annotée, coordonnées <strong>absolues (pixels)</strong>, frames 1-based.
                </p>
              )}
              {outputFormat === 'coco' && (
                <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
                  Format COCO : <code className="text-emerald-400">annotations/instances_&#123;train,val,test&#125;.json</code> +
                  images/. bbox en <strong>pixels</strong>, <code>category_id</code> 1-based, segmentation
                  incluse pour les polygones (masques SAM). Splits ci-dessous appliqués.
                </p>
              )}
            </div>
            ...
            <div className={`space-y-4 mb-5 ${outputFormat === 'ver' ? 'hidden' : ''}`}>
              ...
              {(valRatio === 0 || testRatio === 0) && (
                <p className="text-xs text-yellow-400/80 text-center">
                  {valRatio === 0 && testRatio === 0
                    ? 'Export 100% train — aucun split val/test'
                    : valRatio === 0 ? 'Aucun split validation'
                    : 'Aucun split test'}
                </p>
              )}
            </div>
              ...
              <label className="flex items-start gap-2 cursor-pointer p-2 rounded-lg border border-slate-700 hover:border-slate-600 transition-colors">
                ...
                <div>
                  <span className="text-xs text-slate-200 flex items-center gap-1">
                    ...
                    Liens symboliques pour les images
                  </span>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {symlinkImages
                      ? 'Crée un dossier dataset avec symlinks — usage direct sur ce serveur, pas de ZIP.'
                      : 'Copie les images et génère un ZIP téléchargeable.'}
                  </p>
                </div>
              </label>
              ...
              {appMode === 'solo' && (
                <div className="p-2 rounded-lg border border-amber-700/40 bg-amber-900/10">
                  ...
                  <input
                    ...
                    placeholder={defaultExportsDir || 'Chemin du dossier de sortie...'}
              ...
              {appMode === 'orchestrator' && (
                <div className="p-2 rounded-lg border border-blue-700/40 bg-blue-900/10">
                  <p className="text-xs text-blue-300 flex items-center gap-1">
                    <FolderOpen size={11} />
                    Export automatiquement enregistré dans le workspace orchestrateur.
                  </p>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-sm text-slate-300">
              {isCompleted ? (
                ...
                <span className="text-red-400">Erreur lors de l'export</span>
              ) : (
                <span>Export en cours... {Math.round(progress * 100)}%</span>
              )}
            </div>
            ...
            {isCompleted && folderPath && (
              <div className="p-2 bg-slate-900/60 rounded-lg border border-slate-700">
                <p className="text-xs text-slate-400 mb-1 flex items-center gap-1">
                  <Link size={11} className="text-blue-400" /> Dataset créé avec symlinks :
                </p>
                <p className="text-xs text-slate-200 font-mono break-all">{folderPath}</p>
                <p className="text-xs text-slate-500 mt-1">
                  Utilisez ce chemin directement avec YOLOv8 sur ce serveur.
                </p>
              </div>
            )}
