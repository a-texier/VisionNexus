...
function formatSize(size: number | null, t: (fr: string) => string): string {
  if (size === null) return ''
  if (size < 1024) return `${size} ${t('o')}`
  if (size < 1024 * 1024) return `${(size / 1024).toFixed(0)} ${t('Ko')}`
  if (size < 1024 * 1024 * 1024) return `${(size / 1024 / 1024).toFixed(1)} ${t('Mo')}`
  return `${(size / 1024 / 1024 / 1024).toFixed(2)} ${t('Go')}`
}
...
export const FileBrowserModal: React.FC<FileBrowserModalProps> = ({
  ...
  const navigate = useCallback(async (path: string) => {
    ...
    try {
      ...
      setError(t('Impossible de lire ce dossier.'))
    } finally {
      setLoading(false)
    }
  }, [filterType])
      ...
      <div className="relative bg-slate-800 border border-slate-700 rounded-xl shadow-2xl flex flex-col"
        ...
        <div className="flex items-center gap-1 px-3 py-2 border-b border-slate-700/50 bg-slate-900/30">
          ...
          <div className="flex items-center gap-0.5 text-xs text-slate-400 overflow-x-auto flex-1 min-w-0">
            {pathParts.length === 0 ? (
              <span className="text-slate-500 italic">{t('Racine')}</span>
            ) : (
          ...
          {!loading && !error && entries.map((entry) => {
            ...
            return (
              ...
              >
                ...
                {entry.size !== null && (
                  <span className="text-xs text-slate-600 flex-shrink-0">{formatSize(entry.size, t)}</span>
                )}
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-t border-slate-700 bg-slate-900/20">
          ...
          <div className="flex-1 min-w-0">
            {selectDir && currentPath && (
              <p className="text-xs text-slate-400 truncate">
                <span className="text-slate-600">{t('Dossier')} : </span>{currentPath}
              </p>
            )}
            {!selectDir && selected && (
              <p className="text-xs text-slate-400 truncate">
                <span className="text-slate-600">{t('Fichier')} : </span>{selected.name}
              </p>
            )}
            {!selectDir && !selected && !selectDir && (
              <p className="text-xs text-slate-600 italic">{t('Double-clic sur un fichier pour sélectionner')}</p>
            )}
          </div>
          <button
            onClick={onClose}
          ...
          >
            <Check size={11} />
            {selectDir ? t('Choisir ce dossier') : t('Sélectionner')}
          </button>
        </div>
      </div>
    </div>
