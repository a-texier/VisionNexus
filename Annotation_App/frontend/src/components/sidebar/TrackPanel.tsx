...
export const TrackPanel: React.FC<TrackPanelProps> = ({
  ...
  const [algorithm, setAlgorithm] = useState<'grounding_dino' | 'sam3' | 'yolo'>('grounding_dino')
  const [textPrompt, setTextPrompt] = useState('')
  // --- YOLO custom (tracking guidé) ---
  const [yoloConf, setYoloConf] = useState(algo?.yolo_conf_threshold ?? 0.15)
  const [yoloIou, setYoloIou] = useState(algo?.yolo_iou_threshold ?? 0.7)
  const [yoloStatus, setYoloStatus] = useState<import('../../services/api').YoloStatus | null>(null)
  ...
  useEffect(() => {
    ...
    if (algo.yolo_conf_threshold !== undefined) setYoloConf(algo.yolo_conf_threshold)
    if (algo.yolo_iou_threshold !== undefined) setYoloIou(algo.yolo_iou_threshold)
  ...
  // YOLO : (re)charger le statut du modèle configuré quand on sélectionne YOLO
  useEffect(() => {
    if (algorithm !== 'yolo') return
    let cancelled = false
    trackingAPI.getYoloStatus()
      .then((s) => { if (!cancelled) setYoloStatus(s) })
      .catch(() => { if (!cancelled) setYoloStatus(null) })
  ...
  useEffect(() => {
    ...
    const applyUpdate = (
      ...
      if (status === 'completed') {
        ...
        setIsGuidedTask(false)
        setTaskId(null)
        setRunStatus(`Erreur : ${error ?? 'inconnue'}`)
        ...
        while (!httpController.signal.aborted) {
          ...
          try {
            ...
            if (networkErrorCountRef.current >= 10) {
              ...
              setRunStatus('Connexion backend perdue : polling arrete.')
              setTimeout(() => setRunStatus(''), 5000)
              break
            }
          }
  ...
  const handleRunGuidedTracking = async () => {
    ...
    if (selectedAnnotationIds.size === 0) {
      setRunStatus('Selectionnez au moins une annotation cible')
      setTimeout(() => setRunStatus(''), 3000)
      return
    }
    if (algorithm === 'yolo') {
      if (!yoloStatus?.model_found) {
        setRunStatus('Aucun modèle YOLO valide (Paramètres > Algorithmes)')
        setTimeout(() => setRunStatus(''), 3500)
        return
      }
    } else if (!textPrompt.trim()) {
      setRunStatus('Le prompt texte est obligatoire')
      setTimeout(() => setRunStatus(''), 3000)
      return
    }
    setIsRunning(true)
    ...
    setTaskProgress(0)
    setRunStatus('Lancement detection + association...')
    ...
    try {
      const result = await trackingAPI.runGuidedTracking(projectId, {
        ...
        ...(algorithm === 'yolo'
          ? { yolo_conf_threshold: yoloConf, yolo_iou_threshold: yoloIou }
          : {}),
        auto_stop_lost_ratio: autoStopEnabled ? autoStopLostRatio : 0.0,
        auto_stop_consecutive_frames: autoStopConsecutiveFrames,
      })
      setTaskId(result.task_id)
      onPropagationStarted(result.task_id, 'Detection + association')
      setRunStatus(`En cours (${result.targets_count} cibles, ${result.frames_to_process} frames)...`)
    } catch (e) {
      setIsRunning(false)
      setRunStatus(apiErrorDetail(e) ?? 'Erreur lancement tracking')
  ...
  const handleRunXFeat = async () => {
    ...
    if (annotationIds.length === 0) {
      setRunStatus('Aucune annotation sur cette frame')
      setTimeout(() => setRunStatus(''), 3000)
      return
    }
    const endFrame = await resolveFrameByIndex(xfeatEndFrameIndex)
    if (!endFrame) {
      setRunStatus('Frame de fin introuvable')
      setTimeout(() => setRunStatus(''), 3000)
      return
    }
    if (endFrame.frame_index <= currentFrame.frame_index) {
      setRunStatus(`La frame de fin (${endFrame.frame_index}) doit être après la frame courante (${currentFrame.frame_index})`)
      setTimeout(() => setRunStatus(''), 4000)
      return
    }
    setIsRunning(true)
    setTaskProgress(0)
    setRunStatus('Lancement propagation homographique...')
    try {
      ...
      onPropagationStarted(result.task_id, 'Propagation homographie')
      setTaskId(result.task_id)
    } catch (e) {
      setIsRunning(false)
      setRunStatus(apiErrorDetail(e) ?? 'Erreur propagation')
  ...
  const handleRunOptflow = async () => {
    ...
    setIsRunning(true)
    setTaskProgress(0)
    setRunStatus('Lancement flux optique...')
    try {
      const result = await trackingAPI.propagateHomography(projectId, {
        keyframe_id: currentFrame.id,
        end_frame_id: endFrame.id,
      ...
      onPropagationStarted(result.task_id, 'Flux optique LK')
      setTaskId(result.task_id)
    } catch (e) {
      setIsRunning(false)
      setRunStatus(apiErrorDetail(e) ?? 'Erreur flux optique')
  ...
  const handleRunSam2Tracking = async () => {
    ...
    const annotationIds = trackingTargetSet.size > 0
      ...
      : annotations.map((a) => a.id)
    if (annotationIds.length === 0) {
      setRunStatus('Aucune annotation sur cette frame'); setTimeout(() => setRunStatus(''), 3000); return
    }
    const endFrame = await resolveFrameByIndex(sam2EndFrameIndex)
    if (!endFrame) { setRunStatus('Frame de fin introuvable'); setTimeout(() => setRunStatus(''), 3000); return }
    if (endFrame.frame_index === currentFrame.frame_index) {
      setRunStatus('La frame de fin doit être différente de la frame courante')
    ...
    setRunStatus(`Lancement ${samuraiLabel} video tracking${isReverse ? ' (sens inverse)' : ''}...`)
    try {
      ...
      onPropagationStarted(result.task_id,
        `${samuraiLabel} video tracking${isReverse ? ' (inverse)' : ''}`)
      setTaskId(result.task_id)
    } catch (e) {
      setIsRunning(false); setRunStatus(apiErrorDetail(e) ?? 'Erreur SAM2 tracking')
  ...
  const handleDeleteAllTracks = async () => {
    if (!window.confirm(`Supprimer tous les tracks de ce projet ? Les annotations restent mais seront détachées.`)) return
    try {
  ...
  const handleDeleteTrack = async (trackId: number) => {
    ...
    const uid = t ? t.track_uid : trackId
    if (!window.confirm(
      `Supprimer le track #${uid} ?\n\n` +
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-2 py-2 border-b border-slate-700 bg-slate-800/50">
        <div className="flex items-center justify-between">
          <span className="text-xs font-medium text-slate-300">Frame courante</span>
          <span className="text-xs font-mono text-blue-400 bg-blue-900/30 px-2 py-0.5 rounded">
            #{currentFrameIndex}
          </span>
        </div>
        <p className="text-xs text-slate-500 mt-0.5">
          {annotations.length} annotation{annotations.length !== 1 ? 's' : ''} disponible{annotations.length !== 1 ? 's' : ''}
        </p>
      </div>
      ...
      <div className="flex border-b border-slate-700 flex-shrink-0 overflow-x-auto scrollbar-none">
        {(
          [
            { id: 'samurai', label: 'SAMURAI',  icon: <Layers size={10} />,      activeColor: 'text-teal-400 border-teal-400 bg-teal-900/10',     title: 'SAMURAI local si disponible, fallback SAM2' },
            { id: 'guided',  label: 'Detect.',  icon: <Zap size={10} />,         activeColor: 'text-blue-400 border-blue-400 bg-blue-900/10',    title: 'GD/SAM3 + matching centroide' },
            { id: 'xfeat',   label: 'Homogr.',  icon: <ArrowRight size={10} />,  activeColor: 'text-purple-400 border-purple-400 bg-purple-900/10', title: 'Propagation par homographie XFeat/SIFT' },
            { id: 'optflow', label: 'Flux opt.', icon: <Wind size={10} />,        activeColor: 'text-green-400 border-green-400 bg-green-900/10',  title: 'Flux optique Lucas-Kanade' },
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-2 space-y-3">
            <p className="text-xs text-slate-400 leading-relaxed">
              ...
              Mode utile quand l'objet reste proche de sa position attendue.
            </p>

            {/* Selection des annotations cibles */}
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-slate-300">Cibles a suivre</span>
                {annotations.length > 0 && (
                  ...
                  >
                    {selectedAnnotationIds.size === annotations.length ? 'Tout désel.' : 'Tout sél.'}
                  </button>
                )}
              </div>
              {annotations.length === 0 && !isRunning ? (
                <p className="text-xs text-amber-400 bg-amber-900/20 px-2 py-1.5 rounded border border-amber-700/30">
                  Annotez d'abord la frame #{currentFrameIndex}.
                </p>
              ) : (
            ...
            {/* Plage de frames */}
            <div>
              <span className="text-xs font-medium text-slate-300 block mb-1">Plage de frames</span>
            ...
            {/* Algorithme */}
            <div>
              <span className="text-xs font-medium text-slate-300 block mb-1">Algorithme</span>
              <div className="flex gap-1">
                {(['grounding_dino', 'sam3', 'yolo'] as const).map((alg) => (
                  <button
                    key={alg}
                  ...
                  >
                    {alg === 'grounding_dino' ? 'GDINO' : alg === 'sam3' ? 'SAM3.1' : 'YOLO'}
                  </button>
                ))}
              </div>
            </div>

            {/* YOLO custom : statut du modèle configuré + seuil de confiance */}
            {algorithm === 'yolo' && (
              <div className="space-y-1.5">
                <div className="text-[11px] rounded border border-slate-700 bg-slate-800/50 px-2 py-1.5">
                  {!yoloStatus ? (
                    <span className="text-slate-500">Vérification du modèle…</span>
                  ) : !yoloStatus.available ? (
                    <span className="text-red-400">ultralytics non installé côté serveur.</span>
                  ) : !yoloStatus.model_found ? (
                    <span className="text-amber-400">
                      Aucun modèle YOLO. Définissez un chemin dans <b>Paramètres &gt; Algorithmes</b>.
                    </span>
                  ) : (
                    <div className="text-slate-400">
                      <div className="text-emerald-400 truncate" title={yoloStatus.model_resolved ?? ''}>
                        ✓ {yoloStatus.model_resolved?.split(/[\\/]/).pop()}
                      </div>
                      <div className="text-[10px] text-slate-500 mt-0.5">
                        {Object.keys(yoloStatus.class_names).length} classe(s) · {yoloStatus.device}
                      </div>
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-500 w-28 flex-shrink-0">Confiance YOLO</span>
                  <input
                  ...
                  />
                  <span className="text-[10px] text-slate-600">bas = + de rappel</span>
                </div>
                <p className="text-[10px] text-slate-600 leading-snug pl-1">
                  Seuil volontairement <b>bas</b> : le matching centroïde vers vos cibles écarte
                  les fausses alarmes → bon rappel sans polluer les annotations.
                </p>
              </div>
            )}
            ...
            {algorithm === 'sam3' && (
              <div>
                <span className="text-xs font-medium text-slate-300 block mb-1">Mode de sortie</span>
                <div className="flex gap-1">
            ...
            {/* Prompt texte (GD/SAM3 uniquement — YOLO a une tâche fixe) */}
            {algorithm !== 'yolo' && (
              <div>
                <span className="text-xs font-medium text-slate-300 block mb-1">Prompt de detection</span>
                <textarea
                  value={textPrompt}
                  onChange={(e) => setTextPrompt(e.target.value)}
                  data-tour="guided-prompt"
                  placeholder='ex: "voiture. personne. velo."'
                  rows={2}
                ...
                />
                <p className="text-xs text-slate-600 mt-0.5">Separez les classes par des points.</p>
              </div>
            )}
            ...
            {showAdvanced && (
              <div className="space-y-1.5 pl-2 border-l border-slate-700">
                {[
                  { label: 'Seuil boite', value: boxThreshold, set: setBoxThreshold, min: 0.05, max: 0.9, step: 0.05,
                    hint: 'Confiance minimale des détections. ↑ = moins de boîtes, plus fiables.' },
                  { label: 'Seuil texte', value: textThreshold, set: setTextThreshold, min: 0.05, max: 0.9, step: 0.05,
                    hint: 'Correspondance prompt ↔ boîte (Grounding DINO). ↑ = plus strict sur le mot.' },
                  { label: 'Dist. centroide', value: maxCentroidDist, set: setMaxCentroidDist, min: 0.01, max: 0.5, step: 0.01,
                    hint: 'Écart max cible ↔ détection pour les apparier, en fraction de l\'image (0 = même point, 1 ≈ un bord à l\'autre). 0.15 = 15 % de l\'image.' },
                  { label: 'Var. taille max', value: sizeVarThreshold, set: setSizeVarThreshold, min: 0.1, max: 2.0, step: 0.1,
                    hint: 'Variation de SURFACE tolérée entre 2 frames avant de signaler une anomalie. 0.5 = ±50 %, 1.0 = ×2.' },
                ...
                <div className="pt-1 border-t border-slate-700/50">
                  <label className="flex items-center gap-2 cursor-pointer mb-1.5">
                    ...
                    />
                    <span className="text-xs text-slate-400">Auto-stop si objets perdus</span>
                  </label>
                  {autoStopEnabled && (
                    <div className="space-y-1 pl-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-slate-500 w-28 flex-shrink-0">% perdu max</span>
                        <input
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-slate-500 w-28 flex-shrink-0">Frames consec.</span>
                        <input
            ...
            <button
              ...
              disabled={isRunning || currentFrameTargetCount === 0 ||
                (algorithm === 'yolo' ? !yoloStatus?.model_found : !textPrompt.trim())}
            ...
            >
              <Play size={11} />
              {isRunning ? 'Propagation en cours...' : `Detecter + associer (${currentFrameTargetCount} cible${currentFrameTargetCount !== 1 ? 's' : ''})`}
            </button>
          </div>
        </div>
      )}
      ...
      {tab === 'xfeat' && (
        <div className="p-2 border-b border-slate-700 space-y-2">
          <p className="text-xs text-slate-400 leading-relaxed">
            Propage les annotations de la frame #{currentFrameIndex} vers les suivantes par matching de keypoints (XFeat GPU ou SIFT+RANSAC).
          </p>

          <div className="p-2 bg-slate-800/60 rounded border border-slate-700/50 space-y-1">
            <div className="flex justify-between text-xs">
              <span className="text-slate-500">Annotations source</span>
              <span className="text-slate-200">{annotations.length}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-slate-500">Frame source</span>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-medium text-slate-300">Annotations à propager</span>
              {annotations.length > 0 && (
                <button
                  onClick={() =>
                  ...
                  className="text-xs text-slate-500 hover:text-slate-300"
                >
                  {trackingTargetSet.size === annotations.length ? 'Tout désel.' : 'Tout sél.'}
                </button>
              )}
            </div>
            {annotations.length === 0 && !isRunning ? (
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 flex-shrink-0">Jusqu'a la frame</span>
            <input
              type="number"
          ...
          <div className="space-y-1">
            ...
            {[
              { label: 'Inliers min.', value: minInlierCount, set: (v: number) => setMinInlierCount(v), min: 5, max: 500, step: 5, hint: 'pts — augmenter si dérive' },
              { label: 'Ratio inliers', value: minInlierRatio, set: (v: number) => setMinInlierRatio(v), min: 0.1, max: 0.95, step: 0.05, hint: '0-1 — 0.5 = 50% min.' },
              { label: 'Seuil reproj.', value: ransacThreshold, set: (v: number) => setRansacThreshold(v), min: 0.5, max: 10, step: 0.5, hint: 'px — + strict = moins de bruit' },
            ].map(({ label, value, set, min, max, step, hint }) => (
          ...
          {homographyMethod === 'xfeat' && (
            <div className="space-y-1">
              ...
              {[
                { label: 'Top-K pts', value: xfeatTopK, set: (v: number) => setXfeatTopK(v), min: 512, max: 8192, step: 512, hint: 'keypoints/image' },
                { label: 'Min cossim', value: xfeatMinCossim, set: (v: number) => setXfeatMinCossim(v), min: 0.5, max: 0.99, step: 0.02, hint: '0.82 = strict' },
              ].map(({ label, value, set, min, max, step, hint }) => (
                <div key={label} className="flex items-center gap-2">
                  <span className="text-xs text-slate-500 w-24 flex-shrink-0">{label}</span>
                  <input
          ...
          >
            <ArrowRight size={11} />
            {isRunning ? 'Propagation...' : 'Propager par homographie'}
          </button>
        </div>
      )}
      ...
      {tab === 'optflow' && (
        <div className="flex-1 overflow-y-auto">
          <div className="p-2 space-y-3">
            <p className="text-xs text-slate-400 leading-relaxed">
              Suivi par <span className="text-green-400 font-medium">flux optique Lucas-Kanade</span> —
              suit le mouvement réel de chaque objet (pas seulement la caméra).
              Idéal pour objets en mouvement (véhicules, personnes).
            </p>

            {/* Sélection annotations */}
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-slate-300">Cibles à suivre</span>
                {annotations.length > 0 && (
                  <button
                    onClick={() =>
                      trackingTargetSet.size === annotations.length
            ...
            {/* Frame de fin */}
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-400 flex-shrink-0">Jusqu'à la frame</span>
              <input
                type="number"
                min={toLocal(currentFrameIndex + 1)}
                max={seqFrameCount}
            ...
            <div className="space-y-1">
              ...
              {[
                { label: 'Fenêtre (px)', value: optflowWinSize, set: setOptflowWinSize, min: 5, max: 63, step: 2, hint: '21 — plus grand = + robuste' },
                { label: 'Niveaux pyra.', value: optflowMaxLevel, set: setOptflowMaxLevel, min: 1, max: 6, step: 1, hint: '3 — + niveaux = gds dépl.' },
                { label: 'Pts min.', value: optflowMinPts, set: setOptflowMinPts, min: 1, max: 25, step: 1, hint: '4 — si < → bbox copiée' },
              ].map(({ label, value, set, min, max, step, hint }) => (
                <div key={label} className="flex items-center gap-2">
                  <span className="text-xs text-slate-500 w-24 flex-shrink-0">{label}</span>
                  <input
            ...
            <div className="p-2 bg-green-900/10 rounded border border-green-800/30 text-xs text-green-300 leading-relaxed">
              Grille <strong>5×5</strong> de points dans chaque bbox → translation médiane robuste aux outliers.
              Ne compense <strong>pas</strong> la rotation/zoom — utiliser Homographie pour ça.
            </div>
            ...
            >
              <Wind size={11} />
              {isRunning ? 'Suivi en cours...' : 'Suivre par flux optique'}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-2 space-y-3">
            <p className="text-xs text-slate-400 leading-relaxed">
              ...
              Robuste aux occlusions partielles et changements de pose.
            </p>
            ...
            {samuraiStatus !== null && (
              <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium w-fit ${
                ...
                {samuraiStatus.loaded ? 'SAMURAI actif (Kalman)' : samuraiStatus.installed ? 'SAMURAI installé, SAM2 en cours' : 'SAM2 standard (SAMURAI absent)'}
              </div>
            )}
            {samuraiStatus === null || (!samuraiStatus.installed) ? (
              <p className="text-xs text-slate-600">
                Installer SAMURAI : exécuter <code className="text-teal-500">install_samurai.bat</code>
              </p>
            ) : null}
                ...
                {annotations.length > 0 && (
                  <button
                    onClick={() => trackingTargetSet.size === annotations.length
                      ...
                      : setTrackingTargets(annotations.map((a) => a.id))}
                    className="text-xs text-slate-500 hover:text-slate-300"
                  >{trackingTargetSet.size === annotations.length ? 'Tout désel.' : 'Tout sél.'}</button>
                )}
              </div>
              {annotations.length > 1 && (
                <p className="text-[10px] text-slate-500 mb-1 leading-snug">
                  Double-clic sur une boîte du canvas = cocher/décocher cette cible
                  (anneau pointillé autour de l'objet).
                </p>
              )}
              {annotations.length === 0 && !isRunning ? (
                <p className="text-xs text-amber-400 bg-amber-900/20 px-2 py-1.5 rounded border border-amber-700/30">
            ...
            <div className="flex items-center gap-2" data-tour="samurai-end-frame">
              ...
              {sam2EndFrameIndex < currentFrameIndex && (
                <span className="text-[10px] text-amber-400 flex items-center gap-1 flex-shrink-0"
                      title="La frame de fin est avant la frame courante : la propagation remonte le temps">
                  <ArrowLeft size={10} /> sens inverse
                </span>
              )}
            </div>
                ...
                return (
                  <div className="text-xs bg-slate-800/60 rounded px-2 py-1.5 border border-slate-700/50 space-y-0.5">
                    <div className="flex justify-between text-slate-400">
                      <span className="font-medium">Offload CPU actif</span>
                      <span className="text-slate-500">{g.name} · {g.vram_free_gb}/{g.vram_total_gb} Go</span>
                    </div>
                    <p className="text-slate-600 leading-snug">
                      Frames en RAM → aucune limite VRAM, mais ~1.5–3× plus lent. Décoche
                      « Offload CPU » dans les paramètres pour le mode GPU rapide.
                    </p>
                  </div>
                )
              }
              ...
              return (
                <div className="text-xs bg-slate-800/60 rounded px-2 py-1.5 border border-slate-700/50 space-y-1"
                  ...
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-slate-300">Frames max estimées (GPU rapide)</span>
                  ...
                  {over && (
                    <p className="text-red-400/90 leading-snug">
                      Plage &gt; capacité VRAM → risque d'OOM. Réduis la plage, décime à
                      l'import, ou coche « Offload CPU » dans les paramètres.
                    </p>
                  )}
                </div>
              )
              ...
              <div className="flex gap-1">
                {(['bbox', 'segmentation'] as const).map((m) => (
                  <button key={m} onClick={() => setSam2OutputMode(m)}
                    ...
                    }`}>{m === 'bbox' ? 'BBox' : 'Segmentation'}</button>
                ))}
              </div>
            </div>
            ...
            <div data-tour="samurai-strategy">
              <span className="text-xs font-medium text-slate-300 block mb-1">Stratégie multi-cible</span>
              <div className="flex gap-1">
                ...
                <button onClick={() => setSam2TrackingMode('samurai_per_object')}
                  ...
                  title={samuraiStatus?.loaded ? '' : 'Nécessite SAMURAI chargé'}
                  ...
                  }`}>SAMURAI / objet</button>
              </div>
            </div>

            <div className="p-2 bg-teal-900/10 rounded border border-teal-800/30 text-xs text-teal-300 leading-relaxed space-y-1">
              <p>
                <strong>SAMURAI</strong> (filtre de Kalman) ne suit qu'<strong>une</strong> cible :
                son état de mouvement est unique. Pour <strong>plusieurs</strong> cibles :
              </p>
              <p>
                • <strong>Auto</strong> = <strong>SAM2 multi-objets</strong> natif — 1 seule passe,
                rapide, mais sans Kalman.
              </p>
              <p>
                • <strong>SAMURAI / objet</strong> = 1 passe SAMURAI (Kalman) par cible —
                meilleur suivi MOT (occlusions, objets similaires), mais ~N× plus lent.
              </p>
              <p className="text-teal-400/70">
                Prompt = box de référence. Nécessite <strong>sam2.1_hiera_small.pt</strong> (présent).
                Officiel : <em>yangchris11/samurai</em>.
              </p>
            </div>
            ...
            <button onClick={() => void handleRunSam2Tracking()}
              ...
              <Layers size={11} />
              {isRunning
                ? `Propagation ${samuraiStatus?.loaded ? 'SAMURAI' : 'SAM2'} en cours...`
                : `Propager par ${samuraiStatus?.loaded ? 'SAMURAI' : 'SAM2'}`}
            </button>
          </div>
        </div>
      )}
