    ...
    <div className="flex flex-col gap-1 p-2 overflow-y-auto">
      ...
      {isCreating && (
        <div className="bg-slate-700 rounded p-2 mb-2 flex flex-col gap-1.5">
          <input
            autoFocus
            type="text"
            placeholder={t('Classe (détection) * — ex : drone')}
            value={newClassName}
          ...
          <input
            type="text"
            placeholder={t('Sous-classe (reconnaissance) — ex : quadcoptere')}
            value={newSubclass}
          ...
          <input
            type="text"
            placeholder={t('Sous-sous-classe (identification) — ex : mavic')}
          ...
          <p className="text-[10px] text-slate-400 leading-tight">
            {t('Seule la classe de base est obligatoire.')}
          </p>
        </div>
      )}
          ...
          {editingId === cls.id ? (
            <div className="flex-1 flex flex-col gap-1" onClick={(e) => e.stopPropagation()}>
              ...
              <input
                ...
                placeholder={t('Sous-sous-classe')}
      ...
      {classes.length === 0 && !isCreating && (
        <p className="text-xs text-slate-500 text-center py-4">
          {t('Aucune classe définie. Cliquez sur + pour créer.')}
        </p>
      )}
    </div>
  )
