...
export const GlobalAnnotationsPanel: React.FC<GlobalAnnotationsPanelProps> = ({
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      ...
      setError(`Impossible de charger le résumé. ${msg}`)
    } finally {
      setIsLoading(false)
    }
  }, [projectId])
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      setError('Erreur lors de la suppression batch')
    } finally {
  ...
  const handleDeleteFromFrame = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Erreur inconnue'
      setError(`Erreur lors de la suppression : ${msg}`)
    } finally {
      setIsBatchDeleting(false)
  ...
  return (
    <div className="flex flex-col h-full text-xs">
      ...
      <div className="px-2 py-1 border-b border-slate-700 flex items-center gap-2 flex-shrink-0">
        <span className="text-slate-500">Score min</span>
        <input
      ...
      {filtered.length > 0 && (
        <div className="px-2 py-1 border-b border-slate-700 flex items-center gap-2 flex-shrink-0">
          ...
          <span className="flex-1 text-slate-600 text-right">
            {selected.size > 0 ? `${selected.size} sél.` : ''}
          </span>
          {selected.size > 0 && (
            <button
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="text-slate-600 text-center py-8 px-4">
            <p className="text-sm mb-2">Résumé indisponible</p>
            <p className="text-xs">Vérifiez que le serveur backend est démarré.</p>
          </div>
        ) : filtered.length === 0 ? (
          <p className="text-slate-500 text-center py-6">
            {items.length === 0 ? 'Aucune annotation dans ce projet.' : 'Aucune annotation pour ce filtre.'}
          </p>
        ) : (
          filtered.map((item) => {
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  title={`Aller à la frame ${item.frame_index}`}
                >
                  <Navigation size={11} />
                </button>
              </div>
        ...
        {showDeleteFrom && (
          <div className="px-2 pb-2 bg-red-900/10 border-t border-red-900/30">
            <p className="text-slate-500 mt-1.5 mb-1">
              Supprime toutes les annotations à partir de la frame d'index :
            </p>
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              >
                ...
                <option value="ia">IA seulement</option>
              </select>
            </div>
            {!confirmDeleteFrom ? (
              <button
              ...
              <div className="flex gap-1">
                <span className="text-red-400 flex-1">Confirmer ?</span>
                <button
