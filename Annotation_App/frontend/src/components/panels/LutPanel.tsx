  ...
  return (
    <div data-tour="lut-panel"
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-800">
        ...
        <span className="text-sm font-semibold text-white flex-1">{t('LUT / Affichage')}</span>
      ...
      <div className="p-3 space-y-2.5">
        ...
        <div>
          <div className="flex rounded-lg border border-gray-700 overflow-hidden text-[11px] font-medium">
            ...
            <button onClick={() => canScopeSequence && setScope('sequence')}
              ...
              title={canScopeSequence ? '' : t('Séquence principale : réglez la LUT au niveau projet')}
        ...
        {/* Modes */}
        <div className="flex rounded-lg border border-gray-700 overflow-hidden text-[11px] font-medium">
          <button onClick={() => chooseMode('sigma')} className={`flex-1 flex items-center justify-center gap-1 px-2 py-1.5 ${mode === 'sigma' ? 'bg-cyan-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-gray-200'}`}><Wand2 size={11} />{t('Auto σ')}</button>
          ...
          <button onClick={() => chooseMode('manual')} className={`flex-1 flex items-center justify-center gap-1 px-2 py-1.5 border-l border-gray-700 ${mode === 'manual' ? 'bg-cyan-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-gray-200'}`}><SlidersHorizontal size={11} />{t('Manuel')}</button>
        </div>

        {/* Contrôles selon le mode */}
        {mode === 'sigma' && (
          <div>
            <div className="flex justify-between text-[11px] text-gray-400"><span>{t('Sigma (N)')}</span><span className="text-cyan-300 font-mono">{sigma.toFixed(1)}σ</span></div>
            ...
            <p className="text-[10px] text-gray-600">{t('Étire [moy − Nσ, moy + Nσ] → 0-255. Défaut 3σ.')}</p>
          </div>
        )}
        {mode === 'minmax' && (
          <p className="text-[10px] text-gray-500">{t('Étire [min, max] réels de la frame → 0-255 (contraste maximal).')}</p>
        )}
        {mode === 'manual' && (
          <div className="space-y-1.5">
            <div className="grid grid-cols-2 gap-2">
              <label className="text-[11px] text-gray-400">{t('Bas (lo)')}
              ...
              <label className="text-[11px] text-gray-400">{t('Haut (hi)')}
        ...
        <p className="text-[10px] text-gray-600">
          {scope === 'sequence'
            ? t('Mémorisé pour cette séquence (prioritaire sur le projet) et appliqué à l’IA.')
            : t('Mémorisé pour tout le projet.')}
        </p>
      </div>
    </div>
  )
