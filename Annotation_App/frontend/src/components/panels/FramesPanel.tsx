...
const FrameJumpInput: React.FC<{ total: number; onJump: (index: number) => void }> = ({ total, onJump }) => {
  ...
  return (
    <input
      ...
      placeholder={t('N°')}
      title={`${t('Aller à la frame')} (0–${total - 1}), ${t('Entrée pour valider')}`}
    ...
    >
      ...
      <div className="flex items-center gap-1.5 pl-3 pr-2 py-2 border-b border-slate-700 flex-shrink-0">
        ...
        {selectedIndices.size > 0 && (
          <button
            ...
            title={`${t('Supprimer les annotations sur')} ${selectedIndices.size} ${t('frame(s) sélectionnée(s)')}`}
          >
            <Trash2 size={10} />
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {frame.is_empty && (
                  <div className="text-xs text-slate-500 leading-tight">{t('vide')}</div>
                )}
              </div>
      ...
      {selectedIndices.size > 0 && (
        <div className="px-3 py-1.5 border-t border-slate-700 flex-shrink-0 text-xs text-slate-500">
          {selectedIndices.size} frame{selectedIndices.size > 1 ? 's' : ''} — {t('Suppr. pour effacer')}
        </div>
      )}
    </div>
  )
