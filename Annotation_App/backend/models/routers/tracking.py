...
class GuidedTrackingRequest(BaseModel):
    ...
    algorithm: str = "grounding_dino"  # "grounding_dino" | "sam3" | "yolo"
    text_prompt: str = ""              # Prompt texte (GD/SAM3 ; ignore pour YOLO)
    ...
    # --- YOLO custom (algorithm="yolo") ---
    yolo_model_path: Optional[str] = None   # override ; sinon settings.algorithms.yolo_model_path
    yolo_conf_threshold: float = 0.15       # seuil confiance BAS (tracking filtre les FA)
    yolo_iou_threshold: float = 0.7         # NMS interne YOLO
    yolo_class_filter: Optional[List[int]] = None  # indices classes YOLO a garder (None=toutes)
...
def run_guided_tracking(
    ...
    if data.algorithm not in ("grounding_dino", "sam3", "yolo"):
        raise HTTPException(status_code=400, detail="Algorithme invalide (grounding_dino | sam3 | yolo)")

    # Le prompt texte est obligatoire pour GD/SAM3 ; YOLO a une tache fixe (pas de prompt).
    if data.algorithm != "yolo" and not data.text_prompt.strip():
        raise HTTPException(status_code=400, detail="Le prompt texte est obligatoire")

    # YOLO : resolution du modele (override requete, sinon settings) avant de lancer.
    yolo_model_path = None
    if data.algorithm == "yolo":
        from backend.services.settings_service import settings_service
        from backend.services.yolo_service import get_yolo_service, resolve_model_path
        yolo_model_path = data.yolo_model_path or (
            settings_service.load().get("algorithms", {}).get("yolo_model_path", "")
        )
        if not get_yolo_service().is_available:
            raise HTTPException(status_code=503, detail="ultralytics non disponible (detection YOLO)")
        if resolve_model_path(yolo_model_path) is None:
            raise HTTPException(
                status_code=400,
                detail=f"Modele YOLO introuvable : '{yolo_model_path}'. "
                       f"Definissez un chemin valide dans Parametres > Algorithmes.",
            )
    ...
    async def _run():
        ...
        if data.algorithm == "grounding_dino":
            ...
            if not detector.load():
                ...
                return
        elif data.algorithm == "yolo":
            from backend.services.yolo_service import get_yolo_service
            detector = get_yolo_service()
            # Modele resolu en amont (yolo_model_path). Chargement paresseux au 1er detect.
            append_log(task_id, f"[YOLO] modele {yolo_model_path} conf>={data.yolo_conf_threshold}")
        else:
            ...
            with DBSession(engine) as db:
                ...
                try:
                    if data.algorithm == "grounding_dino":
                        ...
                        _conf = data.yolo_conf_threshold
                        _iou = data.yolo_iou_threshold
                        _cls_filter = data.yolo_class_filter
                        _img_width = frame.width
                        _img_height = frame.height
                        detections = await loop.run_in_executor(
                            None,
...
@router.get("/api/yolo/status", response_model=dict)
def yolo_status(model_path: Optional[str] = None):
    """Statut du détecteur YOLO custom : ultralytics dispo, modèle configuré
    trouvé, et liste des classes du modèle. Si `model_path` n'est pas fourni,
    on prend celui des paramètres (settings.algorithms.yolo_model_path)."""
    from backend.services.yolo_service import get_yolo_service
    from backend.services.settings_service import settings_service
    mp = model_path
    if mp is None:
        mp = settings_service.load().get("algorithms", {}).get("yolo_model_path", "")
    return get_yolo_service().status(mp)
