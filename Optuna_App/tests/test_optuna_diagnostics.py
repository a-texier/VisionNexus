...
class HistoricalDiagnosticsTests(unittest.TestCase):
    ...
    def test_stale_windows_trial_reports_observed_files_and_workers_hypothesis(self):
        # hpo_trial.py (moteur YOLOX) n'ecrit plus d'args.yaml (artefact
        # Ultralytics) : le nombre de workers se lit desormais dans la
        # commande subprocess stockee sur le trial (cf. _workers_from_command).
        trial = FakeTrial(number=0, state="RUNNING")
        trial.user_attrs["command"] = ["python", "hpo_trial.py", "--workers", "8"]
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
