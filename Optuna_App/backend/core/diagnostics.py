...
def diagnose_failure(raw: str | None) -> dict[str, str]:
    ...
    low = text.lower()
    rules = [
        # Avant les regles modele : une trace MLflow contient "model"/"store" et se
        ...
        (("mlflow", "tracking backend", "maintenance mode"),
         ...
         "Le trial est mort dans le logging MLflow, pas dans l'entrainement YOLOX. "
        ...
        (("no such file", "weights", ".pth", "model"),
         "model_missing", "Poids ou modèle introuvable",
         "Vérifiez la taille de modèle YOLOX, le chemin du .pth et l’accès au cache de poids."),
        (("timeout", "timed out"),
