...
def _legacy_log_diagnostic(trial, logs_root: Path | None = None) -> dict | None:
    ...
    Le motif recherché (get_dataset/check_det_dataset) est specifique aux
    anciens trials ultralytics.YOLO d'avant la migration YOLOX ; les runs
    recents echouent avec des traces yolox_dataset differentes, deja
    couvertes par diagnose_failure()."""
...
def _workers_from_command(trial) -> int | None:
    command = (trial.user_attrs or {}).get("command") or []
    for i, tok in enumerate(command):
        if tok == "--workers" and i + 1 < len(command):
            try:
...
def _stale_trial_evidence(trial, study_name: str) -> dict:
    ...
    expected = {
        ...
        "best_ckpt.pth": (trial_dir / "best_ckpt.pth").exists(),
    }
    workers = _workers_from_command(trial)
    only_setup = trial_dir.exists() and not any(expected.values())
    facts = (
        ...
        f"poids best_ckpt.pth={'présents' if expected['best_ckpt.pth'] else 'absents'}"
...
def _recover_legacy_artifacts(trial, runs_root: Path | None = None) -> dict:
    """Rapproche un ancien FAIL de son dossier de run sans modifier la DB.

    Cible specifiquement les runs produits AVANT la migration vers le moteur
    YOLOX maison (Training_App) : ceux-la ont ete ecrits par ultralytics.YOLO
    sous runs/detect/<name>/{args.yaml,results.csv,weights/best.pt}. Les runs
    post-migration ecrivent directement dans le trial_dir (cf. hpo_trial.py,
    _stale_trial_evidence) et n'ont jamais besoin de ce rapprochement.
...
def _load_study(study_name: str) -> optuna.Study:
    try:
        ...
        raise HTTPException(404, f"Étude introuvable : {exc}") from exc
...
class StartBody(BaseModel):
    script_path:  str
    # Arguments fixes places avant les hyperparametres de chaque trial
    # (ex. --data_yaml ... --engine ... pour hpo_trial.py).
...
def best_trial(study_name: str):
    ...
    try:
        ...
        raise HTTPException(404, "Aucun meilleur trial disponible") from exc
