...
// Rendu markdown -> HTML des pages de docs/ et liens vers la page
// Documentation (/guide).
//
...
export const DOCS_ROUTE = '/guide'

/** URL de la page Documentation sur une page de doc, et optionnellement un titre. */
export function docLink(name: string, headingIdx?: number): string {
  const hash = headingIdx === undefined ? '' : `#h-${headingIdx}`
  return `${DOCS_ROUTE}?doc=${encodeURIComponent(name)}${hash}`
}
