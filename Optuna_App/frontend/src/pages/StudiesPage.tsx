...
function CreateModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('Étude créée'))
      onCreated()
      onClose()
    } catch {
      toast.error(t("Erreur lors de la création (nom déjà utilisé ?)"))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-md p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-white font-semibold text-base">{t('Nouvelle étude')}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">{t('Direction')}</label>
...
function StudyState({ study }: { study: StudySummary }) {
  const t = useT()
  if (study.status === 'running') return <span className="inline-flex items-center gap-1 rounded border border-blue-800/50 bg-blue-950/30 px-2 py-1 text-xs font-semibold text-blue-300"><LoaderCircle size={12} className="animate-spin"/> {t('En cours')}</span>
  if (study.status === 'finished') return <span className="inline-flex items-center gap-1 rounded border border-emerald-800/50 bg-emerald-950/30 px-2 py-1 text-xs font-semibold text-emerald-300"><CheckCircle2 size={12}/> {t('Terminé')}</span>
  if (study.status === 'error') return <span className="inline-flex items-center gap-1 rounded border border-red-800/50 bg-red-950/30 px-2 py-1 text-xs font-semibold text-red-300"><AlertTriangle size={12}/> {t('Échec HPO')}</span>
  return <span className="rounded border border-gray-700 bg-gray-800 px-2 py-1 text-xs text-gray-400">{t('Vide')}</span>
}
...
export default function StudiesPage() {
  ...
  const handleDelete = async (s: StudySummary, e: React.MouseEvent) => {
    e.stopPropagation()
    if (!window.confirm(`${t("Supprimer l'étude")} "${s.study_name}" ${t('et tous ses trials ?')}`)) return
    setDeleting(s.study_name)
    try {
      await studiesAPI.delete(s.study_name)
      toast.success(t('Étude supprimée'))
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-white">{t('Études')}</h1>
          {studies && (
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-500">
          ...
          <p className="text-sm">{t('Aucune étude Optuna trouvée')}</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium uppercase">{t('Direction')}</th>
                <th className="text-right px-4 py-2.5 text-xs text-gray-500 font-medium uppercase">Trials C/F/P</th>
                <th className="text-right px-4 py-2.5 text-xs text-gray-500 font-medium uppercase">{t('Meilleure val.')}</th>
            ...
            <tbody>
              {studies.map((s, i) => (
                <tr key={s.study_name}
                  ...
                  <td className="px-4 py-3"><p className="font-medium text-gray-200">{s.study_name}</p>{(s.graph_id || s.run_id || s.metric) && <p className="mt-1 max-w-xs truncate font-mono text-[10px] text-gray-500" title={`graph=${s.graph_id || '—'} · run=${s.run_id || '—'} · metric=${s.metric || '—'}`}>{s.metric || t('objectif —')} · graph {s.graph_id || '—'} · run {s.run_id || '—'}</p>}</td>
