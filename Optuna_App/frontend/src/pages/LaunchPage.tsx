...
export default function LaunchPage() {
  ...
  async function handleLaunch() {
    ...
    for (const row of rows) {
      ...
      if (!spec) {
        toast.error(`Paramètre "${row.name || '?'}" invalide — vérifiez les champs`)
        return
      }
    ...
    if (!scriptPath.trim()) {
      toast.error('Chemin du script requis')
      return
    }
    ...
    try {
      ...
      const msg = e?.response?.data?.detail ?? 'Erreur au démarrage'
      toast.error(msg)
      setRunning(false)
      return
    }
    ...
    _cancelSSE = streamLogs(
      ...
      (evt: LogEvent) => {
        if (evt.type === 'log' && evt.line) {
          ...
          setLogs(l => [...l, '— optimisation terminée —'])
          setRunning(false)
          invalidate(decodedName)
        }
      },
  ...
  return (
    <div className="p-6 space-y-5 max-w-3xl">
      ...
      <h1 className="text-xl font-semibold text-white">Lancer une optimisation</h1>

      {/* Script path */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-4">
        <h2 className="text-sm font-semibold text-gray-300">Script d'objectif</h2>
        <div>
          <label className="block text-xs text-gray-500 mb-1">Chemin absolu vers le script Python</label>
          <input
            value={scriptPath}
          ...
          <p className="text-xs text-gray-600 mt-1.5">
            Le script reçoit les hyperparamètres comme arguments (<code className="text-gray-400">--param_name valeur</code>) et doit imprimer la métrique en dernière ligne sur stdout.
          </p>
        </div>

        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className="block text-xs text-gray-500 mb-1">Nombre de trials</label>
            <input
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">Nom de la métrique</label>
            <input
              value={metricName}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-gray-300">Espace des hyperparamètres</h2>
          <button
            onClick={addRow}
