...
#   BUNDLE  : Computer_Vision_App/_lib/launcher_engine.py             #
#             → _CV = Computer_Vision_App (apps = freres de _lib)     #
# ------------------------------------------------------------------ #

_LIB_DIR = Path(__file__).resolve().parent             # .../_lib
_PARENT  = _LIB_DIR.parent                             # App/Launchers/  OU  Computer_Vision_App/

if _PARENT.name == "Computer_Vision_App":
    # BUNDLE : _lib est dans Computer_Vision_App/ (apps = freres de _lib)
    _CV       = _PARENT                                 # Computer_Vision_App/
    _APP_BASE = _PARENT
    _BUNDLE   = True
...
_WS_SUBDIRS: dict = {
    ...
    "inference":    ["runs", "exports", "sequences", "cmd_send", "acquisitions"],
}
