...
# Tests unitaires : api/docs.py (doc markdown de la page d'aide /help).
# Mini app FastAPI avec le seul router docs, sur un dossier docs temporaire :
# ni CLIP ni base de donnees ne sont charges.
# ============================================================
...
def test_parse_frontmatter_absent_or_unclosed() -> None:
    assert docs.parse_frontmatter("# Titre\n") == ({}, "# Titre\n")
    assert docs.parse_frontmatter("---\ntitle: x\n# pas de fin\n")[0] == {}


def test_missing_manifest_falls_back_to_builtin(tmp_path: Path) -> None:
    names = [e["name"] for e in docs.load_doc_set(tmp_path / "absent.json")]
    assert names[0] == "README" and "code-map" in names and len(names) == 9


def test_real_docs_folder_matches_builtin_set() -> None:
    # Le dossier livre doit contenir les 9 pages dans les deux langues.
    for entry in docs.BUILTIN_DOC_SET:
        for lang in ("en", "fr"):
            assert docs.doc_path(docs.DOCS_DIR, entry["name"], lang).is_file()


def test_list_docs_titles_and_langs(docs_client: TestClient) -> None:
...
def test_get_doc_lang_fallback_and_bom(docs_client: TestClient) -> None:
    resp = docs_client.get("/api/docs/concepts", params={"lang": "en"})
    ...
    data = docs_client.get("/api/docs/workflows").json()
    assert data["lang"] == "en"
    assert data["frontmatter"]["tags"] == ["a", "b"]


def test_get_doc_name_validation(docs_client: TestClient) -> None:
    ...
    assert (
        docs_client.get("/api/docs/workflows", params={"lang": "de"}).status_code == 422
    )


def test_assets_safe_join(docs_client: TestClient, tmp_path: Path) -> None:
