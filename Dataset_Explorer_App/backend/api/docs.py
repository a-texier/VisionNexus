# ============================================================
# api/docs.py
# Documentation markdown de l'app (dossier Dataset_Explorer_App/docs/),
# servie a la page d'aide /help du frontend.
#
...
# suite ; l'app pouvant etre deployee seule (zip autonome), on retombe sur
# une liste integree si le manifeste est absent.
# Prefixe : /api/docs
# ============================================================
...
def load_doc_set(manifest_path: Path) -> list[dict]:
    ...
    try:
        ...
        return BUILTIN_DOC_SET
    return [
        e for e in entries if isinstance(e, dict) and isinstance(e.get("name"), str)
    ]
        ...
        pages.append(
            {
                ...
                "langs": [
                    code
                    for code in ("en", "fr")
                    if doc_path(DOCS_DIR, name, code).is_file()
                ],
            }
        )
