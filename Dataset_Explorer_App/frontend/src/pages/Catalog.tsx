...
function DatasetPicker({
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className="text-xs text-gray-500">Restreindre à :</span>
      <button
        onClick={() => onChange([])}
    ...
    >
      <div className="aspect-square bg-gray-950 flex items-center justify-center">
        {thumb
          ...
          : <span className="text-gray-700 text-xs">pas de miniature</span>}
      </div>
      {selected && (
...
function SelectionBar({
  ...
  const create = async () => {
    ...
    if (!base) return toast.error('Nom de subset requis')
    setBusy(true)
    try {
      ...
      toast.success(
        entries.length > 1
          ? `${entries.length} subsets créés (un par dataset)`
          : `Subset "${base}" créé`,
      )
      onClear()
      setName('')
  ...
  return (
    <div className="sticky bottom-0 mt-4 bg-gray-900 border border-indigo-600/40 rounded-lg p-3 flex items-center gap-3 flex-wrap">
      ...
      >
        {busy ? 'Création…' : 'Créer subset'}
      </button>
      <button onClick={onClear} className="text-gray-400 hover:text-white" title="Vider la sélection">
        <X size={16} />
      </button>
    </div>
  )
...
function VisualTab({ datasets }: { datasets: { id: number; name: string }[] }) {
  ...
  const run = async () => {
    if (!query.trim()) return toast.error('Saisir une requête')
    setLoading(true)
    try {
      ...
      setRes(r)
      if (!r.results.length) toast('Aucun résultat', { icon: 'ℹ️' })
    } catch (e: any) {
      toast.error(e?.response?.data?.detail ?? 'Recherche échouée')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 space-y-3">
        <div className="flex gap-2">
          <input
            ...
            placeholder="ex : drone au-dessus de la forêt, véhicule rouge, ciel nuageux…"
      ...
      {res && (
        <div className="mt-4">
          <div className="flex items-center gap-3 flex-wrap text-xs text-gray-400 mb-2">
            <span className="text-gray-300">{res.results.length} résultat(s)</span>
            <span>index global : {res.indexed_vectors.toLocaleString('fr-FR')} vecteurs sur {res.indexed_datasets.length} dataset(s)</span>
...
function MetadataTab({ datasets }: { datasets: { id: number; name: string }[] }) {
  ...
  const run = async (targetPage = 0, q = query) => {
    if (!q.trim()) return toast.error('Saisir un mot-clé')
    setLoading(true)
    try {
      ...
      setRes(r)
      setPage(targetPage)
      if (!r.total) toast('Aucun résultat', { icon: 'ℹ️' })
    } catch (e: any) {
      toast.error(e?.response?.data?.detail ?? 'Recherche échouée')
    } finally {
      setLoading(false)
  ...
  return (
    <div>
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 space-y-3">
        {!hasColumns && (
          <p className="text-xs text-amber-400 flex items-center gap-2">
            ...
            Aucun dataset n'a de métadonnées CSV/Excel associées — associez un fichier
            à l'import pour rendre ces colonnes cherchables ici.
          </p>
        )}
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="mot-clé, valeur ou nom de colonne (ex : zone_forestiere brouillard)"
            className="flex-1 bg-gray-800 border border-gray-700 rounded px-3 py-2 text-sm text-white"
          />
          <select
            value={mode}
            ...
            title="AND : tous les mots. OR : au moins un."
          >
            <option value="and">tous les mots</option>
            <option value="or">au moins un</option>
          </select>
          <button
            onClick={() => run(0)}
            disabled={loading}
        ...
        {hasColumns && (
          <div className="border-t border-gray-800 pt-3">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs text-gray-500">Explorer une colonne :</span>
              {columns!.columns.map(c => (
                <button
                  ...
                  title={`présente dans : ${c.dataset_names.join(', ')}`}
                >
                  {c.column}
            ...
            {Object.keys(columns!.groups).length > 0 && (
              <p className="text-[11px] text-gray-500 mt-2">
                Colonnes rapprochées automatiquement :{' '}
            ...
            {facetColumn && (
              <div className="mt-2 flex items-center gap-2 flex-wrap">
                ...
                {facets && facets.values.length === 0 && (
                  <span className="text-xs text-gray-600">aucune valeur</span>
                )}
              </div>
            )}
          </div>
      ...
      {res && (
        <div className="mt-4">
          <div className="flex items-center gap-3 flex-wrap text-xs text-gray-400 mb-2">
            <span className="text-gray-300">{res.total} résultat(s)</span>
            {totalPages > 1 && (
...
function DuplicatesTab() {
    ...
    try {
      ...
      if (!r.group_count) {
        toast(crossOnly
          ...
          : 'Aucun doublon à ce seuil', { icon: 'ℹ️' })
      }
    } catch (e: any) {
  ...
  const save = async () => {
    ...
    if (!entries.length) return toast.error('Aucune décision à enregistrer')
    try {
      const r = await catalogAPI.patchGlobalDuplicateDecision(
        entries.map(([id, keep]) => ({ image_id: Number(id), keep })),
      )
      toast.success(`${r.updated} décision(s) enregistrée(s)`)
    } catch (e: any) {
      toast.error(e?.response?.data?.detail ?? 'Enregistrement échoué')
    }
  }
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 space-y-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <label className="text-sm text-gray-400 flex items-center gap-2">
            ...
            uniquement les groupes couvrant plusieurs datasets
          </label>
          <button
          ...
          {loading && <span className="text-xs text-gray-500">calcul en cours… {elapsed}s</span>}
        </div>
        <p className="text-[11px] text-gray-500">
          Rien n'est supprimé sur le disque : « rejeter » pose seulement un
          marqueur, exploité ensuite par les cartes et les subsets.
        </p>
      </div>
      ...
      {res && (
        <div className="mt-4 space-y-3">
          <div className="flex items-center gap-3 flex-wrap text-xs text-gray-400">
            <span className="text-gray-300">{res.group_count} groupe(s) affiché(s)</span>
            <span>sur {res.total_group_count} détecté(s)</span>
            <span>{res.indexed_vectors.toLocaleString('fr-FR')} vecteurs indexés</span>
            {Object.keys(decisions).length > 0 && (
              <button
                onClick={save}
                className="px-2 py-0.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded"
              >
                Enregistrer {Object.keys(decisions).length} décision(s)
              </button>
            )}
          </div>
          ...
          {res.groups.map(g => (
            <div key={g.group_id} className="bg-gray-900 border border-gray-800 rounded-lg p-3">
              <div className="flex items-center gap-2 flex-wrap mb-2 text-xs">
                <span className="text-gray-300">Groupe #{g.group_id}</span>
                ...
                {g.truncated && (
                  <span className="px-1.5 py-0.5 bg-amber-600/20 text-amber-300 rounded border border-amber-600/30 flex items-center gap-1">
                    ...
                    affichage tronqué ({g.images.length}/{g.size})
                  </span>
                )}
              </div>
    ...
    <div className="p-6">
      <div className="mb-5">
        ...
        <p className="text-gray-500 text-sm mt-0.5">
          ...
          {searchable.length > 0 && ` ${searchable.length} dataset(s) prêt(s).`}
        </p>
      </div>
