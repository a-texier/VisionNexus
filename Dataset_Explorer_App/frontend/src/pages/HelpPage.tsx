// ============================================================
// pages/HelpPage.tsx
// Aide integree : rend les pages markdown de Dataset_Explorer_App/docs/
// servies par /api/docs. Le contenu n'existe qu'a un seul endroit (les .md) ;
// cette page ne fait que naviguer et afficher.
//
// Etat dans l'URL pour les liens profonds : /help?doc=workflows#h-4
...
export default function HelpPage() {
  ...
  // La page defile dans le <main> de l'app, pas dans la fenetre.
  ...
  const scrollToAnchor = useCallback((target: string) => {
    const node = target ? document.getElementById(target) : topRef.current
  ...
  return (
    <div ref={topRef} className="min-h-full">
      <div className="sticky top-0 z-40 bg-gray-950/95 backdrop-blur border-b border-gray-800 px-6 py-3 flex items-center gap-4">
        <h1 className="flex items-center gap-2 text-base font-semibold text-white">
          <BookOpen size={18} className="text-indigo-400" /> {t('Documentation')}
        </h1>
        ...
        >
          <Play size={13} /> {t('Lancer le tutoriel interactif')}
        </button>
      </div>

      <div className="flex gap-8 px-6 py-6 max-w-7xl mx-auto">
        <aside className="w-60 flex-shrink-0 self-start sticky top-[68px] max-h-[calc(100vh-84px)] overflow-y-auto pr-2 text-sm">
          {pagesFailed && (
        ...
        <section className="flex-1 min-w-0 max-w-4xl">
          {doc.status === 'loading' && (
