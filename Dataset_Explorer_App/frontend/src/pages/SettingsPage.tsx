...
export default function SettingsPage() {
  ...
  useEffect(() => {
    settingsAPI.get()
      ...
      .catch(() => toast.error(t('Impossible de charger les paramètres')))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(t('Paramètres sauvegardés — thème appliqué'))
    } catch {
  ...
  if (loading) return <div className="p-6 text-gray-500">{t('Chargement...')}</div>
  if (!draft) return <div className="p-6 text-red-400">{t('Impossible de charger les paramètres.')}</div>

  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Settings size={22} /> {t('Paramètres')}
        </h1>
        <p className="text-gray-400 mt-1">
          {t('Stockés dans')} <code className="text-gray-300 text-xs bg-gray-800 px-1.5 py-0.5 rounded">
      ...
      <Section title="Workspace" icon={<Database size={16} />}>
        <InfoRow label={t('Chemin workspace')} value={settings?.workspace_path ?? '—'}
          hint={t('Configurable via EXPLORER_WORKSPACE dans launcher.py')} readOnly />
        <InfoRow label={t('Base de données')} value={`${settings?.workspace_path ?? ''}/dataset_explorer.db`} readOnly />
        <InfoRow label="Thumbnails" value={`${settings?.workspace_path ?? ''}/thumbs/`} readOnly />
        <InfoRow label={t('Index FAISS')} value={`${settings?.workspace_path ?? ''}/faiss/`} readOnly />
      </Section>

      {/* Export */}
      <Section title="Export Annotation App" icon={<FolderOpen size={16} />}>
        <div className="space-y-2">
          <label className="block text-sm text-gray-400">{t("Dossier d'imports")}</label>
          <input
            type="text"
          ...
          <p className="text-xs text-gray-500">
            {t('Les subsets exportés créeront un sous-dossier ici.')}
            {' '}{t('Configurable aussi via')} <code className="text-gray-400">ANNOTATION_APP_IMPORTS</code> {t('dans launcher.py.')}
          </p>
        </div>
      </Section>

      {/* Subsets & symlinks */}
      <Section title={t('Subsets & liens')} icon={<FolderOpen size={16} />}>
        <div className="flex items-start gap-4">
          <div className="flex-1">
            <label className="block text-sm text-gray-300 font-medium mb-1">{t('Stratégie de liens')}</label>
            <p className="text-xs text-gray-500 mb-3">
              {t('Définit comment les fichiers sont référencés dans les subsets et exports.')}
              <strong className="text-gray-400"> Symlinks</strong> = {t('lien symbolique (aucune copie, rapide, recommandé).')}
              <strong className="text-gray-400"> {t('Copie')}</strong> = {t("copie physique (lent, consomme de l'espace, mais universel).")}
            </p>
            ...
            {draft.use_symlinks && (
              <p className="text-xs text-yellow-500/80 mt-2">
                {t('Windows : nécessite le')} <strong>{t('Mode Développeur')}</strong> {t('(Paramètres → Pour les développeurs → Mode développeur : ON) ou droits admin.')}
              </p>
            )}
          </div>
        </div>
      ...
      <Section title={t('Réduction dimensionnelle')} icon={<Network size={16} />}>
        <p className="text-xs text-gray-500">
          {t('Méthode utilisée pour projeter les embeddings CLIP (512D) en 2D sur la Carte.')}
          {' '}{t("Appliquée à chaque pipeline d'embedding. Les hyperparamètres sont sauvegardés et utilisés pour toutes les nouvelles exécutions.")}
        </p>
        <div className="space-y-1">
          <label className="block text-sm text-gray-300 font-medium">{t('Méthode')}</label>
          ...
          <p className="text-xs text-gray-600 mt-1">
            {draft.reduction_method === 'umap' && t('UMAP : recommandé, meilleure préservation de la structure locale et globale.')}
            {draft.reduction_method === 'tsne' && t('t-SNE : clusters bien séparés, mais distances inter-clusters peu fiables.')}
            {draft.reduction_method === 'pca' && t('PCA : déterministe et rapide, moins expressif sur les grands datasets.')}
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min={2} max={200}
                ...
                className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-indigo-500"
              />
              <p className="text-xs text-gray-600 mt-1">{t('Voisins locaux considérés (défaut : 15)')}</p>
            </div>
            <div>
              ...
              <input type="number" min={0.001} max={1} step={0.01}
                ...
                className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-indigo-500"
              />
              <p className="text-xs text-gray-600 mt-1">{t('Distance minimale entre points (défaut : 0.1)')}</p>
            </div>
          </div>
        )}
        ...
        {draft.reduction_method === 'tsne' && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1">{t('Perplexité')}</label>
              <input type="number" min={5} max={100}
                value={draft.tsne_perplexity}
                onChange={e => setDraft({ ...draft, tsne_perplexity: Number(e.target.value) })}
                className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-indigo-500"
              />
              <p className="text-xs text-gray-600 mt-1">{t('Balance local/global (défaut : 30)')}</p>
            </div>
            <div>
              ...
              <input type="number" min={10} max={1000} step={10}
                ...
                className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-indigo-500"
              />
              <p className="text-xs text-gray-600 mt-1">{t("Vitesse d'apprentissage (défaut : 200)")}</p>
            </div>
          </div>
        )}
        ...
        {draft.reduction_method === 'pca' && (
          <p className="text-xs text-gray-500 italic">
            {t('PCA : pas de paramètre spécifique. Toujours 2 composantes principales.')}
          </p>
        )}
      </Section>
      ...
      <Section title="Clustering" icon={<RefreshCw size={16} />}>
        <div className="space-y-1">
          <label className="block text-sm text-gray-300 font-medium">{t('Méthode par défaut')}</label>
          <div className="flex gap-2">
          ...
          <p className="text-xs text-gray-600 mt-1">
            {draft.cluster_method === 'kmeans' && t('KMeans : nombre de clusters fixé (n_clusters), rapide et déterministe.')}
            {draft.cluster_method === 'hdbscan' && t('HDBSCAN : détecte automatiquement le nombre de clusters + le bruit (min_cluster_size).')}
          </p>
        </div>
        {draft.cluster_method === 'hdbscan' && (
          <div className="max-w-xs">
            ...
            />
            <p className="text-xs text-gray-600 mt-1">{t("Taille minimale d'un cluster (défaut : 5)")}</p>
          </div>
        )}
      </Section>

      {/* Pipeline defaults */}
      <Section title={t('Valeurs par défaut')} icon={<RefreshCw size={16} />}>
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" min={2} max={100}
              ...
              className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-indigo-500"
            />
            <p className="text-xs text-gray-500 mt-1">{t('Par défaut à la création de dataset')}</p>
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">{t('Top-K recherche')}</label>
            <input type="number" min={5} max={200}
              value={draft.default_top_k}
              onChange={e => setDraft({ ...draft, default_top_k: Number(e.target.value) })}
              className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-indigo-500"
            />
            <p className="text-xs text-gray-500 mt-1">{t('Résultats par défaut en recherche sémantique')}</p>
          </div>
        </div>
        <div>
          <label className="block text-sm text-gray-400 mb-1">{t('Mode couleur carte UMAP (défaut)')}</label>
          <select value={draft.scatter_default_color}
            ...
            <option value="cluster">{t('Par cluster')}</option>
            <option value="rarity">{t('Par rareté')}</option>
            <option value="uniform">{t('Uniforme')}</option>
          </select>
        </div>
      </Section>

      {/* ---- Thème visuel ---- */}
      <Section title={t('Thème visuel')} icon={<Palette size={16} />}>
        <p className="text-xs text-gray-500">
          {t("Choisissez un fond et une couleur d'accent pour personnaliser l'atmosphère de l'app.")}
          {' '}{t('Cliquez sur un thème pour le prévisualiser instantanément — sauvegardez pour le conserver.')}
        </p>

        {/* Fond */}
        <div>
          <label className="block text-sm text-gray-400 mb-2">{t("Fond de l'application")}</label>
        ...
        {/* Accent */}
        <div>
          <label className="block text-sm text-gray-400 mb-2">{t("Couleur d'accent")}</label>
        ...
        <div className="bg-gray-900 rounded-lg p-3 border border-gray-700">
          <p className="text-xs text-gray-500 mb-2">{t('Aperçu des éléments interactifs :')}</p>
          <div className="flex flex-wrap gap-2">
      ...
      {/* Guide technique */}
      <Section title={t('Guide technique')} icon={<Info size={16} />}>
        <div className="space-y-3 text-sm text-gray-400">
          <TechCard title="Invariant FAISS">
            {t('Position')} <code className="text-indigo-300">i</code> {t("dans l'index = Image rang")} <code className="text-indigo-300">i</code> {t('triée par')} <code className="text-indigo-300">Image.id</code> {t("ascendant. Ne jamais ajouter d'images sans reconstruire l'index.")}
          </TechCard>
          <TechCard title="Normalisation L2">
            {t('Tous les embeddings CLIP sont L2-normalisés. Similarité cosinus = produit scalaire (dot product) → FAISS IndexFlatIP.')}
          </TechCard>
          <TechCard title="Symlinks Windows">
            {t('Nécessitent le Mode Développeur ou droits admin. Configurable ci-dessus : activez "Copie physique" pour éviter cette restriction.')}
          </TechCard>
      ...
      <div className="flex justify-end pt-2">
        <button onClick={handleSave} disabled={saving || !hasChanges}
          ...
          <Save size={16} />
          {saving ? t('Sauvegarde...') : hasChanges ? t('Sauvegarder les modifications') : t('Aucune modification')}
        </button>
      </div>
    </div>
  )
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        ...
        {readOnly && <span className="text-xs text-gray-600 whitespace-nowrap">{t('lecture seule')}</span>}
      </div>
