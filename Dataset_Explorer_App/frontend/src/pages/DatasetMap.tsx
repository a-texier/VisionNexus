...
export default function DatasetMap() {
  ...
  useEffect(() => {
    if (wasReclustering.current && !isReclustering) {
      ...
      toast.success('Clustering mis à jour')
    }
  ...
  useEffect(() => {
    if (wasReducing.current && !isReducing) {
      qc.invalidateQueries({ queryKey: ['dataset', datasetId] })
      qc.invalidateQueries({ queryKey: ['dataset-map', datasetId] })
      toast.success('Carte 2D mise à jour')
    }
  ...
  const handleReclusterMap = async () => {
    ...
    try {
      ...
      if (res.status === 'already_running') toast('Clustering déjà en cours')
      else toast.success('Clustering relancé')
  ...
  const handleReduceMap = async () => {
    try {
      ...
      else toast.success('Réduction relancée')
      qc.invalidateQueries({ queryKey: ['datasets'] })
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? String(e)
  ...
  const handleSelectAllCluster = () => {
    ...
    toast.success(`${filteredPoints.length} images sélectionnées`)
  }
  ...
  const handleExcludeImages = async () => {
    ...
    if (!confirm(`Exclure ${selectedIds.size} image(s) du dataset ? Elles seront traitées comme des rejets (is_duplicate_kept=False).`)) return
    setExcluding(true)
    try {
      const res = await datasetsAPI.excludeImages(datasetId, Array.from(selectedIds))
      toast.success(`${res.excluded} image(s) exclues du dataset`)
      clear()
      qc.invalidateQueries({ queryKey: ['dataset-map', datasetId] })
      qc.invalidateQueries({ queryKey: ['datasets'] })
    } catch {
      toast.error('Erreur lors de l\'exclusion des images')
    } finally {
      setExcluding(false)
    }
  }
  ...
  const handleCreateSubset = async () => {
    if (!subsetName.trim()) return toast.error('Nom du subset requis')
    if (selectedIds.size === 0) return toast.error('Sélectionnez des images d\'abord')
    setSaving(true)
    try {
      ...
      toast.success(`Subset "${subsetName}" créé — ${selectedIds.size} images`)
      setSubsetName('')
      clear()
    } catch {
      toast.error('Erreur lors de la création du subset')
    } finally {
      setSaving(false)
    }
  }
    ...
    return (
      <div className="p-6 flex items-center justify-center h-64 text-gray-500">
        <div className="text-center">
          ...
          <p>Carte non calculée pour ce dataset.</p>
          <p className="text-sm mt-1">Lancez les embeddings depuis le Dashboard.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex flex-col h-screen p-4 gap-3">
      ...
      <div className="flex-shrink-0 flex items-center justify-between">
        <h1 className="text-xl font-bold text-white flex items-center gap-2">
          <Map size={20} /> {dataset?.name} — Carte {(summary?.reduction_method ?? dataset?.reduction_method ?? settings?.reduction_method ?? 'umap').toUpperCase()}
        </h1>
      ...
      <div className="flex-shrink-0 space-y-2">
        <div className="flex items-center gap-2 text-xs text-gray-500 bg-gray-900/60 px-3 py-1.5 rounded-lg border border-gray-800">
          ...
          <span>Utilisez le <strong className="text-gray-400">lasso Plotly</strong> pour sélectionner des points, puis nommez et créez le subset ci-dessous.</span>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              <RefreshCw size={12} /> Réduction {showReduce ? <ChevronUp size={10} /> : <ChevronDown size={10} />}
            </button>
          </div>
        </div>
        ...
        {isReclustering && (
          <div>
            <div className="flex justify-between text-xs text-gray-400 mb-1">
              <span>Clustering : {summary?.recluster_phase || 'clustering'}</span>
        ...
        {isReducing && (
          <div>
            <div className="flex justify-between text-xs text-gray-400 mb-1">
              <span>Réduction : {summary?.reduce_phase || 'umap'}</span>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-white text-sm font-medium">
                ...
                {clusterInfo.avg_rarity != null && (
                  <span className="text-gray-500 ml-2 text-xs">rareté moy. {Math.round(clusterInfo.avg_rarity * 100)}%</span>
                )}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={handleSelectAllCluster}
                ...
                <Layers size={12} /> Tout sélectionner ({filteredPoints.length})
              </button>
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-3">
        <div className="flex-1 min-h-[300px] bg-gray-900 rounded-xl border border-gray-700 overflow-hidden">
          {mapLoading ? (
            <div className="flex items-center justify-center h-full text-gray-500">Chargement de la carte...</div>
          ) : (
            <ScatterPlot
        ...
        {selectedIds.size > 0 && (
          <div className="flex-shrink-0 bg-gray-800 rounded-xl border border-amber-600/30 p-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-amber-300 font-medium text-sm">{selectedIds.size} image(s) sélectionnée(s)</span>
              <div className="flex items-center gap-2">
                <input type="text" placeholder="Nom du subset..." value={subsetName}
                ...
                <button
                  ...
                  title="Exclure ces images du dataset (traitées comme des rejets)"
                >
                  <Ban size={14} /> Exclure du dataset
                </button>
                <button onClick={clear} className="p-1.5 text-gray-500 hover:text-white" title="Effacer sélection">
                  <X size={16} />
                </button>
              </div>
            </div>
...
function SelCard({ point, onZoom, onDeselect }: {
  ...
  return (
    <div className="flex-shrink-0 w-24 rounded-lg overflow-hidden border-2 border-amber-500 group">
      ...
      <div className="px-1 py-0.5 bg-gray-900 cursor-pointer" onClick={onDeselect} title="Désélectionner">
