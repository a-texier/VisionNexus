...
export default function SemanticSearch() {
  ...
  const handleSearch = async () => {
    if (!query.trim()) return toast.error(t('Entrez une requête'))
    setLoading(true)
    clear()
    try {
      ...
      const msg = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? t('Erreur serveur')
      toast.error(msg)
    } finally {
      setLoading(false)
    }
  ...
  const handleSaveSubset = async () => {
    if (!subsetName.trim()) return toast.error(t('Nom requis'))
    const ids = selectedIds.size > 0
      ? Array.from(selectedIds)
      : results.map(r => r.image_id)
    if (ids.length === 0) return toast.error(t('Aucune image'))
    setSaving(true)
    try {
      await subsetsAPI.create({ dataset_id: datasetId, name: subsetName, image_ids: ids })
      toast.success(`Subset "${subsetName}" ${t('créé')} (${ids.length} images)`)
      setSubsetName('')
      clear()
    } catch {
      toast.error(t('Erreur création subset'))
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Search size={22} /> {t('Recherche sémantique')}
        </h1>
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
        <span className="font-medium text-gray-400">{t('Légende :')}</span>
        <span className="px-2 py-0.5 rounded-full bg-green-600/15 text-green-400 border border-green-600/30">Match 87%</span>
        <span>= {t('score de similarité CLIP (plus élevé = meilleure correspondance)')}</span>
        <span className="px-2 py-0.5 rounded-full bg-indigo-600/15 text-indigo-400 border border-indigo-600/30">C3</span>
        <span>= {t('cluster UMAP')}</span>
        <span className="px-2 py-0.5 rounded-full bg-amber-600/15 text-amber-400 border border-amber-600/30">{t('Rareté')} 72%</span>
        <span>= {t('rareté dans le dataset (score élevé = image atypique)')}</span>
      </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {!useThreshold ? (
            <>
              ...
              />
              <span className="text-gray-600 text-xs">{t('résultats à retourner')}</span>
            </>
          ) : (
            <>
              <input
              ...
              <span className="text-gray-600 text-xs">{t('toutes les images ≥ ce score de match')}</span>
            </>
          )}
        </div>
      </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-3">
              <p className="text-gray-400 text-sm">
                <span className="text-white font-medium">{results.length}</span> {t('résultats pour')}{' '}
                <span className="text-indigo-300 italic">"{query}"</span>
                {selectedIds.size > 0 && (
                  <span className="ml-2 text-amber-400 font-medium">· {selectedIds.size} {t('sélectionnée(s)')}</span>
                )}
              </p>
              <button
                onClick={handleToggleAll}
                className="flex items-center gap-1 text-xs text-gray-500 hover:text-white transition-colors"
                title={t('Tout sélectionner / désélectionner')}
              >
                {selectedIds.size === results.length
                  ? <><CheckSquare size={13} /> {t('Tout désélectionner')}</>
                  : <><Square size={13} /> {t('Tout sélectionner')}</>
                }
              </button>
            </div>
            <div className="flex items-center gap-2">
              ...
              >
                <Save size={14} />
                {selectedIds.size > 0 ? `${t('Sauver sélection')} (${selectedIds.size})` : `${t('Tout sauver')} (${results.length})`}
              </button>
            </div>
          </div>
      ...
      {!loading && results.length === 0 && query && (
        <p className="text-gray-500 text-center py-12">{t('Aucun résultat pour cette recherche.')}</p>
      )}
      ...
      <div className="p-1.5 bg-gray-800" onClick={onToggle}>
        ...
        <div className="flex flex-wrap gap-1">
          ...
          <span
            ...
            title={`${t('Score de similarité CLIP :')} ${matchPct}%`}
          >
            {matchPct}%
          </span>
          ...
          {result.cluster_id !== null && (
            <span
              ...
              title={`${t('Cluster UMAP :')} ${result.cluster_id}`}
            >
              C{result.cluster_id}
            </span>
          )}
          ...
          {rarityPct !== null && (
            <span
              ...
              title={`${t('Rareté dans le dataset :')} ${rarityPct}%`}
            >
              R{rarityPct}%
            </span>
          )}
