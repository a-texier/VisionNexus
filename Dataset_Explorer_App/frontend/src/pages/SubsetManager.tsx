...
export default function SubsetManager() {
  ...
  const toggleLock = async (subset: SubsetSummary) => {
    try {
      ...
      toast.success(subset.locked ? 'Subset déverrouillé' : 'Subset verrouillé')
    } catch (e: any) {
      toast.error(e?.response?.data?.detail ?? 'Changement de verrou échoué')
    }
  }

  const handleCreate = async () => {
    if (!newName.trim()) return toast.error('Nom requis')
    if (!newDatasetId) return toast.error('Sélectionnez un dataset')
    if (selectedIds.size === 0) return toast.error('Aucune image sélectionnée')
    setCreating(true)
    try {
      ...
      toast.success(`Subset "${newName}" créé`)
      setNewName('')
      clear()
      qc.invalidateQueries({ queryKey: ['subsets'] })
    } catch {
  ...
  const doExport = async (subset: SubsetSummary, customPath: string | undefined) => {
    ...
    try {
      ...
      toast.success(`Exporté vers : ${(res as { export_path: string }).export_path}`)
      qc.invalidateQueries({ queryKey: ['subsets'] })
      setExportSubset(null)
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? 'Erreur export'
      toast.error(msg)
    } finally {
      setExporting(false)
    }
  ...
  const handleDuplicate = async (subset: SubsetSummary) => {
    ...
    const name = window.prompt('Nom du subset dupliqué :', defaultName)
    if (name === null) return  // annulé
    try {
      await subsetsAPI.duplicate(subset.id, name.trim() || defaultName)
      toast.success(`Subset "${name.trim() || defaultName}" créé`)
      qc.invalidateQueries({ queryKey: ['subsets'] })
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? 'Erreur duplication'
      toast.error(msg)
    }
  }
  ...
  const handleDelete = (subset: SubsetSummary) => {
    if (subset.locked) {
      return toast.error('Subset verrouillé — déverrouillez d\'abord')
    }
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Layers size={22} /> Gestion des subsets
        </h1>
        <p className="text-gray-400 mt-1">Collections d'images avec symlinks vers le dataset source</p>
      </div>
      ...
      {selectedIds.size > 0 && (
        <div className="bg-indigo-900/20 border border-indigo-600/40 rounded-xl p-4">
          <p className="text-indigo-300 text-sm mb-3 flex items-center gap-1">
            <Plus size={14} />
            {selectedIds.size} image(s) sélectionnée(s) — créer un subset :
          </p>
      ...
      <div className="flex items-center gap-3">
        <span className="text-gray-400 text-sm">Filtrer par dataset :</span>
        <select
          value={filterDataset ?? ''}
      ...
      {!isLoading && subsets.length === 0 && (
        <div className="text-center py-16 text-gray-500">
          ...
          <p>Aucun subset. Sélectionnez des images sur la carte ou en recherche sémantique.</p>
        </div>
      )}
                      ...
                      {subset.exports.map(exp => (
                        <p key={exp.id} className="text-green-700 text-xs font-mono truncate max-w-lg flex items-center gap-1.5">
                          ...
                          <span className={`flex-shrink-0 px-1.5 py-0 rounded text-xs border ${
                            ...
                            {exp.export_type === 'symlink' ? 'symlink' : 'copie'}
                          </span>
                        </p>
                      ))}
                    </div>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {ds?.umap_cached && (
                    <Link
                      ...
                      title="Voir la carte UMAP du dataset source"
                    >
                      <Map size={13} /> Carte
                    </Link>
                  )}
                  ...
                  {!subset.exported_to_annotation_app ? (
                    <button
                      ...
                      title="Détecter et gérer les doublons dans ce subset"
                    >
                      <GitMerge size={13} /> Doublons
                    </button>
                  ) : (
                    <button
                      disabled
                      ...
                      title="Doublons désactivés après export"
                    >
                      <GitMerge size={13} /> Doublons
                    </button>
                  )}
                  ...
                  <button
                    ...
                    title="Exporter vers Annotation App (plusieurs exports possibles)"
                  >
                  ...
                  <button
                    ...
                    title={locked ? 'Déverrouiller' : 'Verrouiller (empêche suppression accidentelle)'}
                  >
                  ...
                  <button
                    ...
                    title={locked ? 'Subset verrouillé' : 'Supprimer le subset et son dossier'}
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
          ...
          <div className="relative bg-gray-800 border border-gray-700 rounded-xl p-5 w-[440px] shadow-2xl">
            ...
            <div className="mb-4">
              <label className="text-xs text-amber-300 mb-1.5 flex items-center gap-1">
                <FolderOpen size={11} /> Dossier de destination
              </label>
              <input
                type="text"
                value={exportPath}
                onChange={e => setExportPath(e.target.value)}
                placeholder={defaultAnnotationDir || 'Chemin du dossier...'}
              ...
              <p className="text-xs text-gray-500 mt-1">
                Le sous-dossier <span className="font-mono text-gray-400">{exportSubset.name}</span> sera créé à l'intérieur.
      ...
      <ConfirmDialog
        open={!!pendingDelete}
        title={`Supprimer le subset "${pendingDelete?.name ?? ''}" ?`}
        message="Cette action retire le subset de la base et supprime son dossier de liens."
        details={[
          `${pendingDelete?.image_count ?? 0} image(s) référencée(s)`,
          'Les images originales ne sont jamais supprimées du disque.',
          ...(pendingDelete?.exported_to_annotation_app
            ? ['Les exports déjà réalisés vers Annotation App ne sont pas retirés.']
            : []),
        ]}
