...
export function UserBadge() {
  ...
  const openFolder = async () => {
    ...
    try {
      ...
      if (!data.path) throw new Error(t('Le backend n’a retourné aucun chemin Workspace.'))
      await openInNativeFileManager(data.path)
  ...
  const openHistoryPath = async (path: string) => {
    setOpeningPath(path)
    try {
      await openInNativeFileManager(path)
    } catch (error) { console.error(error) }
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          onClick={async (e) => {
                            e.stopPropagation()
                            await openInNativeFileManager(u.workspace)
                          }}
