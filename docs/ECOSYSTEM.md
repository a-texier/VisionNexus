...
`Inference_App` est un noeud FREE (pas d'arete entrante dans le graphe), au
meme titre que MLflow qui est un noeud superviseur isole (voir section 4.6).
...
### 4.8 Inference_App
FastAPI + React encapsulant un tracker MOT/SOT vendored (`tracker/`). Trois
onglets : Tracker (inference live/replay), Evaluation (YOLO + eval MOT/SOT ->
MLflow), Acquisition (capture de flux -> dataset). Noeud FREE dans
l'orchestrateur (pas d'arete entrante). Voir
[Inference_App/docs/README.md](../Inference_App/docs/README.md).
...
WORKSPACE/
  ...
  inference_bob/        runs/  exports/  sequences/  acquisitions/
```
...
| Tracking video | ByteTrack, BoT-SORT, BoostTrack, DiMP, OSTrack (Inference_App) |
