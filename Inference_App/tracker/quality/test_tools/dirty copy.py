...
import sys
import os                                # F401 : jamais utilisé
import json                              # F401 : jamais utilisé
from typing import List, Optional        # UP035 + UP006 : List→list  UP045 : Optional→X|None
import pickle
import subprocess
import socket
...
def process_items(items: List[str]) -> Optional[int]:   # UP006, UP045
    unused_local = "jamais lue"                          # F841

    config = dict(host="localhost", port=8080)           # C408 → {"host": ...}

    for idx, item in enumerate(items):                   # B007 : idx jamais utilisé
        sys.stdout.write(item + "\n")

    if config == None:                                   # E711 → `is None`
        return None
