...
def _conv_transpose2d(
    raw, input, weight, bias=None, stride=1, padding=0, output_padding=0, groups=1, dilation=1
):
...
def _avg_pool2d(
    raw, input, kernel_size, stride=None, padding=0, ceil_mode=False, count_include_pad=True
):
...
def _batch_norm(
    ...
    if running_mean is None or running_var is None:
        ...
        layer1.add_data(
            running_mean_clone.cpu().numpy(), running_var_clone.cpu().numpy(), np.array([1.0])
        )
