...
from tracker.assoc import MhDist_similarity, associate, iou_batch, shape_similarity, soft_biou_batch
...
class BoostTrack(object):
    ...
    def update(self, dets, img_tensor, img_numpy, tag):
        ...
        # Rescale
        scale = min(
            img_tensor.shape[2] / img_numpy.shape[0], img_tensor.shape[3] / img_numpy.shape[1]
        )
        dets = deepcopy(dets)
        dets[:, :4] /= scale
        ...
        if mahalanobis_distance.size > 0 and self.frame_count > 1:
            ...
            if len(boost_detections) > 0:
                ...
                for i in range(len(args)):
                    ...
                    args_tmp = np.append(
                        np.intersect1d(boost_detections_args[args], boost_detections_args[tmp]),
