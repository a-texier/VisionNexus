...
freedom (contains values for N=1, ..., 9). Taken from a chi-square inverse table.
...
class KalmanFilter(object):
    ...
    def update(self, z: np.ndarray, confidence=0.0):
        ...
        kalman_gain = scipy.linalg.cho_solve(
            (chol_factor, lower), np.dot(self.covariance, self._update_mat.T).T, check_finite=False
        ).T
