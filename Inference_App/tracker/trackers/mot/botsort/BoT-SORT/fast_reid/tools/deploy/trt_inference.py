...
def get_parser():
    ...
    parser.add_argument(
        "--model-path", default="outputs/trt_model/baseline.engine", help="trt model path"
    )
    parser.add_argument(
        "--input",
        nargs="+",
    ...
    parser.add_argument(
        "--output", default="trt_output", help="path to save trt model inference results"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for img_path in tqdm.tqdm(args.input):
            ...
            np.save(
                os.path.join(args.output, os.path.basename(img_path).split(".")[0] + ".npy"), feat
            )
