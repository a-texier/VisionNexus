...
def make_parser():
    ...
    parser.add_argument(
        "--match_thresh", type=float, default=0.9, help="matching threshold for tracking"
    )
    parser.add_argument("--min-box-area", type=float, default=100, help="filter out tiny boxes")
    # deepsort args
    parser.add_argument(
        "--model_folder", type=str, default="pretrained/ckpt.t7", help="reid model folder"
    )
    return parser
...
def main(exp, args, num_gpu):
    ...
    *_, summary = evaluator.evaluate_deepsort(
    ...
    gt = OrderedDict(
        [(Path(f).parts[-3], mm.io.loadtxt(f, fmt="mot15-2D", min_confidence=1)) for f in gtfiles]
    )
    ts = OrderedDict(
        [
            (
