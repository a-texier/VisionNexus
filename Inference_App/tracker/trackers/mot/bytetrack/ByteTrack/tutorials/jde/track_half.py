    ...
    for i, (path, img, img0) in enumerate(dataloader):
        ...
        if show_image or save_dir is not None:
            online_im = vis.plot_tracking(
                img0, online_tlwhs, online_ids, frame_id=frame_id, fps=1.0 / timer.average_time
            )
        if show_image:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nms-thres", type=float, default=0.4, help="iou threshold for non-maximum suppression"
    )
