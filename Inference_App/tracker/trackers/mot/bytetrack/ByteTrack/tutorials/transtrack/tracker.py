...
class Tracker(object):
    ...
    def step(self, output_results):
        ...
        if N > 0 and M > 0:
            ...
            unmatched_dets = [d for d in range(N) if d not in matched_indices[0]]
            unmatched_tracks = [d for d in range(M) if d not in matched_indices[1]]
        ...
        if N_second > 0 and M_second > 0:
            ...
            unmatched_tracks_second = [
                d for d in range(M_second) if d not in matched_indices_second[1]
            ]
        ...
        self.tracks = ret
        self.tracks_dict = {
            red_ind: red for red_ind, red in results_dict.items() if "tracking_id" in red
        }
