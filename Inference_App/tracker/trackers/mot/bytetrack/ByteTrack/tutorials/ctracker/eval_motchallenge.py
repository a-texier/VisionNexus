...
if __name__ == "__main__":
    ...
    gtfiles = glob.glob(os.path.join(args.groundtruths, "*/gt/gt_{}.txt".format(gt_type)))
    ...
    gt = OrderedDict(
        [(Path(f).parts[-3], mm.io.loadtxt(f, fmt=args.fmt, min_confidence=1)) for f in gtfiles]
    )
    ts = OrderedDict(
        [
            (
