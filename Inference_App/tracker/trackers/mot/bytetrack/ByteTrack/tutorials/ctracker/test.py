...
def run_each_dataset(model_dir, retinanet, dataset_path, subset, cur_dataset):
    ...
    videoWriter = cv2.VideoWriter(
        out_video, cv2.VideoWriter_fourcc("m", "p", "4", "v"), fps, (img_width, img_height)
    )
    ...
    for i in range((int(img_len / 2)), img_len):
        ...
        for j in range(len(det_list_all[i])):
            ...
            draw_caption(
                img, (x1, y1, x2, y2), draw_trace_id, color=color_list[trace_id % len(color_list)]
            )
            cv2.rectangle(
                img, (x1, y1), (x2, y2), color=color_list[trace_id % len(color_list)], thickness=2
            )
...
def main(args=None):
    ...
    parser.add_argument(
        "--model_path", default="./trained_model/model_final.pth", help="Path to model (.pt) file."
    )
