...
class BYTETracker(object):
    ...
    def merge_outputs(self, detections):
        results = {}
        for j in range(1, self.opt.num_classes + 1):
            results[j] = np.concatenate([detection[j] for detection in detections], axis=0).astype(
                np.float32
            )
    ...
    def update(self, im_blob, img0):
        ...
        if len(dets) > 0:
            """Detections"""
            detections = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4]) for tlbrs in dets[:, :5]]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            """Detections"""
            detections_second = [
                STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4]) for tlbrs in dets_second[:, :5]
            ]
        else:
            detections_second = []
        r_tracked_stracks = [
