...
class Tracker(object):
    ...
    def step(self, results_with_low, public_det=None):
        ...
        unmatched_dets = [d for d in range(dets.shape[0]) if d not in matched_indices[:, 0]]
        unmatched_tracks = [d for d in range(tracks.shape[0]) if d not in matched_indices[:, 1]]
        ...
        if self.opt.public_det and len(unmatched_dets) > 0:
            ...
            matched_dets = [d for d in range(dets.shape[0]) if d not in unmatched_dets]
        ...
        if N > 0 and M > 0:
            ...
            dist = ((tracks_second.reshape(1, -1, 2) - dets.reshape(-1, 1, 2)) ** 2).sum(
                axis=2
            )  # N x M
            ...
            unmatched_tracks_second = [
                d for d in range(tracks_second.shape[0]) if d not in matched_indices_second[:, 1]
            ]
            ...
            for m in matches_second:
                ...
                if "embedding" in track:
                    self.alive.append(track["tracking_id"])
                    self.embedding_bank[self_tracks_second[m[1]]["tracking_id"] - 1, :] = (
                        self.alpha * track["embedding"]
                        + (1 - self.alpha)
                        * self.embedding_bank[self_tracks_second[m[1]]["tracking_id"] - 1, :]
                    )
