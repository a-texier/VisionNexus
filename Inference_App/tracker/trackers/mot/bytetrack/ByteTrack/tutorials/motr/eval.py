...
def draw_points(img: np.ndarray, points: np.ndarray, color=(255, 255, 255)) -> np.ndarray:
    assert len(points.shape) == 2 and points.shape[1] == 2, "invalid points shape: {}".format(
        points.shape
    )
    ...
    def update(self, dt_instances: Instances):
        ...
        if len(ret) > 0:
            ...
            for t in online_targets:
                online_ret.append(
                    np.array(
                        [t.tlbr[0], t.tlbr[1], t.tlbr[2], t.tlbr[3], t.score, t.track_id]
                    ).reshape(1, -1)
                )
        ...
        for i in tqdm(range((int(self.img_len / 2)), self.img_len)):
            ...
            if vis:
                ...
                self.visualize_img_with_bbox(
                    cur_vis_img_path, ori_img, dt_instances, ref_pts=all_ref_pts, gt_boxes=gt_boxes
                )
