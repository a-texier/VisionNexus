...
from .deformable_detr import MLP, SetCriterion
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                unmatched_outputs_layer = {
                    "pred_logits": aux_outputs["pred_logits"][0, unmatched_track_idxes].unsqueeze(
                        0
                    ),
                ...
                for loss in self.losses:
                    ...
                    self.losses_dict.update(
                        {
                            "frame_{}_aux{}_{}".format(self._current_frame_idx, i, key): value
...
class MOTR(nn.Module):
    ...
    def _generate_empty_tracks(self):
        ...
        track_instances.mem_bank = torch.zeros(
            (len(track_instances), mem_bank_len, dim // 2), dtype=torch.float32, device=device
        )
