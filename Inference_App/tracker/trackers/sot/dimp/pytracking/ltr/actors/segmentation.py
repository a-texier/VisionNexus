...
class LWLActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        acc_l = [
            ...
            for rm, lb in zip(
                segm_pred.view(-1, *segm_pred.shape[-2:]), gt_segm.view(-1, *segm_pred.shape[-2:])
            )
        ]
        acc += sum(acc_l)
        cnt += len(acc_l)
...
class LWLBoxActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        train_feat_clf = self.net.extract_target_model_features(
            train_feat
        ...
        train_box_enc = train_box_enc.view(
            num_train_frames, num_sequences, *train_box_enc.shape[-3:]
        )

        mask_pred_box_train, decoder_feat_train = self.net.decoder(
            train_box_enc, train_feat, train_imgs.shape[-2:]
        )
