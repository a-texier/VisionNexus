...
class TaMOsActor(BaseActor):
    ...
    def __init__(
        self, net, objective, loss_weight=None, prob=False, plot_save=None, fg_cls_loss=False
    ):
        super().__init__(net, objective)
        if loss_weight is None:
            loss_weight = {"bb_ce": 1.0}
    ...
    def compute_iou_pred(self, target_scores, bbox_preds, test_ltrb_target, test_sample_region):
        ...
        return self.compute_iou_at_max_score_pos(
            target_scores[:, fg_mask], test_ltrb_target[:, fg_mask], bbox_preds[:, fg_mask]
        )
    ...
    def __call__(self, data):
        ...
        if "trafo" in bbox_preds:
            loss_giou_trafo, ious_trafo = self.compute_bbreg_loss(
                data["test_ltrb_target"], data["test_sample_region"], bbox_preds["trafo"]
            )
        ...
        if "lowres" in bbox_preds:
            loss_giou_lowres, ious_lowres = self.compute_bbreg_loss(
                data["test_ltrb_target"], data["test_sample_region"], bbox_preds["lowres"]
            )
        ...
        if "highres" not in target_scores and "highres" in bbox_preds:
            if "trafo" in target_scores:
                ...
                target_scores_trafo_interp = F.interpolate(
                    target_scores_trafo, data["test_ltrb_target_highres"].shape[-2:], mode="bicubic"
                )

                ious_pred_center_highres = self.compute_iou_pred(
                    target_scores_trafo_interp.reshape(data["test_sample_region_highres"].shape),
                    bbox_preds["highres"],
                    data["test_ltrb_target_highres"],
                    data["test_sample_region_highres"],
                )
            ...
            if "lowres" in target_scores:
                ...
                target_scores_lowres_interp = F.interpolate(
                    ...
                    data["test_ltrb_target_highres"].shape[-2:],
                    mode="bicubic",
                )
