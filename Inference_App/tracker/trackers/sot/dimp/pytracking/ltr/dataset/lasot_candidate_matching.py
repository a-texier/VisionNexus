...
class LasotCandidateMatching(BaseVideoDataset):
    ...
    def __init__(
        ...
        path_to_json = (
            env_settings().lasot_candidate_matching_dataset_path
    ...
    def get_frame_states(self):
        ...
        for seq_id, seq_name in enumerate(self.sequence_list):
            if "frame_states" in self.dataset[seq_name]:
                for frame_state_name, frame_state_indices in self.dataset[seq_name][
                    "frame_states"
                ].items():
    ...
    def get_subseq_states(self):
        ...
        for seq_id, seq_name in enumerate(self.sequence_list):
            if "subseq_states" in self.dataset[seq_name]:
                for subseq_state_name, subseq_state_indices in self.dataset[seq_name][
                    "subseq_states"
                ].items():
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        for key in dumped_data_frame_list[0].keys():
            frames_dict[key] = [
                data[key] for data in dumped_data_frame_list
            ]  # is cloning needed here?
