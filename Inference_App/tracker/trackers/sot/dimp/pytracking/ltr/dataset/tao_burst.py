...
class TAOBURST(BaseVideoDataset):
    ...
    def _preload_sequence_info(self, seq_id):
        ...
        if objid is None:
            ...
            bbox = torch.tensor(
                [b[objid] if objid in b else [-1, -1, -1, -1] for b in anno["annotations"]],
                dtype=torch.float,
            )
    ...
    def _get_sequence_path(self, seq_id):
        ...
        return os.path.join(
            self.root,
            "annotated_frames",
            self.annos[sequence]["split"],
            self.annos[sequence]["dataset_name"],
            self.annos[sequence]["seq_name"],
        )

    def _get_frame_path(self, seq_path, seq_id, frame_id):
        return os.path.join(
            seq_path, self.annos[self.sequence_list[seq_id][1]]["annotated_image_paths"][frame_id]
        )
        ...
        for key, value in anno.items():
            if isinstance(value, list) or torch.is_tensor(value):
                if isinstance(value[frame_ids[0]], dict):
                    anno_frames[key] = [
                        {k: torch.tensor(v).clone() for k, v in value[f_id].items()}
