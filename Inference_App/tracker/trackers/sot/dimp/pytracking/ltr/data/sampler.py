                    ...
                    if occlusion_sampling:
                        ...
                        if (end_frame - base_frame_id) < num_test_frames:
                            ...
                            end_frame = random.randint(
                                end_frame, min(len(visible) - 1, end_frame + rem_frames)
                            )
                        ...
                        test_frame_ids = [
                            base_frame_id + int(x * step_len) for x in range(0, num_test_frames)
                        ]
                        ...
                        test_frame_ids = list(
                            range(base_frame_id, min(len(visible), base_frame_id + num_test_frames))
                        )
                        test_valid_image[: len(test_frame_ids)] = 1
    ...
    def __init__(
        ...
        if subseq_modes is not None:
            ...
            if p_subseq_modes is None:
                p_subseq_modes = [
                    self.dataset_subseq_states[mode].shape[0] for mode in self.subseq_modes
                ]
        ...
        if frame_modes is not None:
            ...
            if p_frame_modes is None:
                p_frame_modes = [
                    self.dataset_frame_states[mode].shape[0] for mode in self.frame_modes
                ]
...
class TaMOsDatasetSampler(TrackingSampler):
    ...
    def __getitem__(self, index):
        ...
        if sample is None:
            ...
            self.samples = [
                dict(zip(keys, self._sample_training_data())) for _ in range(self.buffer_size)
            ]

            sample = self.samples[self.buffer_idx]

        train_frames, train_anno, meta_obj_train = self.datasets[sample["dataset_id"]].get_frames(
            sample["seq_id"], sample["train_frame_ids"]
        )
        test_frames, test_anno, meta_obj_test = self.datasets[sample["dataset_id"]].get_frames(
            sample["seq_id"], sample["test_frame_ids"]
        )
    ...
    def _try_sample_training_data(self):
        ...
        while not enough_visible_frames:
            ...
            if not is_video_dataset:
                ...
                visible_mot = torch.ByteTensor(
                    [
                        [1 if str(i) in anno else 0 for i in range(num_obj)]
                        for anno in seq_info_dict["bbox"]
                    ]
                )
                visible = torch.any(visible_mot, dim=1)
            else:
        ...
        if is_video_dataset:
            ...
            while test_frame_ids is None:
                ...
                if is_mot_dataset:
                    visible_for_train_frames = self.recompute_visiblity_for_train_frame_objects(
                        visible_mot, train_frame_ids
                    )
                else:
                    visible_for_train_frames = visible.clone()

                test_frame_ids = self._sample_visible_ids(
                    visible_for_train_frames,
                    min_id=train_frame_ids[0] + 1 + gap,
