...
class ATOMProcessing(BaseProcessing):
    ...
    def _get_jittered_box(self, box, mode):
        ...
        max_offset = (
            jittered_size.prod().sqrt() * torch.tensor(self.center_jitter_factor[mode]).float()
        )
...
class KLDiMPProcessing(BaseProcessing):
    ...
    def _generate_label_density(self, target_bb):
        ...
        feat_sz = self.label_density_params["feature_sz"] * self.label_density_params.get(
            "interp_factor", 1
        )
        gauss_label = prutils.gaussian_label_function(
            target_bb.view(-1, 4),
        ...
        for i in range(num_frames):
            ...
            for _ in range(10):
                ...
                if self.center_jitter_param.get(mode + "_mode", "uniform") == "uniform":
                    max_offset = (
                        jittered_size.prod().sqrt() * self.center_jitter_param[mode + "_factor"]
                    ).item()
                    ...
                    if self.center_jitter_param.get(mode + "_limit_motion", False) and i > 0:
                        ...
                        if (
                            ...
                            jittered_center[0] = (
                                orig_box[0] + 0.5 * orig_box[2] + max_offset * offset_factor[0] * -1
                            )
                        ...
                        if (
                            ...
                            jittered_center[1] = (
                                orig_box[1] + 0.5 * orig_box[3] + max_offset * offset_factor[1] * -1
                            )
...
class TargetCandiateMatchingProcessing(BaseProcessing):
    ...
    def _original_and_augmented_frame(self, data: TensorDict):
        ...
        gt_assignment[
            torch.arange(self.num_target_candidates), torch.arange(self.num_target_candidates)
        ] = valid0 * valid1
    ...
    def _pad_with_fake_candidates(
        self, img_coords_pad0, img_coords_pad1, valid0, valid1, sa_box0, sa_box1, img_shape
    ):
        H, W = img_shape[:2]
        ...
        for i in range(0, self.num_target_candidates):
            for k in range(0, 2):
                if filled[k][i] == 0:
                    ...
                    cs_used = torch.cat(
                        [img_coords_pad[0][filled[0] == 1], img_coords_pad[1][filled[1] == 1]],
                        dim=0,
                    )
    ...
    def _add_fake_candidate_scores(self, scores, valid):
        ...
        scores_pad[valid == 0] = (torch.abs(torch.randn((valid == 0).sum())) / 50).clamp_max(
            0.025
        ) + 0.05
        return scores_pad
...
class LTRBDenseRegressionProcessing(BaseProcessing):
    ...
    def _generate_ltbr_regression_targets(self, target_bb):
        shifts_x = torch.arange(
            0, self.output_sz, step=self.stride, dtype=torch.float32, device=target_bb.device
        )
...
class TaMOsProcessing(BaseProcessing):
    ...
    def randomly_remap_trk_ids(self, train_anno, test_anno):
        ...
        train_anno = [
            {trk_id_map[k]: v for k, v in anno.items() if k in trk_ids} for anno in train_anno
        ]
        test_anno = [
            {trk_id_map[k]: v for k, v in anno.items() if k in trk_ids} for anno in test_anno
        ]
