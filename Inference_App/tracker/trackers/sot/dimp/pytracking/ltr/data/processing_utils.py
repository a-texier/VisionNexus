...
def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None):
    ...
    if masks is None:
        crops_resize_factors = [
            sample_target(f, a, search_area_factor, output_sz) for f, a in zip(frames, box_extract)
        ]
...
def sample_target_adaptive(
    ...
    # Crop image
    crop_sz_x, crop_sz_y = (
        (output_sz * (target_bb[2:].prod() / output_sz.prod()).sqrt() * search_area_factor)
        .ceil()
        .long()
        .tolist()
    )
...
def crop_and_resize(im, box, crop_bb, output_sz, mask=None):
    ...
    if mask is not None:
        mask_out = F.interpolate(
            mask_crop_padded[None, None], (output_sz[1], output_sz[0]), mode="nearest"
        )[0, 0]
...
def transform_box_to_crop(
    box: torch.Tensor, crop_box: torch.Tensor, crop_sz: torch.Tensor
) -> torch.Tensor:
    """Transform the box co-ordinates from the original image co-ordinates to the co-ordinates of the cropped image
    args:
...
def target_image_crop(
    ...
    if masks is None:
        ...
        frame_crops_boxes_masks = [
            sample_target_adaptive(
                f, a, search_area_factor, output_sz, mode, max_scale_change, mask=m
            )
            for f, a, m in zip(frames, box_extract, masks)
        ]
    ...
    # find the bb location in the crop
    box_crop = [
        transform_box_to_crop(bb_gt, crop_bb, crop_sz) for bb_gt, crop_bb in zip(box_gt, crop_boxes)
    ]
