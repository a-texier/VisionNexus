...
class TransformBase:
    ...
    def transform_bboxes(self, bboxes, image_shape, *rand_params):
        return {
            i: self.transform_bbox(bbox, image_shape, *rand_params) for i, bbox in bboxes.items()
        }
...
class Blur(TransformBase):
    ...
    def transform_image(self, image):
        if torch.is_tensor(image):
            ...
            return F.conv2d(im1, self.filter[1], padding=(0, self.filter_size[1])).view(
                -1, sz[0], sz[1]
            )
        else:
...
class RandomAffine(TransformBase):
    ...
    def roll(self):
        ...
        return (
            ...
            theta,
            (shear_x, shear_y),
            (scale_factor, scale_factor * ar_factor),
            scale_center,
        )

    def _construct_t_mat(
    ...
    def transform_image(self, image, do_flip, theta, shear_values, scale_factors, scale_center):
        ...
        t_mat = self._construct_t_mat(
            image.shape[:2], do_flip, theta, shear_values, scale_factors, scale_center
        )
    ...
    def transform_coords(
        self, coords, image_shape, do_flip, theta, shear_values, scale_factors, scale_center
    ):
        t_mat = self._construct_t_mat(
            image_shape, do_flip, theta, shear_values, scale_factors, scale_center
        )
        ...
        coords_xy_t_cropped = torch.stack(
            [
                torch.clamp(coords_xy_t[0], min=0, max=image_shape[1]),
                torch.clamp(coords_xy_t[1], min=0, max=image_shape[0]),
            ],
            dim=0,
        )
    ...
    def transform_mask(self, mask, do_flip, theta, shear_values, scale_factors, scale_center):
        t_mat = self._construct_t_mat(
            mask.shape[:2], do_flip, theta, shear_values, scale_factors, scale_center
        )
        output_sz = (mask.shape[1] + 2 * self.pad_amount, mask.shape[0] + 2 * self.pad_amount)

        mask_t = cv.warpAffine(
            mask.numpy(), t_mat, output_sz, flags=cv.INTER_NEAREST, borderMode=self.border_flag
        )
