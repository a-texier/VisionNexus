...
def run(settings):
    ...
    settings.use_test_frame_encoding = (
        False  # Set to True to use the same as in the paper but is less stable to train.
    )
    ...
    data_processing_train = processing.TaMOsProcessing(
        max_num_objects=settings.max_num_objects,
    ...
    loader_train = MultiEpochLTRLoader(
        "train",
        dataset_train,
        training=True,
    ...
    dataset_sot_val = sampler.TaMOsDatasetSampler(
        [got10k_val],
        [1],
    ...
    dataset_mot_val = sampler.TaMOsDatasetSampler(
        [imagenetvid_val],
        [1],
        samples_per_epoch=settings.val_samples_per_epoch,
        max_gap=settings.max_gap,
    ...
    net = tamosnet.tamosnet_swin_base(
        ...
        head_layer=["1", "2"],
        num_tokens=settings.max_num_objects,
        label_enc="gaussian",
        box_enc="ltrb_token",
    ...
    actor = actors.TaMOsActor(
        net=net, objective=objective, loss_weight=loss_weight, prob=True, fg_cls_loss=True
    )
    ...
    trainer = LTRTrainer(
        actor,
        [loader_train, loader_sot_val, loader_mot_val],
        optimizer,
        settings,
        lr_scheduler,
