...
class ResponsePredictor(nn.Module):
    def __init__(
        ...
        for out_dim in representation_predictor_dims:
            representation_predictor_list.append(
                conv_block(
                    in_dim, out_dim, kernel_size=3, stride=1, padding=1, batch_norm=False, relu=True
                )
            )
            in_dim = out_dim
    ...
    def forward(
        ...
        propagation_weight_norm = cost_volume_p2.view(
            num_sequences, feat_sz[0] * feat_sz[1], 1, feat_sz[0], feat_sz[1]
        )
        ...
        if self.conf_measure == "max":
            propagation_conf = propagation_weight_norm.view(
                num_sequences, -1, feat_sz[0], feat_sz[1]
            ).max(dim=1)[0]
        ...
        if self.conf_measure != "none":
            ...
            predictor_input = torch.cat(
                (
                    propagated_h,
                    dimp_score_cur.view(num_sequences, 1, *dimp_score_cur.shape[-2:]),
                    propagation_conf,
                ),
                dim=1,
            )
