...
class AtomIoUNet(nn.Module):
    ...
    def forward(self, feat1, feat2, bb1, proposals2):
        ...
        feat1 = [
            f[0, ...] if f.dim() == 5 else f.reshape(-1, num_sequences, *f.shape[-3:])[0, ...]
            for f in feat1
        ]
        bb1 = bb1[0, ...]
        ...
        modulation = [
            f.reshape(1, num_sequences, -1)
            .repeat(num_images, 1, 1)
            .reshape(num_sequences * num_images, -1)
            for f in modulation
        ]
    ...
    def predict_iou(self, modulation, feat, proposals):
        ...
        roi2 = torch.cat(
            (
                batch_index.reshape(batch_size, -1, 1).expand(-1, num_proposals_per_batch, -1),
                proposals_xyxy,
            ),
            dim=2,
        )
