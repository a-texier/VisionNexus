    ...
    def forward(self, prediction, label, target_bb=None, valid_samples=None):
        ...
        if valid_samples is not None:
            ...
            if self.use_with_logits:
                prediction_accuracy_persample = F.binary_cross_entropy_with_logits(
                    prediction, label, reduction="none"
                )
            else:
                prediction_accuracy_persample = F.binary_cross_entropy(
                    prediction, label, reduction="none"
                )
            ...
            if self.use_with_logits:
                prediction_accuracy = F.binary_cross_entropy_with_logits(
                    prediction, label, reduction="none"
                )
            else:
...
class TrackingClassificationAccuracy(nn.Module):
    ...
    def forward(self, prediction, label, valid_samples=None):
        ...
        if valid_samples is not None:
            ...
            if num_valid_samples > 0:
                prediction_accuracy = (
                    valid_samples * prediction_correct.float()
