    ...
    def forward(self, feat1, feat2):
        ...
        if self.abs_coordinate_output:
            cost_volume = cost_volume.view(
                batch_size, -1, cost_volume.shape[-2], cost_volume.shape[-1]
            )
...
def remap_cost_volume(cost_volume):
    ...
    cost_volume_remapped = torch.zeros(
        (batch_size, num_rows, num_cols, num_rows, num_cols),
    ...
    for r in range(num_rows):
        for c in range(num_cols):
            ...
            cost_volume_remapped[
                :, r1_ + r1_pad_ : r2_ - r2_pad_, c1_ + c1_pad_ : c2_ - c2_pad_, r, c
