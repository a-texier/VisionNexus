...
class DiMPSteepestDescentGN(nn.Module):
    ...
    def __init__(
        ...
        self.target_mask_predictor[0].weight.data = (
            mask_init_factor * torch.tanh(2.0 - d) + init_bias
        )
    ...
    def forward(self, weights, feat, bb, sample_weight=None, num_iter=None, compute_losses=True):
        ...
        center = ((bb[..., :2] + bb[..., 2:] / 2) / self.feat_stride).reshape(-1, 2).flip(
            (1,)
        ) - dmap_offset
        ...
        if sample_weight is None:
            ...
            sample_weight = (
                sample_weight.sqrt().reshape(num_images, num_sequences, 1, 1) * spatial_weight
            )
        ...
        for i in range(num_iter):
            ...
            if compute_losses:
                losses.append(
                    ((residuals**2).sum() + reg_weight * (weights**2).sum()) / num_sequences
                )

            # Compute gradient
            residuals_mapped = score_mask * (sample_weight * residuals)
            weights_grad = (
                filter_layer.apply_feat_transpose(
        ...
        if compute_losses:
            ...
            losses.append(
                (
                    ((sample_weight * (scores - label_map)) ** 2).sum()
                    + reg_weight * (weights**2).sum()
                )
                / num_sequences
            )
...
class PrDiMPSteepestDescentNewton(nn.Module):
    ...
    def forward(self, weights, feat, bb, sample_weight=None, num_iter=None, compute_losses=True):
        ...
        def _compute_loss(scores, weights):
            return (
                torch.sum(
                    ...
                    * (
                        torch.log(scores.exp().sum(dim=(-2, -1)) + exp_reg)
        ...
        for i in range(num_iter):
            ...
            # Compute gradient
            weights_grad = (
                filter_layer.apply_feat_transpose(feat, res, filter_sz, training=self.training)
                + reg_weight * weights
            )
            ...
            grad_hes_grad = (
                (scores_grad * hes_scores_grad)
                .reshape(num_images, num_sequences, -1)
                .sum(dim=2)
                .clamp(min=0)
            )
            grad_hes_grad = (sample_weight.reshape(sample_weight.shape[0], -1) * grad_hes_grad).sum(
                dim=0
            )
