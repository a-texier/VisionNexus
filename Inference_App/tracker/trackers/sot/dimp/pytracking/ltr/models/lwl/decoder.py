...
class TSE(nn.Module):
    def __init__(self, fc, ic, oc):
        ...
        self.reduce = nn.Sequential(
            conv(fc, oc, 1), relu(), conv(oc, oc, 1)
        )  # Reduce number of feature dimensions
        nc = ic + oc
        self.transform = nn.Sequential(
            conv(nc, nc, 3), relu(), conv(nc, nc, 3), relu(), conv(nc, oc, 3), relu()
        )
    ...
    def __init__(self, oc, use_bn=False):
        ...
        if use_bn:
            self.bblock = nn.Sequential(
                conv(oc, oc, 3), nn.BatchNorm2d(oc), relu(), conv(oc, oc, 3, bias=False)
            )
        else:
            self.bblock = nn.Sequential(
                conv(oc, oc, 3), relu(), conv(oc, oc, 3, bias=False)
            )  # Basic block
...
class Upsampler(nn.Module):
    ...
    def forward(self, x, image_size):
        x = F.interpolate(
            x, (2 * x.shape[-2], 2 * x.shape[-1]), mode="bicubic", align_corners=False
        )
...
class LWTLDecoder(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=32, ft_channels=None, use_bn=False):
        ...
        for L, fc in self.ft_channels.items():
            if not L == last_layer:
                self.proj[L] = nn.Sequential(
                    conv(oc[prev_layer] * out_channels, oc[L] * out_channels, 1), relu()
                )
