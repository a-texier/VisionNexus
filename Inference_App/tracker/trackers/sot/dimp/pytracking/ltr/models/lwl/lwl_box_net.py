...
class LWTLBoxNet(nn.Module):
    ...
    def forward(
        self, train_imgs, test_imgs, train_masks, test_masks, bb_train, num_refinement_iter=2
    ):
        ...
        train_feat = self.extract_backbone_features(
            train_imgs.contiguous().view(
                -1, train_imgs.shape[-3], train_imgs.shape[-2], train_imgs.shape[-1]
            )
        )
        ...
        train_feat_clf = self.extract_classification_feat(
            train_feat
        )  # seq*frames, channels, height, width
        test_feat_clf = self.extract_classification_feat(
            test_feat
        )  # seq*frames, channels, height, width
        ...
        test_feat_clf = test_feat_clf.view(
            num_test_frames, num_sequences, *test_feat_clf.shape[-3:]
        )
        ...
        decoder_feat["mask_enc"] = target_scores_last_iter.view(
            -1, *target_scores_last_iter.shape[-3:]
        )
