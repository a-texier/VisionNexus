    ...
    if multiple_filters:
        if dilation_factors is None:
            ...
            for d_factor, num_filters_with_d in dilation_factors.items():
                ...
                scores_d = scores_d.view(
                    num_images, num_sequences, -1, scores_d.shape[-2], scores_d.shape[-1]
                )
...
def _apply_filter_ksz1(feat, filter):
    ...
    scores = torch.matmul(
        filter.reshape(num_sequences, 1, num_filters, num_channels),
...
def _apply_feat_transpose_v1(feat, input, filter_ksz):
    ...
    return filter_grad.view(
        num_images, num_sequences, -1, filter_grad.shape[-2], filter_grad.shape[-1]
    ).sum(dim=0)
...
def _apply_feat_transpose_v2(feat, input, filter_ksz):
    ...
    if multiple_filters:
        ...
        if num_images == 1:
            return (
                filter_grad.view(
                    num_filters, num_sequences, -1, filter_grad.shape[-2], filter_grad.shape[-1]
                )
                .flip((3, 4))
                .permute(1, 0, 2, 3, 4)
            )
...
def _apply_feat_transpose_v3(feat, input, filter_ksz):
    ...
    return (
        filter_grad.view(
            -1, num_images, num_sequences, filter_grad.shape[-2], filter_grad.shape[-1]
        )
        .sum(dim=1)
        .permute(1, 0, 2, 3)
    )
