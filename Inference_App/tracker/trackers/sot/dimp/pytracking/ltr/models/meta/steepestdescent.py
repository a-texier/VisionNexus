    ...
    def forward(self, meta_parameter: TensorList, num_iter=None, *args, **kwargs):
        ...
        for i in range(num_iter):
            ...
            step = g.apply(
                lambda e: (
                    alpha.reshape(
                        [-1 if d == self._parameter_batch_dim else 1 for d in range(e.dim())]
                    )
                    * e
                )
            )
...
class KLRegSteepestDescent(nn.Module):
    ...
    def forward(self, meta_parameter: TensorList, num_iter=None, **kwargs):
        ...
        for i in range(num_iter):
            ...
            # Multiply gradient with Jacobian
            scores_grad = torch.autograd.grad(weights_grad, dLds, weights_grad, create_graph=True)[
                0
            ]
            ...
            alpha_num = sum(gg)
            alpha_den = (
                grad_hes_grad + sum(gg * reg_weight) + self.steplength_reg * alpha_num
            ).clamp(1e-8)
        ...
        if self.compute_losses:
            losses.append(
                _compute_loss(self.score_predictor(meta_parameter, **kwargs), meta_parameter)
            )
