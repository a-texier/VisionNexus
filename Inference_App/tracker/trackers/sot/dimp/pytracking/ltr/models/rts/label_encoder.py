...
class ResidualDS16SW(nn.Module):
    ...
    def __init__(self, layer_dims, use_bn=True):
        ...
        self.samp_w_pred = nn.Conv2d(
            layer_dims[2], layer_dims[3], kernel_size=3, padding=1, stride=1
        )
...
class ResidualDS16SW_Clf(nn.Module):
    ...
    def __init__(self, layer_dims, use_bn=True):
        super().__init__()
        self.conv_block = conv_block(
            1, layer_dims[0], kernel_size=3, stride=1, padding=1, batch_norm=use_bn
        )
        self.pool = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)

        ds1 = nn.Conv2d(layer_dims[0], layer_dims[1], kernel_size=3, padding=1, stride=1)
        self.res1 = BasicBlock(
            layer_dims[0], layer_dims[1], stride=1, downsample=ds1, use_bn=use_bn
        )

        ds2 = nn.Conv2d(layer_dims[1], layer_dims[2], kernel_size=3, padding=1, stride=1)
        self.res2 = BasicBlock(
            layer_dims[1], layer_dims[2], stride=1, downsample=ds2, use_bn=use_bn
        )
...
class ResidualDS16FeatSWBoxCatMultiBlock(nn.Module):
    ...
    def bbox_to_gauss(self, bbox, sz):
        ...
        for i, bb in enumerate(bbox):
            ...
            dtotsqr = (
                d_xcoords**2 / (self.gauss_scale * w) ** 2
                + d_ycoords**2 / (self.gauss_scale * h) ** 2
            )
...
class ResidualDS16FeatSWBox(nn.Module):
    def __init__(
        ...
        self.res3 = BasicBlock(
            layer_dims[2] + feat_dim, layer_dims[3], stride=1, downsample=ds3, use_bn=use_bn
        )
        ...
        if self.use_sample_w:
            self.samp_w_pred = nn.Conv2d(
                layer_dims[3], layer_dims[4], kernel_size=3, padding=1, stride=1
            )
