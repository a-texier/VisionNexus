...
class FilterPredictor(nn.Module):
    ...
    def predict_filter(
        ...
        test_feat_seq = (
            test_feat.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)
        )  # Nf_te*H*W, Ns, C
        train_feat_seq = (
            train_feat.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)
        )  # Nf_tr*H*W, Ns, C
        train_label_seq = (
            train_label.permute(1, 0, 2, 3).flatten(1).permute(1, 0).unsqueeze(2)
        )  # Nf_tr*H*W,Ns,1
        ...
        train_ltrb_target_enc = self.box_encoding(train_ltrb_target_seq_T).permute(
            2, 0, 1
        )  # Nf_tr*H*H,Ns,C
    ...
    def predict_cls_bbreg_filters_parallel(
        ...
        test_feat_seq = (
            test_feat_stack.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)
        )  # Nf_te*H*W, Ns, C
        train_feat_seq = (
            train_feat_stack.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)
        )  # Nf_tr*H*W, Ns, C
        train_label_seq = (
...
class GOTFilterPredictor(nn.Module):
    ...
    def predict_filter(
        ...
        train_label_seq = (
            train_label.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)
        )  # Nf_tr*H*W,Ns,num_objects
        train_ltrb_target_seq_T = train_ltrb_target.permute(1, 3, 0, 2, 4, 5).flatten(
            2
        )  # Ns,4,Nf_tr*H*W
        ...
        if self.box_enc == "ltrb" and self.num_tokens == 1:
            ...
            train_ltrb_target_seq_T_nobj = train_ltrb_target.permute(1, 2, 3, 0, 4, 5).reshape(
                nseq * nobj, c, nframes * h * w
            )
            train_ltrb_target_enc = self.box_encoding(train_ltrb_target_seq_T_nobj).reshape(
                nseq, nobj, -1, nframes * h * w
            )
        ...
        output_embed, enc_mem = self.transformer(
            feat, mask=None, query_embed=self.query_embed_fg.weight[p_indices], pos_embed=pos
        )

        enc_opt = enc_mem[-h * w :].transpose(0, 1)
        enc_opt = enc_opt.permute(0, 2, 1).reshape(test_feat.shape)

        dec_opt = output_embed.permute(1, 0, 2, 3).reshape(
            test_feat.shape[1], -1, test_feat.shape[2], 1, 1
        )
    ...
    def predict_cls_bbreg_filters_parallel(
        ...
        if train_feat.shape[0] == num_gth_frames:
            dec_opt, enc_opt = self.predict_filter(
                train_feat, test_feat, train_label, train_ltrb_target
            )
            cls_dec_opt, bbreg_dec_opt, cls_enc_opt, bbreg_enc_opt = (
                dec_opt,
                dec_opt,
                enc_opt,
                enc_opt,
            ...
            cls_dec_opt, bbreg_dec_opt, cls_enc_opt, bbreg_enc_opt = (
                self._predict_cls_bbreg_filters_parallel(
                    train_feat, test_feat, train_label, num_gth_frames, train_ltrb_target
                )
            )
        return cls_dec_opt, bbreg_dec_opt, cls_enc_opt, bbreg_enc_opt

    def _predict_cls_bbreg_filters_parallel(
        self, train_feat, test_feat, train_label, num_gth_frames, train_ltrb_target, *args, **kwargs
    ):
        # train_label size guess: Nf_tr, Ns, H, W.
        if train_feat.dim() == 4:
        ...
        if self.box_enc == "ltrb" and self.num_tokens == 1:
            ...
            train_ltrb_target_seq_T_nobj = train_ltrb_target_stack.permute(
                1, 2, 3, 0, 4, 5
            ).reshape(nseq * nobj, c, nframes * h * w)
            train_ltrb_target_enc = self.box_encoding(train_ltrb_target_seq_T_nobj).reshape(
                nseq, nobj, -1, nframes * h * w
            )
            fg_token = self.query_embed_fg.weight[p_indices].reshape(1, self.num_tokens, -1, 1)
        ...
        output_embed, enc_mem = self.transformer(
            feat, mask=src_key_padding_mask, query_embed=self.query_embed_fg.weight, pos_embed=pos
        )
