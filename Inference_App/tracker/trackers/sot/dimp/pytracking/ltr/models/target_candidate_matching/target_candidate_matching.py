...
class DescriptorExtractor(nn.Module):
    ...
    def forward(self, x, coords):
        ...
        desc = feats[
            torch.arange(x.shape[0]).unsqueeze(1), :, coords[:, :, 0].long(), coords[:, :, 1].long()
        ]
...
class TargetCandidateMatchingNetwork(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        frame_feat0 = self.extract_backbone_features(
            img_cropped0.reshape(-1, *img_cropped0.shape[-3:])
        )
        frame_feat1 = self.extract_backbone_features(
            img_cropped1.reshape(-1, *img_cropped1.shape[-3:])
        )
...
def target_candidate_matching_net_resnet50(
    ...
    descriptor_extractor = DescriptorExtractor(
        backbone_feat_dim=1024, descriptor_dim=256, kernel_size=4
    )
