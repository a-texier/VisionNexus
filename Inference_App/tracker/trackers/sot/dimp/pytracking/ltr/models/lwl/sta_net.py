...
class STANet(nn.Module):
    ...
    def forward(self, train_imgs, train_bbox):
        ...
        train_feat_clf = self.extract_target_model_features(
            train_feat
        )  # seq*frames, channels, height, width

        train_bbox_enc, _ = self.label_encoder(
            train_bbox, train_feat_clf, list(train_imgs.shape[-2:])
        )
        train_mask_enc, train_mask_sw = self.bbox_encoder(
            train_bbox, train_feat_clf, list(train_imgs.shape[-2:])
        )
        train_feat_clf = train_feat_clf.view(
            num_train_frames, num_sequences, *train_feat_clf.shape[-3:]
        )

        _, filter_iter, _ = self.target_model.get_filter(
            train_feat_clf, train_mask_enc, train_mask_sw
        )
        target_scores = [
            self.target_model.apply_target_model(f, train_feat_clf) for f in filter_iter
        ]
        ...
        _, filter_iter_segm, _ = self.target_model_segm.get_filter(
            train_feat_clf, train_segm_enc, train_segm_sw
        )
        target_scores_segm = [
            self.target_model_segm.apply_target_model(f, train_feat_clf) for f in filter_iter_segm
        ]
    ...
    def segment_target_add_bbox_encoder(
        self, bbox_mask, target_filter, test_feat_clf, test_feat, segm
    ):
