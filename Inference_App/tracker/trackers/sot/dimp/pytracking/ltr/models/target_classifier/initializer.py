...
class FilterPool(nn.Module):
    ...
    def forward(self, feat, bb):
        ...
        batch_index = (
            torch.arange(num_images_total, dtype=torch.float32).reshape(-1, 1).to(bb.device)
        )
...
class FilterInitializer(nn.Module):
    ...
    def forward(self, feat, bb):
        ...
        if num_images > 1:
            weights = torch.mean(
                weights.reshape(
                    num_images, -1, weights.shape[-3], weights.shape[-2], weights.shape[-1]
                ),
                dim=0,
            )
...
class FilterInitializerZero(nn.Module):
    ...
    def forward(self, feat, bb):
        ...
        return feat.new_zeros(
            num_sequences, self.filter_size[0], self.filter_size[1], self.filter_size[2]
        )
