...
class PredictorWrapper(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.fix_coordinate_shift:
            shift_value = (
                -torch.ones(dimp_score_cur.shape[0], 2).to(dimp_score_cur.device)
                * 0.5
                / feat_map_size
            )
        ...
        if self.fix_coordinate_shift:
            ...
            if "is_target" in auxiliary_outputs:
                auxiliary_outputs["is_target"] = shift_features(
                    auxiliary_outputs["is_target"].view(-1, 1, *dimp_score_cur.shape[-2:]),
                    shift_value,
                ).view(score_shape)

            if "is_target_after_prop" in auxiliary_outputs:
                auxiliary_outputs["is_target_after_prop"] = shift_features(
                    auxiliary_outputs["is_target_after_prop"].view(
                        -1, 1, *dimp_score_cur.shape[-2:]
                    ),
                    shift_value,
                ).view(score_shape)
            ...
            if "is_target_new" in auxiliary_outputs:
                auxiliary_outputs["is_target_new"] = shift_features(
                    auxiliary_outputs["is_target_new"].view(-1, 1, *dimp_score_cur.shape[-2:]),
                    shift_value,
                ).view(score_shape)

            pred_response = shift_features(
                pred_response.view(-1, 1, *dimp_score_cur.shape[-2:]), shift_value
            ).view(score_shape)
