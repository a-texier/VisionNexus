...
class RTSNet(nn.Module):
    def __init__(
        ...
        self.output_layers = sorted(
            list(set(self.target_model_input_layer + self.decoder_input_layers))
        )
    ...
    def forward_box_mask_sta(self, train_imgs, train_bb):
        ...
        train_bbox_enc, _ = self.box_label_encoder(
            train_bb, train_feat_tm, list(train_imgs.shape[-2:])
        )
        train_mask_enc, train_mask_sw = self.bbox_encoder(
            train_bb, train_feat_tm, list(train_imgs.shape[-2:])
        )
        train_feat_tm = train_feat_tm.view(
            num_train_frames, num_sequences, *train_feat_tm.shape[-3:]
        )

        _, filter_iter, _ = self.box_target_model.get_filter(
            train_feat_tm, train_mask_enc, train_mask_sw
        )
        target_scores = [
            self.box_target_model.apply_target_model(f, train_feat_tm) for f in filter_iter
        ]
        target_scores_last_iter = target_scores[-1]
        coarse_mask = torch.cat((train_bb, target_scores_last_iter), dim=2)
        pred_all, _ = self.box_label_decoder(
            coarse_mask, train_feat_backbone, train_imgs.shape[-2:]
        )
        ...
        _, filter_iter_segm, _ = self.box_target_model_segm.get_filter(
            train_feat_tm, train_segm_enc, train_segm_sw
        )
        target_scores_segm = [
            self.box_target_model_segm.apply_target_model(f, train_feat_tm)
            for f in filter_iter_segm
        ]
        target_scores_last_iter_segm = target_scores_segm[-1]
        coarse_mask = torch.cat((train_bb, target_scores_last_iter_segm), dim=2)
        pred_all_segm, _ = self.box_label_decoder(
            coarse_mask, train_feat_backbone, train_imgs.shape[-2:]
        )
        pred_all_segm = pred_all_segm.view(
            num_train_frames, num_sequences, *pred_all_segm.shape[-2:]
        )
    ...
    def forward_classifier_only(self, train_imgs, test_imgs, train_bb, train_label):
        ...
        clf_target_scores = self.classifier(
            train_feat_clf, test_feat_clf, train_bb, train_label=train_label
        )
    ...
    def forward(
        ...
        test_feat_backbone = self.extract_backbone_features(
            test_imgs.contiguous().view(
                -1, test_imgs.shape[-3], test_imgs.shape[-2], test_imgs.shape[-1]
            )
        )
        ...
        filter, filter_iter, _ = self.target_model.get_filter(
            train_feat_tm, few_shot_label, few_shot_sw
        )
        ...
        for i in range(num_test_frames):
            ...
            test_feat_tm_it = test_feat_tm.view(
                num_test_frames, num_sequences, *test_feat_tm.shape[-3:]
            )[i : i + 1, ...]

            # Apply the target model to obtain mask encodings.
            mask_encoding_pred = [
                self.target_model.apply_target_model(f, test_feat_tm_it) for f in filter_iter
            ]

            test_feat_backbone_it = {
                k: v.view(num_test_frames, num_sequences, *v.shape[-3:])[i, ...]
                for k, v in test_feat_backbone.items()
            }
            ...
            mask_pred, decoder_feat = self.decoder(
                decoder_input, test_feat_backbone_it, test_imgs.shape[-2:]
            )
    ...
    def segment_target(self, target_filter, test_feat_tm, test_feat, encoded_clf_scores=None):
        ...
        if encoded_clf_scores is not None:
            encoded_clf_scores = interpolate(
                encoded_clf_scores[0, :, :, :, :], mask_encoding_pred.shape[-2:]
            )
    ...
    def extract_target_model_features(self, backbone_feat):
        return self.target_model.extract_target_model_features(
            self.get_backbone_target_model_features(backbone_feat)
        )
    ...
    def extract_classification_feat(self, backbone_feat):
        return self.classifier.extract_classification_feat(
...
def build_base_components(
    ...
    decoder = rts_decoder.RTSDecoder(
        num_filters, decoder_mdim, decoder_input_layers_channels, use_bn=True
    )
