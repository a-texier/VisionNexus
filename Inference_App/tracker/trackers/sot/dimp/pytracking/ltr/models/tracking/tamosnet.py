...
class TaMOsNet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_bb, *args, **kwargs):
        ...
        test_scores, bbox_preds = self.head(
            train_feat_head, test_feat_head, train_bb, *args, **kwargs
        )
...
def tamosnet_resnet50(
    ...
    label_enc="gaussian",
    box_enc="ltrb",
    fpn_head_cls_output_mode=["high"],
    fpn_head_bbreg_output_mode=["high"],
    **kwargs,
):
    # Backbone
    ...
    filter_predictor = fp.GOTFilterPredictor(
        transformer,
    ...
    head = heads.FPNHead(
        ...
        fpn=fpn,
        head_layer=head_layer[-1],
        cls_output_mode=fpn_head_cls_output_mode,
        bbreg_output_mode=fpn_head_bbreg_output_mode,
    )
...
def tamosnet_swin_base(
    ...
    **kwargs,
    ...
    # Backbone
    backbone_net = backbones.swin_base384_flex(
        pretrained=backbone_pretrained, frozen_layers=frozen_backbone_layers
    )
