...
class NerfPositionalEncoding(nn.Module):
    def __init__(
        self, depth=10, sine_type="lin_sine", avoid_aliasing=False, max_spatial_resolution=None
    ):
        """
        ...
        if avoid_aliasing and max_spatial_resolution == None:
            raise ValueError(
                "Please specify the maxima spatial resolution (h, w) of the feature map"
            )
        elif avoid_aliasing:
...
class PositionEmbeddingSine(nn.Module):
    ...
    def __init__(
        ...
        self.sine = NerfPositionalEncoding(
            num_pos_feats // 2, sine_type, avoid_aliazing, max(max_spatial_resolution)
        )
