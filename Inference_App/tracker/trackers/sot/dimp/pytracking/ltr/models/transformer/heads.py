...
class DenseBoxRegressor(nn.Module):
    ...
    def forward(self, feat, filter):
        ...
        if attention.dim() == 5:
            ltrb = ltrb.reshape(
                1, feats_att.shape[1], feats_att.shape[2], 4, feats_att.shape[4], feats_att.shape[5]
            )  # (1, nf*ns, num_obj, 4, h, w)
...
class FPNHead(nn.Module):
    def __init__(
        ...
        head_layer,
        separate_filters_for_cls_and_bbreg=False,
        cls_output_mode=["high"],
        bbreg_output_mode=["high"],
    ):
        super().__init__()
    ...
    def forward(self, train_feat, test_feat, train_bb, *args, **kwargs):
        ...
        cls_filter, breg_filter, test_feat_enc = self.get_filter_and_features(
            train_head_feat, test_head_feat, *args, **kwargs
        )
