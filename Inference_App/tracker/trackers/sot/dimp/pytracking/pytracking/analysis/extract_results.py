...
def extract_results_prec_rec_f1(
    trackers,
    dataset,
    report_name,
    skip_missing_seq=False,
    ...
    for trk in tqdm(trackers):
        ...
        for seq_id, seq in enumerate(dataset):
            ...
            base_results_path = os.path.join(
                settings.results_path, os.path.relpath(base_results_path, settings.results_path)
            )
            results_path = "{}.txt".format(base_results_path)
            ...
            if os.path.isfile(results_path) and os.path.isfile(scores_path):
                ...
                scores = torch.tensor(
                    load_text(str(scores_path), delimiter=("\t", ","), dtype=np.float64)
                )
        ...
        err_overlap_cat, _, _, valid_frame_cat = calc_seq_err_robust(
            pred_bb_cat, anno_bb_cat, dataset[0].dataset, target_visible_cat
        )
        ...
        overlap_sorted[target_invisible_sorted] = 0.0
        ...
        raw_data[key] = {"prec": prec, "rec": rec, "f1_max": f1_max, "ths_max": ths_max, "idx": idx}
