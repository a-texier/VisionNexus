...
def check_and_load_precomputed_results(
    trackers, dataset, report_name, force_evaluation=False, f1_prec_rec=False, **kwargs
):
    # Load data
...
def plot_results(
    ...
    **kwargs,
    ...
    if "f1_prec_rec" in plot_types:
        f1_prec_rec_plot_opts = {
            "plot_type": "f1_prec_rec",
        ...
        plot_draw_save_f1_prec_rec(
            eval_data, tracker_names, plot_draw_styles, result_plot_path, f1_prec_rec_plot_opts
        )
...
def plot_draw_save_f1_prec_rec(data, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    data = data["raw_data"]
    index_sort = torch.argsort(
        torch.tensor([val["f1_max"] for val in data.values()]), descending=False
    )
    ...
    for id, id_sort in enumerate(index_sort):
        ...
        if len(disp_name) > 3 and disp_name[:3] == r"\bf":
            legend_text.append(
                r"$\mathbf{{{}}}$ $\mathbf{{[{:.3f}]}}$".format(disp_name[3:], val["f1_max"])
            )
        else:
...
def print_results(
    ...
    if "f1_prec_rec" in plot_types:
        names = [
            "{}_{}".format(trk["disp_name"], trk["run_id"])
            if trk["run_id"] is not None
            else trk["disp_name"]
...
def print_results_per_attribute(
    trackers, datasets, report_name, merge_results=False, verbose=False, **kwargs
):
    ...
    for name, dataset in datasets.items():
        # Load pre-computed results
        eval_data = check_and_load_precomputed_results(
            trackers, dataset, report_name, verbose=verbose, **kwargs
        )
...
def plot_radar_draw_save(scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    for id, id_sort in enumerate(index_sort):
        ...
        if len(disp_name) > 3 and disp_name[:3] == r"\bf":
            legend_text.append(
                r"$\mathbf{{{}}}$ $\mathbf{{[{:.1f}]}}$".format(
                    disp_name[3:], avg_score_per_tracker[id_sort]
                )
            )
        else:
    ...
    fig.tight_layout()
    fig.savefig(
        "{}/radar_plot.pdf".format(result_plot_path),
        dpi=300,
        format="pdf",
        transparent=True,
        bbox_inches="tight",
