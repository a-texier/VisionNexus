...
class OxUvADataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_id):
        ...
        sequence_name = "{}_{}_frames[{:06d}:{:06d}]".format(
            video_id, object_id, init_frame, last_frame + 1
        )

        frames_path = os.path.join("{}", "images", self.split, "{}").format(
            self.base_path, video_id
        )
        ...
        if self.split == "dev":
            ground_truth_rect = self.build_dev_annotations(
                frames_list, init_frame, last_frame, object_id, video_id
            )
        else:
            ground_truth_rect = self.build_test_annotations(
                frames_list, init_frame, last_frame, object_id, video_id
            )
    ...
    def build_test_annotations(self, frames_list, init_frame, last_frame, object_id, video_id):
        ...
        annotation_mask = np.logical_and(
            self.tasks_df["video_id"] == video_id, self.tasks_df["object_id"] == object_id
        )
        ...
        xxyy[:, 2:] *= h
        xywh = np.vstack(
            [xxyy[:, 0], xxyy[:, 2], xxyy[:, 1] - xxyy[:, 0], xxyy[:, 3] - xxyy[:, 2]]
        ).T
    ...
    def build_dev_annotations(self, frames_list, init_frame, last_frame, object_id, video_id):
        ...
        w, h = Image.open(frames_list[0]).size
        annotation_mask = np.logical_and(
            self.dev_annotations_df["video_id"] == video_id,
