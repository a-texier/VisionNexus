...
class LaGOTDataset(BaseDataset):
    def __init__(self, sot_mode=False):
        ...
        if sot_mode:
            self.anno_path = os.path.join(
                self.env_settings.lagot_path, "LaGOT_one_object_per_sequence_annotations_final.json"
            )
        else:
            self.anno_path = os.path.join(
                self.env_settings.lagot_path,
    ...
    def _construct_sequence(self, sequence_name):
        if self.sot_mode:
            ...
            frames_list = [
                os.path.join(self.base_path, p) for p in self.annos[sequence_name]["frames"]
            ]
            ...
            gt_bboxes = OrderedDict(
                {key: val for key, val in gt_bboxes.items() if key in track_ids}
            )
