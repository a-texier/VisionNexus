...
def determine_frame_state(tracking_data, tracker, seq, th=0.25):
    ...
    if num_candidates >= 2:
        ...
        anno_and_target_candidate_score_dists = torch.sqrt(
            torch.sum(
                (tracking_data["target_anno_coord"] - tracking_data["target_candidate_coords"])
                ** 2,
                dim=1,
            ).float()
        )
...
def extract_candidate_data(tracker, seq):
    ...
    anno_label = tracker.get_label_function(
        gth_center, tracker_data["sample_pos"], tracker_data["sample_scale"]
    )[0]
    _, target_anno_coord = dcf.max2d(anno_label.squeeze())

    score_map = tracker_data["score_map"].cpu()

    target_candidate_coords, target_candidate_scores = prutils.find_local_maxima(
        score_map.squeeze(), th=0.05, ks=5
    )
...
def run_tracker(tracker_name, parameter_file_name, dataset_name, save_dir):
    save_path = os.path.join(
        save_dir, "target_candidates_dataset_{}_{}.json".format(tracker_name, parameter_file_name)
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run tracker and dump tracker states to form distractor dataset."
    )
    parser.add_argument("tracker_name", type=str, help="Name of tracker.")
    parser.add_argument(
        "parameter_file_name", type=str, help="Name of parameter file in the tracker folder."
    )
