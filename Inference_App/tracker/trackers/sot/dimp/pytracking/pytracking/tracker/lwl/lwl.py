...
class LWL(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        if init_mask is not None and not self.params.get("init_with_box", False):
            ...
            with torch.no_grad():
                ...
                init_mask_raw, _ = self.net.decoder(
                    init_mask_enc, init_backbone_feat, im.shape[-2:]
                )
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        prev_segmentation_prob_im = (
            torch.from_numpy(prev_segmentation_prob_im).unsqueeze(0).unsqueeze(0).float()
        )
        ...
        new_state = torch.cat(
            (pred_pos[[1, 0]] - (pred_target_sz[[1, 0]] - 1) / 2, pred_target_sz[[1, 0]])
        )
    ...
    def generate_init_samples(self, im: torch.Tensor, init_mask):
        ...
        im_patches = sample_patch_transformed(
            im, init_sample_pos, init_sample_scale, aug_expansion_sz, self.transforms
        )
