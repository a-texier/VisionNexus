...
from pytracking import TensorList, complex, dcf, fourier
...
from .optim import FactorizedConvProblem, FilterOptim
...
class ECO(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.reg_filter = TensorList(
            [
                dcf.get_reg_filter(self.img_support_sz, self.base_target_sz, fparams).to(
                    self.params.device
                )
        ...
        sigma = (
            (self.filter_sz / self.img_support_sz)
        ...
        if "shift" in self.params.augmentation:
            for xf in train_xf:
                ...
                for i, shift in enumerate(self.params.augmentation["shift"]):
                    ...
                    xf[1 + i : 2 + i, ...] = fourier.shift_fs(
                        xf[1 + i : 2 + i, ...], shift=shift_samp
                    )

        # Shift sample
        shift_samp = (
            2 * math.pi * (self.pos - self.pos.round()) / (self.target_scale * self.img_support_sz)
        )
        ...
        for sw, init_sw, num in zip(
            self.sample_weights, self.init_sample_weights, num_init_samples
        ):
            sw[:num] = init_sw
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # Shift the sample
        shift_samp = (
            2 * math.pi * (self.pos - sample_pos) / (sample_scales[scale_ind] * self.img_support_sz)
        )
        train_xf = fourier.shift_fs(train_xf, shift=shift_samp)
    ...
    def localize_target(self, sf: TensorList):
        if self.params.score_fusion_strategy == "sum":
            ...
            sample_sz = torch.round(
                self.output_sz.view(1, -1) * self.params.scale_factors.view(-1, 1)
            )
            scores = 0
    ...
    def preprocess_sample(self, x: TensorList) -> TensorList:
        ...
        return TensorList(
            [dcf.interpolate_dft(xf, bf) for xf, bf in zip(sample_xf, self.interp_fs)]
        )
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        if "shift" in self.params.augmentation:
            transforms.extend(
                [augmentation.Translation(shift) for shift in self.params.augmentation["shift"]]
            )
        ...
        if "rotate" in self.params.augmentation:
            transforms.extend(
                [augmentation.Rotate(angle) for angle in self.params.augmentation["rotate"]]
            )
        if "blur" in self.params.augmentation:
            transforms.extend(
                [augmentation.Blur(sigma) for sigma in self.params.augmentation["blur"]]
            )
