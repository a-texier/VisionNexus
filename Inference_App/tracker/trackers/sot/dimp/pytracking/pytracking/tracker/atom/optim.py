...
from pytracking import TensorList, operation, optimization
...
class FactorizedConvProblem(optimization.L2Problem):
    ...
    def __call__(self, x: TensorList):
        ...
        residuals = operation.conv2d(compressed_samples, filter, mode="same").apply(
...
class ConvProblem(optimization.L2Problem):
    ...
    def __call__(self, x: TensorList):
        ...
        residuals = operation.conv2d(self.training_samples, x, mode="same").apply(
            self.response_activation
        )
        residuals = residuals - self.y
