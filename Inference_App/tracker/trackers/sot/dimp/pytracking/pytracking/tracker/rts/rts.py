...
class RTS(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if self.frame_num > 2 and update_target_model:
            ...
            if self.frame_num % self.params.get("train_sample_interval", 1) == 0 or force_seg_train:
                self.update_memory(
                    TensorList([self.prev_segm_test_x]),
                    seg_prob_crop.clone(),
        ...
        # Location of sample
        seg_scores_im, _, _, _, _ = self.convert_seg_scores_crop_to_image(
            segmentation_scores, im, sample_coords
        )
        ...
        segmentation_mask_im = (
            segmentation_mask_im.view(*segmentation_mask_im.shape[-2:]).cpu().numpy()
        )
        ...
        pos_for_clf, target_sz_for_clf, target_scale_for_clf = self.target_state_update_from_bbox(
            output_state
        )
        ...
        if trust_clf_always or (
            (self.is_lost_seg or self.seg_too_small)
    ...
    def draw_visdom_output(self, seg_mask_im, mask_encoding_pred, clf_scores):
        ...
        self.visdom.register(
            torch.from_numpy(seg_mask_im), "image", 2, "Seg Raw Mask" + self.id_str
        )
        ...
        for key in out_types:
            if "segmentation" in key or "clf_search_area" in key:
                ...
                for obj_id, out in out_all.items():
                    seg_mask_im = (
                        torch.from_numpy(out_merged["segmentation"]) == int(obj_id)
                    ).unsqueeze(0)
                    output_state = masks_to_bboxes(seg_mask_im, fmt="t")
                    output_state = output_state.cpu().view(-1).tolist()
        ...
        if self.params.get("seg_to_bb_mode") == "var":
            ...
            e_y = (
                torch.sum(
                    ...
                    * torch.arange(segmentation_prob_im.shape[-2], dtype=torch.float32)
                )
                / prob_sum
            )
            e_x = (
                torch.sum(
                    ...
                    * torch.arange(segmentation_prob_im.shape[-1], dtype=torch.float32)
                )
                / prob_sum
            )
            ...
            return torch.Tensor([e_y, e_x]), torch.Tensor(
                [e_h.sqrt() * sz_factor, e_w.sqrt() * sz_factor]
            )
        else:
            raise Exception(
                "Unknown seg_to_bb_mode mode {}".format(self.params.get("seg_to_bb_mode"))
            )
    ...
    def clip_scale_change(self, new_target_scale):
        ...
        if not isinstance(self.params.get("max_scale_change"), (tuple, list)):
            max_scale_change = (
                self.params.get("max_scale_change"),
                self.params.get("max_scale_change"),
            )
        else:
    ...
    def convert_seg_scores_crop_to_image(self, segmentation_scores, im, sample_coords):
        ...
        segmentation_scores_re = F.interpolate(
            segmentation_scores, scale_factor=sample_scale.item(), mode="bilinear"
        )
        ...
        segmentation_scores_im = torch.ones(im.shape[-2:], dtype=segmentation_scores_re.dtype) * (
            -100.0
        )
    ...
    def extract_backbone_features(
        self, im: torch.Tensor, pos: torch.Tensor, scale, sz: torch.Tensor
    ):
    ...
    def update_memory(self, sample_x: TensorList, mask, learning_rate=None):
        ...
        self.num_stored_samples = [
            n + 1 if n < self.params.sample_memory_size else n for n in self.num_stored_samples
        ]
