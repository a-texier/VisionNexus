...
from pytracking.features.preprocessing import (
    numpy_to_torch,
...
class KeepTrack(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if (
            self.candidate_collection is None
            or self.candidate_collection.object_id_of_selected_candidate == 0
        ):
            object_presence_score = torch.max(
                object_presence_score, torch.sqrt(object_presence_score.clone().detach())
            )
        ...
        if flag == "not_found":
            ...
            if self.params.get("use_iou_net", True):
                update_scale_flag = (
                    self.params.get("update_scale_when_uncertain", True) or flag != "uncertain"
                )
        ...
        if (
            self.params.get("use_iou_net", True)
            and flag != "not_found"
            and hasattr(self, "pos_iounet")
        ):
        ...
        if update_flag and self.params.get("update_classifier", False):
            ...
            train_y = self.get_label_function(
                self.pos, sample_pos[scale_ind, :], sample_scales[scale_ind]
    ...
    def search_area_rescaling(self):
        if len(self.target_scales) > 0:
            ...
            target_scales = target_scales[
                -num_scales:
            ]  # look as many samples into past as not found endures.
    ...
    def localize_target_by_candidate_matching(
        ...
        if max_score1 < self.params.get("local_max_candidate_score_th", 0.05):
            translation_vec, scale_ind, scores, flag = self.localize_target(
                score_map1, sample_pos, sample_scales
            )
            return translation_vec, scale_ind, scores, flag, max_score1, matching_visualization_data

        current_candidates = self.extract_descriptors_and_keypoints(
            backbone_feat1, score_map1, search_area_box1
        )

        if (
            self.previous_candidates is None
            or (self.frame_num - self.previous_candidates["frame_num"]) > 1
        ):
            ...
            candidate_score = current_candidates["scores"][
                self.candidate_collection.candidate_id_of_selected_candidate
            ]
            ...
            translation_vec = (
                target_disp * (self.img_support_sz / output_sz) * sample_scales[scale_ind]
            )
    ...
    def extract_descriptors_and_keypoints(self, backbone_feat, score_map, search_area_box):
        ...
        if self.params.get("matching_coordinate_system_reference", "full") == "full":
            ...
            img_coords = torch.stack(
                [
                    self.params.image_sample_size
                    * (tsm_coords[:, 0].float() / (score_map.shape[0] - 1)),
                    self.params.image_sample_size
                    * (tsm_coords[:, 1].float() / (score_map.shape[1] - 1)),
                ]
            ).permute(1, 0)

        candidates = dict(
            descriptors=descriptors, img_coords=img_coords, scores=scores, tsm_coords=tsm_coords
        )
        return candidates

    def extract_matches(
        self, descriptors0, descriptors1, img_coords0, img_coords1, scores0, scores1, image_shape
    ):
        data = {
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        if random_shift_factor > 0:
            get_rand_shift = lambda: (
                ((torch.rand(2) - 0.5) * self.img_sample_sz * random_shift_factor + global_shift)
                .long()
                .tolist()
            )
        ...
        if "relativeshift" in augs:
            get_absolute = lambda shift: (
                (torch.Tensor(shift) * self.img_sample_sz / 2).long().tolist()
            )
            self.transforms.extend(
                [
                    augmentation.Translation(
                        get_absolute(shift), aug_output_sz, global_shift.long().tolist()
                    )
    ...
    def init_target_label_certainties(self, train_x: TensorList):
        ...
        self.target_label_certainties = train_x[0].new_zeros(
            self.params.sample_memory_size, 1, 1, 1
        )
    ...
    def init_target_labels(self, train_x: TensorList):
        ...
        target_center_norm = (self.pos - self.init_sample_pos) / (
            self.init_sample_scale * self.img_support_sz
        )
        ...
        for target, x in zip(self.target_labels, train_x):
            ...
            for i, T in enumerate(self.transforms[: x.shape[0]]):
                sample_center = (
                    center_pos + torch.Tensor(T.shift) / self.img_support_sz * self.feature_sz
                )
    ...
    def update_memory(
        ...
        if replace_ind[0] >= len(self.mem_sort_indices):
            self.mem_sort_indices = torch.cat(
                [self.mem_sort_indices, torch.zeros(1, dtype=torch.long)]
            )
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        if self.params.get("filter_init_zero", False):
            self.net.classifier.filter_initializer = FilterInitializerZero(
                self.net.classifier.filter_size, x.shape[-3]
            )
        ...
        if "dropout" in self.params.augmentation and self.params.get("use_augmentation", True):
            ...
            x = torch.cat(
                [x, F.dropout2d(x[0:1, ...].expand(num, -1, -1, -1), p=prob, training=True)]
            )
        ...
        if self.params.get("window_output", False):
            if self.params.get("use_clipped_window", False):
                ...
                self.output_window = dcf.hann2d(self.output_sz.long(), centered=True).to(
                    self.params.device
                )
    ...
    def update_classifier(
        ...
        if hard_negative_flag or self.frame_num % self.params.get("train_sample_interval", 1) == 0:
            self.update_memory(
                TensorList([train_x]), train_y, target_box, learning_rate, target_label_certainty
            )
        ...
        if hard_negative_flag:
            ...
            if (
                self.params.get("use_certainty_for_weight_computation", False)
                and ths_cert > target_label_certainty
            ):
                num_iter = 0
        ...
        if self.params.get("use_certainty_for_weight_computation", False):
            target_label_certainties = self.target_label_certainties[
    ...
    def refine_target_box(
        self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale=True
    ):
        """Run the ATOM IoUNet to refine the target bounding box."""

        if hasattr(self.net.bb_regressor, "predict_bb"):
            return self.direct_box_regression(
                backbone_feat, sample_pos, sample_scale, scale_ind, update_scale
            )
    ...
    def direct_box_regression(
        ...
        # Optimize the boxes
        output_boxes = (
            self.net.bb_regressor.predict_bb(self.iou_modulation, iou_features, init_boxes)
            .view(-1, 4)
            .cpu()
        )
    ...
    def visualize_candidate_assignment_matrix(self, data):
        if data is not None:
            ...
            ax.set_xticklabels(
                ["{}".format(i) for i in range(assignment_probs.shape[1] - 1)] + ["NM"]
            )
            ax.set_yticklabels(
                ["{}".format(i) for i in range(assignment_probs.shape[0] - 1)] + ["NM"]
            )
                ...
                for j in range(assignment_probs.shape[1]):
                    if assignment_probs[i, j] < 0.5:
                        ax.text(
                            j,
                            i,
                            "{:0.2f}".format(assignment_probs[i, j]),
                            ha="center",
                            va="center",
                            color="w",
                        )
            ...
            self.visdom.visdom.matplot(
                plt, opts={"title": "assignment matrix"}, win="assignment matrix"
            )
            plt.close(fig)
    ...
    def visualize_candidate_matching(self, data):
        ...
        def add_circle(axis, x, y, id, color):
            axis.add_patch(
                patches.Circle((x, y), 20, linewidth=3, edgecolor=color, facecolor="none")
            )
            axis.text(
                x,
                y,
        ...
        if data is not None:
            ...
            ax.axis("off")
            self.visdom.visdom.matplot(
                plt, opts={"title": "matching between frames"}, win="matching between frames"
            )
            plt.close(fig)
