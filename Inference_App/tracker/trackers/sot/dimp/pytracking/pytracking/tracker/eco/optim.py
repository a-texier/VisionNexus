...
from pytracking import TensorList, complex, fourier, optimization
...
class FactorizedConvProblem(optimization.L2Problem):
    def __init__(
        ...
        self.sample_energy = (
            complex.abs_sqr(compressed_samples).mean(dim=2, keepdim=True).permute(2, 3, 0, 1)
        )
        ...
        self.diag_M.extend(
            self.params.precond_proj_param * (self.proj_energy + self.params.projection_reg)
        )
    ...
    def __call__(self, x: TensorList):
        ...
        residuals = complex.mtimes(
            compressed_samples, hf.permute(2, 3, 1, 0, 4)
        )  # (h, w, num_samp, num_filt, 2)
        ...
        for hfe, reg_filter in zip(hf, self.reg_filter):
            ...
            hfe_conv = hfe_conv.permute(0, 1, 4, 2, 3).reshape(
                -1, 1, hfe_conv.shape[-3], hfe_conv.shape[-2]
            )
    ...
    def ip_output(self, a: TensorList, b: TensorList):
        ...
        for ar, br, res_data, reg_filter in zip(a_filt_reg, b_filt_reg, a_data, self.reg_filter):
            ...
            ip_filt_reg += fourier.inner_prod_fs(
                arp[:, :, :, 2 * reg_pad2 :, :], brp[:, :, :, 2 * reg_pad2 :, :]
            )
...
class FilterOptim(optimization.ConjugateGradientBase):
    def __init__(self, params, reg_energy):
        super(FilterOptim, self).__init__(
            ...
            params.direction_forget_factor,
            (params.debug >= 3),
        )
    ...
    def run(self, num_iter, new_xf: TensorList = None):
        ...
        self.b = complex.mtimes(
            self.sample_weights.view(1, 1, 1, -1), self.training_samples
        ).permute(2, 3, 0, 1, 4)
    ...
    def A(self, hf: TensorList):
        # Classify
        sh = complex.mtimes(
            self.training_samples, hf.permute(2, 3, 1, 0, 4)
        )  # (h, w, num_samp, num_filt, 2)
        sh = complex.mult(self.sample_weights.view(1, 1, -1, 1), sh)

        # Multiply with transpose
        hf_out = complex.mtimes(
            sh.permute(0, 1, 3, 2, 4), self.training_samples, conj_b=True
        ).permute(2, 3, 0, 1, 4)
        ...
        for hfe, hfe_out, reg_filter in zip(hf, hf_out, self.reg_filter):
            ...
            if reg_pad2 > 0:
                hfe_conv = torch.cat(
                    [complex.conj(hfe[..., 1 : reg_pad2 + 1, :].flip((2, 3))), hfe], -2
                )
            else:
                hfe_conv = hfe.clone()
            ...
            hfe_out += hfe_conv.reshape(
                hfe.shape[0], hfe.shape[1], 2, hfe.shape[2], hfe.shape[3]
            ).permute(0, 1, 3, 4, 2)
