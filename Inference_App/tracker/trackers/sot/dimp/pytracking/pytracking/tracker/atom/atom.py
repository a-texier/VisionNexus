...
from pytracking import TensorList, dcf, fourier, operation
...
class ATOM(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        if self.params.feature_size_odd:
            ...
            self.img_sample_sz += feat_max_stride - (self.img_sample_sz + feat_max_stride) % (
                2 * feat_max_stride
            )
        ...
        self.output_sz = (
            self.params.score_upsample_factor * self.img_support_sz
        )  # Interpolated size of the output
        ...
        if self.params.get("window_output", False):
            if self.params.get("use_clipped_window", False):
                ...
                self.output_window = dcf.hann2d(self.output_sz.long(), centered=False).to(
                    self.params.device
                )
    ...
    def init_optimization(self, train_x, init_y):
        ...
        self.filter = TensorList(
            [
                x.new_zeros(1, cdim, sz[0], sz[1])
                for x, cdim, sz in zip(train_x, self.compressed_dim, self.kernel_size)
            ]
        )
        ...
        if self.params.update_projection_matrix:
            ...
            if isinstance(self.params.init_CG_iter, (list, tuple)):
                ...
                self.joint_optimizer.run(
                    self.params.init_CG_iter // self.params.init_GN_iter, self.params.init_GN_iter
                )
        ...
        if flag != "not_found":
            if self.use_iou_net:
                ...
                self.refine_target_box(
                    sample_pos, sample_scales[scale_ind], scale_ind, update_scale_flag
                )
    ...
    def localize_target(self, scores_raw):
        ...
        if self.output_window is not None and not self.params.get(
        ...
        translation_vec = (
            disp[scale_ind, ...].view(-1)
            * (self.img_support_sz / self.output_sz)
            * self.target_scale
        )
    ...
    def localize_advanced(self, scores):
        ...
        if self.output_window is not None and self.params.get(
            ...
            scores_orig = torch.cat(
                [scores_orig[..., (sz[0] + 1) // 2 :, :], scores_orig[..., : (sz[0] + 1) // 2, :]],
                -2,
            )
            scores_orig = torch.cat(
                [scores_orig[..., :, (sz[1] + 1) // 2 :], scores_orig[..., :, : (sz[1] + 1) // 2]],
                -1,
            )
        ...
        # Shift scores back
        scores = torch.cat(
            [scores[..., (sz[0] + 1) // 2 :, :], scores[..., : (sz[0] + 1) // 2, :]], -2
        )
        scores = torch.cat(
            [scores[..., :, (sz[1] + 1) // 2 :], scores[..., :, : (sz[1] + 1) // 2]], -1
        )
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        if random_shift_factor > 0:
            get_rand_shift = lambda: (
                ((torch.rand(2) - 0.5) * self.img_sample_sz * random_shift_factor).long().tolist()
            )
                ...
                if use_aug:
                    init_samples[i] = torch.cat(
                        [
                            init_samples[i],
                            F.dropout2d(
                                init_samples[i][0:1, ...].expand(num, -1, -1, -1),
                                p=prob,
                                training=True,
                            ),
                        ]
            ...
            if proj_init_method == "pca":
                ...
                self.projection_matrix = TensorList(
                    [
                        None
                        if cdim is None
                        else torch.svd(C)[0][:, :cdim].t().unsqueeze(-1).unsqueeze(-1).clone()
                ...
                self.projection_matrix = TensorList(
                    [
                        None
                        if cdim is None
                        else ex.new_zeros(cdim, ex.shape[1], 1, 1).normal_(
    ...
    def init_label_function(self, train_x):
        ...
        self.y = TensorList(
            [
                x.new_zeros(self.params.sample_memory_size, 1, x.shape[2], x.shape[3])
                for x in train_x
            ]
        )
        ...
        for y, sig, sz, ksz, x in zip(
            self.y, self.sigma, self.feature_sz, self.kernel_size, train_x
        ):
    ...
    def init_memory(self, train_x):
        ...
        for sw, init_sw, num in zip(
            self.sample_weights, self.init_sample_weights, self.num_init_samples
        ):
            sw[:num] = init_sw
    ...
    def init_iou_net(self):
        ...
        self.iou_target_box = self.get_iounet_box(
            self.pos, self.target_sz, self.pos.round(), self.target_scale
        )
        ...
        if self.params.iounet_augmentation:
            for T in self.transforms:
                if not isinstance(
                    ...
                    break
                target_boxes.append(
                    self.iou_target_box + torch.Tensor([T.shift[1], T.shift[0], 0, 0])
                )
        else:
        ...
        if self.params.get("iounet_not_use_reference", False):
            self.target_feat = TensorList(
                [torch.full_like(tf, tf.norm() / tf.numel()) for tf in self.target_feat]
            )
    ...
    def optimize_boxes(self, iou_features, init_boxes):
        ...
        if box_refinement_space == "default":
            ...
            for i_ in range(self.params.box_refinement_iter):
                ...
                # Update proposal
                step = (
                    update_mask_float * step_length * bb_init_rel.grad
                    - (1.0 - update_mask_float) * step
                )
