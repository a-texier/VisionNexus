...
class KYS(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        scores_fused, motion_feat, new_state_vector = self.get_response_prediction(
            backbone_feat, scores_dimp
        )

        # Localize the target
        translation_vec, scale_ind, s, flag = self.localize_target(
            scores_fused, scores_dimp, sample_scales
        )
        ...
        # Update motion state
        if flag != "not_found":
            box_patch = self.get_iounet_box(
                self.pos, self.target_sz, sample_pos[0, :], sample_scales[0]
            )

            prev_label = self.get_label_function(
                sample_pos[scale_ind, :], sample_scales[scale_ind]
            ).to(self.params.device)[0]
    ...
    def init_motion_module(self, im):
        ...
        current_bb = torch.cat(
            (self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]])
        )

        self.prev_state_handler.set_data(
            0, motion_feat, None, test_patch, current_bb, prev_label, box_patch
        )
    ...
    def get_response_prediction(self, backbone_feat, scores_dimp):
        ...
        prev_motion_feat, prev_motion_state_vector, prev_test_patch, prev_label, box_patch = (
        ...
        if (
            ...
            and not (
                (box_c < box_c_max.to(box_c.device)) * (box_c > box_c_min.to(box_c.device))
            ).all()
        ):
            # In case the target was not near the center in the prev. frame, shift the feature map so that the target
            # is in the center.
            prev_motion_feat = CenterShiftFeatures(feature_stride=16)(
                prev_motion_feat.clone(), box_patch
            )
            prev_motion_state_vector = CenterShiftFeatures(feature_stride=16)(
                prev_motion_state_vector.clone(), box_patch
            )
        elif (
            self.params.get("prev_feat_remove_subpixel_shift", False)
            and prev_motion_state_vector is not None
        ):
            box_c_feat = box_c / 16.0
            box_c_feat_round = box_c_feat.round() + 0.5 * self.net.predictor.fix_coordinate_shift
            feat_trans = (box_c_feat_round - box_c_feat).view(-1, 2) / self.output_sz.view(
                -1, 2
            ).to(box_c_feat.device)

            prev_motion_feat = shift_features(prev_motion_feat.clone(), feat_trans)

            if prev_motion_state_vector is not None:
                prev_motion_state_vector = shift_features(
                    prev_motion_state_vector.clone(), feat_trans
                )
            else:
        ...
        with torch.no_grad():
            resp_output = self.net.predictor.predict_response(
                predictor_input,
                dimp_thresh=self.params.get("dimp_threshold", None),
    ...
    def localize_target(self, score_fused, score_dimp, sample_scales):
        ...
        translation_vec, scale_ind, scores, max_dimp_score, flag, max_disp1 = (
            self.compute_target_location(score_fused, loc_params, sample_scales, score_dimp_win)
        )
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        if self.params.get("window_output", False):
            ...
            if self.params.get("use_clipped_window", False):
                ...
                self.output_window = dcf.hann2d(score_map_sz.long(), centered=True).to(
                    self.params.device
                )
            ...
            if self.params.get("windom_clamp_factor", None) is not None:
                self.output_window = (
                    self.output_window * (1.0 / self.params.get("windom_clamp_factor"))
                ).clamp(0.0, 1.0)
    ...
    def refine_target_box(
        ...
        if update_scale:
            if self.params.has("target_scale_update_rate"):
                self.target_scale = (
                    new_scale * self.params.target_scale_update_rate
