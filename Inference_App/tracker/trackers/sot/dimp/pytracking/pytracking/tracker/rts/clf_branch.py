...
class ClassifierBranch:
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        if random_shift_factor > 0:
            get_rand_shift = lambda: (
                (
                    (torch.rand(2) - 0.5) * self.clf_img_sample_sz * random_shift_factor
                    + global_shift
                )
                .long()
                .tolist()
        ...
        augs = (
            self.params.clf_augmentation if self.params.get("clf_use_augmentation", False) else {}
        )
        ...
        if "relativeshift" in augs:
            get_absolute = lambda shift: (
                (torch.Tensor(shift) * self.clf_img_sample_sz / 2).long().tolist()
            )
            self.clf_transforms.extend(
                [
        ...
        im_patches = sample_patch_transformed(
            im,
            self.clf_init_sample_pos,
            self.clf_init_sample_scale,
            aug_expansion_sz,
            self.clf_transforms,
        )
    ...
    def update_state(
        ...
        translation_vec, scale_ind, s, flag = self.localize_target(
            clf_scores, clf_sample_pos, clf_sample_scale
        )

        # Update position and scale
        if (
            not is_lost_seg
            and not is_seg_too_small
            and self.params.get("use_seg_pos_sz_for_clf_update", True)
        ):
            ...
            if new_scale is not None:
                self.clf_target_scale = new_scale.clamp(
                    self.clf_min_scale_factor, self.clf_max_scale_factor
                )
            ...
            self.clf_pos = torch.max(
                torch.min(new_pos, self.clf_image_sz - inside_offset), inside_offset
            )

        clf_pos, clf_target_scale, clf_target_sz = (
            self.clf_pos,
            self.clf_target_scale,
            self.clf_target_sz,
        )
        ...
        clf_new_state = torch.cat(
            (
                self.clf_pos[[1, 0]] - (self.clf_target_sz[[1, 0]] - 1) / 2,
                self.clf_target_sz[[1, 0]],
            )
        )
    ...
    def update_classifier_model(self, track_test_x, sample_coords, scale_ind, s, flag):
        ...
        learning_rate = (
            self.params.get("clf_hard_negative_learning_rate", None) if hard_negative else None
        )
    ...
    def get_classifier_sample_location(self, sample_coord):
        ...
        sample_scales = (
            ((sample_coord[:, 2:] - sample_coord[:, :2]) / self.clf_img_sample_sz)
            .prod(dim=1)
            .sqrt()
        )
    ...
    def init_classifier_memory(self, train_x: TensorList):
        ...
        self.clf_sample_weights = TensorList(
            [x.new_zeros(self.params.clf_sample_memory_size) for x in train_x]
        )
        for sw, init_sw, num in zip(
            self.clf_sample_weights, init_sample_weights, self.clf_num_init_samples
        ):
            sw[:num] = init_sw
        ...
        self.clf_training_samples = TensorList(
            [
                x.new_zeros(self.params.clf_sample_memory_size, x.shape[1], x.shape[2], x.shape[3])
                for x in train_x
            ]
        )
    ...
    def update_classifier_memory(
        self, sample_x: TensorList, sample_y: TensorList, target_box, learning_rate=None
    ):
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        if (
            self.params.get("clf_use_augmentation", False)
            and "dropout" in self.params.clf_augmentation
        ):
    ...
    def update_classifier(self, train_x, train_y, target_box, learning_rate=None, scores=None):
        ...
        if (
            hard_negative_flag
            or self.frame_num % self.params.get("clf_train_sample_interval", 1) == 0
        ):
        ...
        sample_weights = (
            self.clf_sample_weights[0][: self.clf_num_stored_samples[0]].view(-1, 1, 1, 1).clone()
        )
    ...
    def localize_target(self, scores, sample_pos, sample_scales):
        ...
        translation_vec = (
            target_disp * (self.clf_img_support_sz / output_sz) * sample_scales[scale_ind]
        )
    ...
    def init_target_labels(self, train_x: TensorList):
        ...
        for target, x in zip(self.clf_target_labels, train_x):
            ksz_even = torch.Tensor(
                [(self.clf_kernel_size[0] + 1) % 2, (self.clf_kernel_size[1] + 1) % 2]
            )
            center_pos = self.clf_feature_sz * target_center_norm + 0.5 * ksz_even
            for i, T in enumerate(self.clf_transforms[: x.shape[0]]):
                sample_center = (
                    center_pos
                    + torch.Tensor(T.shift) / self.clf_img_support_sz * self.clf_feature_sz
                )
