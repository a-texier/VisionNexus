...
def remap_object_ids(init_info):
    ...
    init_info_new = {}
    loc_to_glob_map = dict(
        zip([str(i) for i in range(1, len(init_info["object_ids"]) + 1)], init_info["object_ids"])
    )
    glob_to_loc_map = dict(
        zip(init_info["object_ids"], [str(i) for i in range(1, len(init_info["object_ids"]) + 1)])
    )
    ...
    init_info_new["init_bbox"] = OrderedDict(
        {glob_to_loc_map[i]: val for i, val in init_info["init_bbox"].items()}
    )
...
class TaMOs(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        if "init_object_ids" in info:
            ...
            self.pos = OrderedDict(
                {
                    int(oid): torch.Tensor(
                        [
                            states[oid][1] + (states[oid][3] - 1) / 2,
                            states[oid][0] + (states[oid][2] - 1) / 2,
                        ]
                    )
                    for oid in info["init_object_ids"]
                }
            )
            self.target_sz = OrderedDict(
                {
                    int(oid): torch.Tensor([states[oid][3], states[oid][2]])
                    for oid in info["init_object_ids"]
                }
            )
            self.target_bbox = OrderedDict(
                {int(oid): torch.Tensor(states[oid]) for oid in info["init_object_ids"]}
            )
            self.pos_prev = OrderedDict(
                {int(oid): state.clone() for oid, state in self.pos.items()}
            )
            ...
            self.pos = OrderedDict(
                {1: torch.Tensor([state[1] + (state[3] - 1) / 2, state[0] + (state[2] - 1) / 2])}
            )
    ...
    def encode_bbox(self, bboxes):
        ...
        shifts_x = torch.arange(
            0, output_sz[1], step=stride, dtype=torch.float32, device=bbox.device
        )
        shifts_y = torch.arange(
            0, output_sz[0], step=stride, dtype=torch.float32, device=bbox.device
        )
        ...
        xyxy = torch.stack(
            [bbox[:, 0], bbox[:, 1], bbox[:, 0] + bbox[:, 2], bbox[:, 1] + bbox[:, 3]], dim=1
        )
        ...
        s = (
            torch.tensor([output_sz[1], output_sz[0], output_sz[1], output_sz[0]])
            .reshape(1, 4)
            .to(bbox.device)
        )
        ...
        nb = bbox.shape[0]
        reg_targets_per_im = reg_targets_per_im.reshape(
            output_sz[0] // stride, output_sz[1] // stride, nb, 4
        ).permute(2, 3, 0, 1)

        reg_targets_per_im_all = torch.zeros(
            bboxes.shape[0],
            reg_targets_per_im.shape[1],
            reg_targets_per_im.shape[2],
            reg_targets_per_im.shape[3],
            device=bbox.device,
        )
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        for oid in self.object_ids:
            scale_ind, s, flag, score_loc = self.localize_target(
                scores_raw[:, :, oid - 1], self.scale_factor, oid
            )

            bbox_raw = self.direct_bbox_regression(
                bbox_preds[:, :, oid - 1],
                sample_coords,
                score_loc,
                scores_raw[:, :, oid - 1],
            ...
            learning_rate = (
                self.params.get("hard_negative_learning_rate", None) if hard_negative else None
            )

            if (
                update_flag
                and self.params.get("update_classifier", False)
                and scores_raw[:, :, oid - 1].max() > self.conf_ths
            ):
                # Get train sample
                train_x = (
                    test_x["layer3"][scale_ind : scale_ind + 1, ...]
                    if "layer3" in test_x
                    else test_x["2"][scale_ind : scale_ind + 1, ...]
                )

                # Create target_box and label for spatial sample
                target_boxes[oid] = self.get_iounet_box(
                    self.pos[oid], self.target_sz[oid], self.scale_factor
                )
                train_ys[oid] = self.get_label_function(self.pos[oid], self.scale_factor).to(
                    self.params.device
                )
            ...
            new_state = torch.cat(
                (
                    self.pos[oid][[1, 0]] - (self.target_sz[oid][[1, 0]] - 1) / 2,
                    self.target_sz[oid][[1, 0]],
                )
            )
        ...
        out = {"target_bbox": output_states, "object_presence_score": object_presence_scores}

        if self.visdom is not None:
            self.visualize_raw_results(scores_raw, output_states, object_presence_scores)

        out["target_bbox"] = OrderedDict(
            {self.id_map[id]: val for id, val in out["target_bbox"].items()}
        )
        out["object_presence_score"] = OrderedDict(
            {self.id_map[id]: val for id, val in out["object_presence_score"].items()}
        )
    ...
    def direct_bbox_regression(self, bbox_preds, sample_coords, score_loc, scores_raw, stride=16):
        shifts_x = torch.arange(0, self.img_sample_sz[1], step=stride, dtype=torch.float32)
        shifts_y = torch.arange(0, self.img_sample_sz[0], step=stride, dtype=torch.float32)
        shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x)
        shift_x = shift_x.reshape(-1)
        shift_y = shift_y.reshape(-1)
        locations = torch.stack((shift_x, shift_y), dim=1) + stride // 2
        ...
        ltrb = (
            bbox_preds.permute(0, 1, 3, 4, 2)[0, 0].cpu() * self.train_img_sample_sz[[1, 0, 1, 0]]
        )
        xs1 = xs - ltrb[:, :, 0]
        xs2 = xs + ltrb[:, :, 2]
        ys1 = ys - ltrb[:, :, 1]
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            ...
            train_feat = self.net.head.extract_head_feat(
                {"layer3": train_samples, "2": train_samples}
            )

            train_ltrb = torch.cat(
                [self.encode_bbox(box).unsqueeze(0) for box in target_boxes], dim=0
            )
            train_bb = [
                {
                    i: target_boxes[k, i].unsqueeze(1)
                    for i in range(target_boxes.shape[1])
                    if torch.all(target_boxes[k, i, 2:] > 0)
                }
            ...
            if self.params.get("cls_feature_type", "trafo") == "trafo":
                ...
                target_scores = self.net.head.classifier(
                    test_feat_enc_fpn_cls["feat2"], cls_weights
                )
            ...
            if (
                target_scores.shape[-1] != bbox_preds.shape[-1]
                and target_scores.shape[-2] != bbox_preds.shape[-2]
            ):
                target_scores = F.interpolate(
                    target_scores[0], bbox_preds.shape[-2:], mode="bicubic"
                ).unsqueeze(0)
    ...
    def localize_target(self, scores, scale_factor, obj_id):
        ...
        if score_filter_ksz > 1:
            ...
            scores = F.conv2d(
                scores.view(-1, 1, *scores.shape[-2:]), kernel, padding=score_filter_ksz // 2
            ).view(scores.shape)
    ...
    def construct_hn_window(self, size, pos):
        ...
        hn1 = (
            0.5
            * (1 - torch.cos(2 * math.pi * (x1) / size[0]))
            * ((0 < x1) & (x1 < size[0])).float()
        )
        hn2 = (
            0.5
            * (1 - torch.cos(2 * math.pi * (x2) / size[1]))
            * ((0 < x2) & (x2 < size[1])).float()
        )
    ...
    def localize_advanced(self, scores, scale_factor, obj_id):
        ...
        target_neigh_sz = (
            self.target_sz[obj_id]
            / ((self.img_support_sz / output_sz) / scale_factor)
            * self.params.target_neighborhood_scale
        )
        ...
        prev_target_vec = (self.pos[obj_id] - self.pos_prev[obj_id]) / (
            (self.img_support_sz / output_sz) / scale_factor
        )
        ...
        patch_coords = torch.Tensor(
            [
                [
                    0,
                    0,
                    self.img_sample_sz[0] / self.scale_factor,
                    self.img_sample_sz[1] / self.scale_factor,
                ]
            ]
        )
    ...
    def init_target_boxes(self):
        ...
        self.classifier_target_box = OrderedDict(
            {
                obj_id: self.get_iounet_box(
                    self.pos[obj_id], self.target_sz[obj_id], self.scale_factor
                )
        ...
        self.target_boxes = init_target_boxes.new_zeros(
            self.params.sample_memory_size, self.max_num_objs, 4
        )
    ...
    def init_target_labels(self, train_x: TensorList):
        ...
        self.sigma = (
            ...
            * self.feature_sz.prod().sqrt().item()
            * torch.ones(2)
        )
        ...
        for label, x in zip(self.target_labels, train_x):
            for obj_id in self.pos.keys():
                target_center_norm = (
                    self.pos[obj_id] * self.scale_factor - self.img_support_sz / 2
                ) / self.img_support_sz
                ksz_even = torch.Tensor(
                    [(self.kernel_size[0] + 1) % 2, (self.kernel_size[1] + 1) % 2]
                )
                center_pos = self.feature_sz * target_center_norm + 0.5 * ksz_even
                label[0, obj_id - 1, ...] = dcf.label_function_spatial(
                    self.feature_sz, self.sigma, center_pos, end_pad=ksz_even
                )
    ...
    def init_memory(self, train_x: TensorList):
        ...
        for sw, init_sw, num in zip(
            self.sample_weights, init_sample_weights, self.num_init_samples
        ):
            sw[:num] = init_sw
        ...
        self.training_samples = TensorList(
            [
                x.new_zeros(self.params.sample_memory_size, x.shape[1], x.shape[2], x.shape[3])
                for x in train_x
            ]
        )
    ...
    def update_memory(
        self, sample_x: TensorList, sample_ys_dict, target_boxes_dict, learning_rate=None
    ):
        sample_y = TensorList(
            [
                torch.zeros(
                    1,
                    self.target_labels[0].shape[1],
                    self.target_labels[0].shape[2],
                    self.target_labels[0].shape[3],
                    device=sample_x[0].device,
                )
            ]
        )
        target_box = torch.zeros(
            self.target_boxes.shape[1], self.target_boxes.shape[2], device=sample_x[0].device
        )
