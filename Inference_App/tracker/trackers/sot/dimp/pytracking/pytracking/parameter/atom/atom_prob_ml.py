...
def parameters():
    ...
    params.CG_iter = (
        5  # The number of Conjugate Gradient iterations in each update after the first frame
    )
    params.init_CG_iter = (
        60  # The total number of Conjugate Gradient iterations used in the first frame
    )
    ...
    params.projection_activation = (
        "none"  # Activation function after projection ('none', 'relu', 'elu' or 'mlu')
    )
    ...
    params.hard_negative_CG_iter = (
        5  # Number of optimization iterations to use if hard negative detected
    )
    ...
    params.num_init_random_boxes = (
        9  # Num extra random boxes in addition to the classifier prediction
    )
    ...
    params.box_refinement_step_length = (
        2e-4,
        10e-4,
    )  # 1   # Gradient step length in the bounding box refinement
