...
# network related
from lib.models.stark import build_stark_lightning_x_trt, build_starks, build_starkst
...
def run(settings):
    ...
    config_module_teacher = importlib.import_module(
        "lib.config.%s.config" % settings.script_teacher
    )
    ...
    if settings.local_rank != -1:
        ...
        net_teacher = DDP(
            net_teacher, device_ids=[settings.local_rank], find_unused_parameters=True
        )
    ...
    if settings.script_name == "stark_lightning_X_trt":
        ...
        actor = STARKLightningXtrtdistillActor(
            net=net,
            objective=objective,
