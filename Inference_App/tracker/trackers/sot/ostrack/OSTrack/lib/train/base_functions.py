    ...
    for name in name_list:
        ...
        if name == "LASOT":
            if settings.use_lmdb:
                ...
                datasets.append(
                    Lasot_lmdb(
                        settings.env.lasot_lmdb_dir, split="train", image_loader=image_loader
                    )
                )
            else:
                datasets.append(
                    Lasot(settings.env.lasot_dir, split="train", image_loader=image_loader)
                )
        if name == "GOT10K_vottrain":
            if settings.use_lmdb:
                ...
                datasets.append(
                    Got10k_lmdb(
                        settings.env.got10k_lmdb_dir, split="vottrain", image_loader=image_loader
                    )
                )
            else:
                datasets.append(
                    Got10k(settings.env.got10k_dir, split="vottrain", image_loader=image_loader)
                )
        if name == "GOT10K_train_full":
            if settings.use_lmdb:
                ...
                datasets.append(
                    Got10k_lmdb(
                        settings.env.got10k_lmdb_dir, split="train_full", image_loader=image_loader
                    )
                )
            else:
                datasets.append(
                    Got10k(settings.env.got10k_dir, split="train_full", image_loader=image_loader)
                )
        if name == "GOT10K_votval":
            if settings.use_lmdb:
                ...
                datasets.append(
                    Got10k_lmdb(
                        settings.env.got10k_lmdb_dir, split="votval", image_loader=image_loader
                    )
                )
            else:
                datasets.append(
                    Got10k(settings.env.got10k_dir, split="votval", image_loader=image_loader)
                )
        if name == "GOT10K_official_val":
            if settings.use_lmdb:
                ...
                datasets.append(
                    Got10k(settings.env.got10k_val_dir, split=None, image_loader=image_loader)
                )
        if name == "COCO17":
            if settings.use_lmdb:
                ...
                datasets.append(
                    MSCOCOSeq_lmdb(
                        settings.env.coco_lmdb_dir, version="2017", image_loader=image_loader
                    )
                )
            else:
                datasets.append(
                    MSCOCOSeq(settings.env.coco_dir, version="2017", image_loader=image_loader)
                )
        if name == "VID":
            if settings.use_lmdb:
                print("Building VID from lmdb")
                datasets.append(
                    ImagenetVID_lmdb(settings.env.imagenet_lmdb_dir, image_loader=image_loader)
                )
            else:
        ...
        if name == "TRACKINGNET":
            if settings.use_lmdb:
                ...
                datasets.append(
                    TrackingNet_lmdb(settings.env.trackingnet_lmdb_dir, image_loader=image_loader)
                )
            else:
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        param_dicts = [
            ...
            {
                "params": [
                    p for n, p in net.named_parameters() if "backbone" in n and p.requires_grad
                ],
    ...
    if cfg.TRAIN.SCHEDULER.TYPE == "step":
        ...
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=cfg.TRAIN.SCHEDULER.MILESTONES, gamma=cfg.TRAIN.SCHEDULER.GAMMA
        )
    else:
