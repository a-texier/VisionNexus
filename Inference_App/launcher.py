#!/usr/bin/env python3
"""
launcher.py -- Inference_App (app UNIFIEE : tracker + inference + evaluation + acquisition)
==========================================================================================
IHM web autour d'un tracker generique MOT/SOT. Fonctionne en STANDALONE et comme noeud
de l'Orchestrator. Onglets : Tracker, Evaluation (mAP/MOTA/IDF1), Acquisition (flux -> dataset).

Usage :
    python launcher.py --user bob --workspace C:/ws
    python launcher.py --user bob --workspace C:/ws --backend-only --no-reload

Workspace final : <workspace>/inference_<user>/  (runs, exports, sequences, cmd_send, acquisitions)
"""
...
def main():
    parser = argparse.ArgumentParser(description="Launch Inference_App")
    ...
    try:
        if args.backend_port and args.frontend_port:
            ...
            claimed = {e.get("backend_port") for e in inst} | {e.get("frontend_port") for e in inst}

            def _free(start):
                for p in range(start, start + 200):
                    if p not in claimed and is_port_free(p):
                        return p
    ...
    env = {**os.environ,
           "INFERENCE_APP_WORKSPACE": ws,
           "INFERENCE_APP_USER": args.user,
           "INFERENCE_APP_FRONTEND_PORT": str(fp),
           "BACKEND_PORT": str(bp),
    ...
    print("=" * 60)
    print(f"  Inference_App -- {args.user}")
    print("=" * 60)
