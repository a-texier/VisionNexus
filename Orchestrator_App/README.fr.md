...
- **FREE** (aucune entrée) -> ouvre Inference App pour choisir un fichier média,
  un fichier de poids et un mode interactif.
- **LOCKED** (entrée) -> évaluation détection ou inférence multi-objet. Le tracker
  est désactivé pour une inférence YOLO pure, ou réglé sur ByteTrack.
  `model_path` vide -> poids du Training amont.
...
- A la fin (et progressivement pendant) chaque run, l'app collecte tout ce que savent les
  ...
  DVC) et genere un rapport : courbes interactives, images d'analyse du moteur d'entrainement
  (matrice de confusion, courbes PR/F1, synthese si le moteur en produit une), journal complet des
  etapes.
