...
function ModelConfig({ node, onUpdate }: { node: Node; onUpdate: Props['onUpdate'] }) {
  ...
  return (
    <>
      ...
      <Field label={`Chemin du modèle (${suffixes.join(', ') || '…'})`}>
        <PathInput value={path} onChange={v => onUpdate(node.id, { model_path: v })} placeholder={`C:/.../best${suffixes[0] ?? ''}`} />
      </Field>
      {badSuffix && (
        <p className="text-[10px] text-amber-400">Extension inattendue pour {eng.labelOf(engine)} ({suffixes.join(', ')}) : ces poids ne se chargeront pas.</p>
      )}
      {catalog && (
        <Field label="Taille (fige le Training)">
          <Select value={size} onChange={v => onUpdate(node.id, { model_size: v })} options={catalog.sizes.map(s => [s, shortSize(catalog, s)])} />
        </Field>
      )}
      <Out>Nœud d'<b>entrée</b> : branché sur un <b>Training</b> (poids de départ / fine-tuning — le moteur <b>{eng.labelOf(engine)}</b> et la taille <b>{size || '—'}</b> figent ceux du training) ou une <b>Inference/Éval</b> (modèle à tester).</Out>
    </>
  )
}
...
function EngineSelect({ eng, value, onChange, locked, lockedBy }:
  ...
  return (
    <>
      {eng.usable.length > 1 && (
        <Field label={locked ? `Moteur (figé par ${lockedBy ?? 'le nœud branché'})` : "Moteur d'entraînement"}>
          <Select value={value} onChange={onChange} locked={locked}
            options={eng.usable.map(e => [e.name, e.label])} />
        </Field>
      )}
      {eng.unavailable.map(e => (
        <p key={e.name} className="text-[10px] text-amber-500/80">{e.label} indisponible : {e.reason}</p>
      ))}
    </>
  )
}
...
function TrainingConfig({ node, onUpdate, appUrls }: { node: Node; onUpdate: Props['onUpdate']; appUrls: Record<string, string> }) {
  ...
  return (
    <>
      ...
      <EngineSelect eng={eng} value={engine} locked={verLocked} lockedBy="le modèle"
        onChange={v => onUpdate(node.id, { engine: v, model_size: eng.catalogOf(v)?.default_size ?? '', hyperparams: {} })} />
      {optunaEngine && optunaEngine !== engine && (
        <p className="text-[10px] text-red-400">L'étude Optuna amont optimise {eng.labelOf(optunaEngine)} : choisissez le même moteur, sinon le lancement sera refusé.</p>
      )}
      <Field label={verLocked ? `Taille ${catalog?.label ?? ''} (figée par le modèle)` : `Taille ${catalog?.label ?? ''}`}>
      ...
      {fullAuto ? (
        <>
          <button onClick={() => setShowHP(h => !h)} className="flex items-center gap-1 text-[11px] text-gray-400 hover:text-gray-200 w-full">
            <ChevronDown size={11} className={showHP ? 'rotate-180' : ''} /> Tous les hyperparamètres ({hpFields.length}){hpoLinked ? ' — figés (Optuna)' : ''}
          </button>
          {showHP && catalog && (
            <div className="space-y-1.5 pl-1 border-l border-gray-800">
              <p className="text-[10px] text-gray-600">Rien de magique : ces valeurs sont exactement ce qui est envoyé au moteur {catalog.label} (Training_App).</p>
          ...
          <Out>Sortie : <b>poids {catalog?.weights_suffixes.join(', ') ?? ''}</b> du moteur {eng.labelOf(engine)} (dans le run « {(d.run_label as string) || modelSize} ») → Inference/Éval, DVC.</Out>
        </>
      ) : (
...
function InferenceConfig({ node, onUpdate, appUrls }: { node: Node; onUpdate: Props['onUpdate']; appUrls: Record<string, string> }) {
    ...
    return (
      <>
        <div className="bg-emerald-950/30 border border-emerald-800/40 rounded-lg px-2.5 py-2 text-[11px] text-emerald-300/80">
          Mode <b>FREE</b> : ouvrez Inference App, choisissez un fichier image/vidéo, un fichier de poids et le mode YOLO, MOT ou SOT.
        </div>
  ...
  return (
    <>
      ...
      {!fullAuto ? (
        <div className="bg-amber-950/30 border border-amber-800/30 rounded-lg px-2.5 py-2 text-[11px] text-amber-300/80 leading-relaxed">
          Mode <b>Manuel</b> (interactif) : ouvrez l'Inference App, lancez l'inférence YOLO/MOT ou cliquez une cible pour le
          <b> SOT CSRT</b>. Aucune option ici — tout se
        ...
        <>
          ...
          <EngineSelect eng={eng} value={engine} locked={modelLocked} lockedBy="le modèle amont"
            onChange={v => onUpdate(node.id, { engine: v, model_size: eng.catalogOf(v)?.default_size ?? '' })} />
          {catalog && (
            <Field label={modelLocked ? `Architecture ${catalog.label} (figée par l'amont)` : `Architecture ${catalog.label}`}>
          ...
          {upstreamEngine && configuredEngine !== upstreamEngine && (
            <p className="text-[10px] text-red-400">Pré-contrôle bloquant : ce nœud est configuré en {eng.labelOf(configuredEngine)}, mais le modèle amont est en {eng.labelOf(upstreamEngine)}.</p>
          )}
          {upstreamSize && configuredSize && configuredSize !== upstreamSize && (
            <p className="text-[10px] text-red-400">Pré-contrôle bloquant : ce nœud déclare {configuredSize}, mais le checkpoint amont est en {upstreamSize}.</p>
          )}
          ...
          {task === 'tracking' && (
            <>
              ...
              <Field label="Tracker multi-objet"><Select value={(d.tracker_mot as string) ?? 'bytetrack'} onChange={v => onUpdate(node.id, { tracker_mot: v })}
                options={[['none', 'Aucun — YOLO pur'], ['bytetrack', 'ByteTrack']]} /></Field>
              <div className="grid grid-cols-3 gap-1.5">
              ...
              {(d.tracker_mot ?? 'bytetrack') === 'bytetrack' && (
                <div className="grid grid-cols-2 gap-1.5">
                  <NumField node={node} onUpdate={onUpdate} k="track_high_thresh" label="Track high" def={0.5} />
                  <NumField node={node} onUpdate={onUpdate} k="match_thresh" label="Match IoU" def={0.3} />
                </div>
              )}
              <Out>Sortie : média annoté + FPS global, temps détecteur et temps ByteTrack. Le SOT par clic utilise CSRT dans l'app.</Out>
            </>
          )}
          ...
          {advRender && (
            <div className="space-y-1 pl-1 border-l border-gray-800">
              <Toggle node={node} onUpdate={onUpdate} field="save_output" label="Sauver le média annoté" def />
              <NumField node={node} onUpdate={onUpdate} k="max_frames" label="Frames max (0 = toutes)" def={0} int />
            </div>
          )}
          ...
          {advSys && (
            <div className="space-y-1.5 pl-1 border-l border-gray-800">
              <div className="grid grid-cols-2 gap-1.5">
                <Field label="Device"><Select value={(d.device as string) ?? ''} onChange={v => onUpdate(node.id, { device: v })} options={[['', 'Auto'], ['cuda', 'CUDA'], ['cpu', 'CPU']]} /></Field>
                <NumField node={node} onUpdate={onUpdate} k="track_buffer" label="Buffer ByteTrack" def={30} int />
              </div>
            </div>
          )}
...
function OptunaConfig({ node, onUpdate, appUrls }: { node: Node; onUpdate: Props['onUpdate']; appUrls: Record<string, string> }) {
  ...
  return (
    <>
      ...
      {catalog && (
        <Field label="Taille entraînée par les trials">
          <Select value={size} onChange={v => onUpdate(node.id, { model_size: v })} options={catalog.sizes.map(s => [s, shortSize(catalog, s)])} />
        </Field>
      )}
      <p className="text-[10px] text-gray-600">Sampler <b>TPE</b>, pruning désactivé. Chaque trial entraîne un {catalog?.label ?? 'modèle'} (moteur Training_App) ; le Training aval doit utiliser le même moteur.</p>
              ...
              {planned.map((p, i) => (
                <div key={i} className="bg-gray-800/40 border border-gray-800 rounded-md px-2 py-1.5">
                  ...
                  <p className="text-[10px] text-gray-500">
                    {p.kind === 'training' ? 'params (hyperparamètres, epochs…) + mAP + plots + poids (registry)' : 'métriques mAP / MOTA / IDF1 + benchmark'}
                  </p>
                </div>
              ))}
            </div>
