...
// Rendu markdown -> HTML des pages de docs/ et liens vers la page Guide.
//
...
/** URL de la page Guide sur une page de doc, et optionnellement un titre. */
export function docLink(name: string, headingIdx?: number): string {
  const hash = headingIdx === undefined ? '' : `#h-${headingIdx}`
  return `/mlops/guide?doc=${encodeURIComponent(name)}${hash}`
}
