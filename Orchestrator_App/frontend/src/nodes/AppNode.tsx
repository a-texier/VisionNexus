        ...
        {(d.node_type === 'explorer' || d.node_type === 'annotation' || d.node_type === 'inference') && d.has_input !== undefined && (
          d.has_input === false
            ? <span title={d.node_type === 'inference'
                ? 'Mode FREE : acquisition — capture un flux et produit un dataset d\'images'
...
function InferenceNodeSummary({ nodeId, data: d }: { nodeId: string; data: AppNodeData }) {
  // FREE (has_input === false) = ACQUISITION (produit un dataset) ; LOCKED = eval/inference.
    ...
    return (
      <div className="text-[11px] text-gray-500 mt-0.5 space-y-0.5">
        <span className="inline-block text-[10px] px-1.5 py-0.5 rounded bg-emerald-900/50 text-emerald-300 border border-emerald-700/40">
          Acquisition → dataset d'images
        </span>
        {d.sequence_dir
          ? <p className="truncate text-[10px]">flux : <span className="text-gray-400">{d.sequence_dir as string}</span></p>
          : <p className="text-[10px] text-amber-500/80 italic">source (MJPEG/ZMQ/optional_format/vidéo) à définir</p>}
        <p className="text-[10px] text-gray-600">→ importable dans Dataset Explorer / Annotation</p>
      </div>
    )
  }
  const fullAuto = d.full_auto === undefined ? true : Boolean(d.full_auto)
    ...
    <div className="text-[11px] text-gray-500 mt-0.5 space-y-0.5">
      ...
      {fullAuto ? (
        <>
          ...
          {task === 'tracking' && (d.cmd_file as string)
            && <span className="ml-1 text-[10px] text-amber-400">SOT+MOT</span>}
...
function NodeSummary({ nodeId, data: d, isFreeMode }: { nodeId: string; data: AppNodeData; isFreeMode: boolean }) {
  ...
  if (d.node_type === 'training') {
    ...
    const size = (d.model_size   as string) ?? 'n'
    const ep   = (d.epochs as number) ?? 100
    return (
      <div className="text-[11px] text-gray-500 mt-0.5 space-y-0.5">
        <FullAutoToggle nodeId={nodeId} value={fullAuto} />
        <p>modèle : <span className="text-blue-400 font-mono">{ver}{size}.pt</span></p>
  ...
  return (
    <div className="px-3 pb-3 pt-2 border-t border-gray-800 space-y-1.5">
      ...
      {d.node_type === 'training' && (
        <>
          <Field label="version"  value={(d.yolo_version as string) ?? 'yolov8'} />
          <Field label="taille"   value={(d.model_size   as string) ?? 'n'} />
          <Field label="epochs"   value={String((d.epochs as number) ?? 100)} />
