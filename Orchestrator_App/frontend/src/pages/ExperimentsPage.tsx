...
import {
  ...
  TrendingUp, Unlock, GitBranch, Zap,
} from 'lucide-react'
...
const SANDGRAPH_TEMPLATES: SandgraphTemplate[] = [
  ...
  // ════════════════════════════════════════════════════════════════════════
  {
    ...
    nodes: [
      ...
      { id: 't1',  type: 'training',       position: { x: 900, y: 100 }, data: { node_type: 'training',       label: 'Training YOLOX', run_label: 'yolox-s', model_size: 'yolox-s', epochs: 50, batch: 16, imgsz: 640, full_auto: true } },
  ...
  },
  {
    ...
    nodes: [
      ...
      { id: 't1', type: 'training', position: { x: 840, y: 120 },
        data: { node_type: 'training', label: 'Training YOLOX', run_label: 'yolox-s', model_size: 'yolox-s', epochs: 50, batch: 16, imgsz: 640, full_auto: true } },
  ...
  },
  {
    ...
    category: 'mainstream',
    description: 'La chaîne standard avec optimisation d\'hyperparamètres : Dataset → Dataset Explorer → Annotation → Optuna (étude TPE, entraîne les trials) → best params → Training final → Inference/Éval → DVC. Un seul training « propre » en aval. MLflow supervise (isolé).',
    ...
    nodes: [
      ...
      { id: 'o1', type: 'optuna', position: { x: 840, y: 120 },
        data: { node_type: 'optuna', label: 'Optuna — étude', full_auto: true, n_trials: 20, optimize: ['basic_lr_per_img', 'mosaic_prob', 'degrees'], best_params: '', stop_on_failure: true } },
      { id: 't1', type: 'training', position: { x: 1120, y: 120 },
        data: { node_type: 'training', label: 'Training final (best params)', run_label: 'yolox-s · best HPO', model_size: 'yolox-s', epochs: 60, batch: 16, imgsz: 640, full_auto: true } },
  ...
  },
  {
    ...
    nodes: [
      ...
      { id: 't1', type: 'training',   position: { x: 320, y: 100 }, data: { node_type: 'training',   label: 'Training YOLOX', run_label: 'yolox-s · retrain', model_size: 'yolox-s', epochs: 50, batch: 16, imgsz: 640, full_auto: true } },
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3 hover:border-gray-700 transition-colors">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">
            {nodeCount} {t('nœud')}{nodeCount !== 1 ? 's' : ''} · {edgeCount} {t('connexion')}{edgeCount !== 1 ? 's' : ''}
            {doneCount > 0 && ` · ${doneCount}/${nodeCount} ${t('terminé')}${lang === 'fr' && doneCount !== 1 ? 's' : ''}`}
          </p>
        </div>
      </div>
      ...
      <p className="text-[11px] text-gray-600">
        {t('Modifié')} {formatDistanceToNow(graph.updated_at)}
...
function TemplateCard({ tpl, onUse }: { tpl: SandgraphTemplate; onUse: () => void }) {
  ...
  return (
    <div className="bg-gray-900/60 border border-gray-800 border-dashed rounded-xl p-4 flex flex-col gap-3 hover:border-gray-700 transition-colors">
      ...
      >
        <Copy size={11} /> {t('Utiliser ce template')}
      </button>
    </div>
  )
}
...
export default function ExperimentsPage() {
  ...
  const createFromTemplateMut = useMutation({
    ...
    onSuccess: g => {
      qc.invalidateQueries({ queryKey: ['graphs'] })
      toast.success(t('Template chargé — ouvrez le sandgraph pour le configurer'))
      navigate(`/?graph_id=${encodeURIComponent(g.graph_id)}`)
    },
    onError: () => toast.error(t('Erreur lors de la création depuis template')),
  })

  const dupMut = useMutation({
    mutationFn: (id: string) => graphsAPI.duplicate(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['graphs'] }); toast.success(t('Dupliqué')) },
  })

  const delMut = useMutation({
    mutationFn: (id: string) => graphsAPI.delete(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['graphs'] }); toast.success(t('Supprimé')) },
  })

  const runMut = useMutation({
    mutationFn: async (id: string) => {
      const g = await graphsAPI.get(id)
      if (!g) throw new Error(t('Graphe introuvable'))
  ...
  const resetMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['graphs'] }); toast(t('Exécution réinitialisée')) },
  })
    ...
    <div className="p-6 h-full overflow-y-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-gray-500 ml-2">
          {t('Sandgraphs enregistrés — chaque expérience est un pipeline visuel')}
        </span>
        <div className="flex-1" />
        <button
      ...
      <div>
        ...
        >
          ...
          <h2 className="text-sm font-semibold text-gray-400 group-hover:text-gray-300 transition-colors">{t('Templates prédéfinis')}</h2>
          <span className="text-xs text-gray-600">{t('(lecture seule — utiliser pour créer une expérience)')}</span>
          <span className="text-xs text-gray-600 ml-auto">{showTemplates ? '▲' : '▼'}</span>
        </button>
        {showTemplates && (
          <div className="space-y-5">
            {([
              { cat: 'mainstream' as const, title: t('Scénarios mainstream'), hint: t('les chaînes recommandées au quotidien') },
              { cat: 'usecase' as const,    title: t('Scénarios exemple / use case'), hint: t('briques isolées et cas particuliers') },
            ]).map(section => (
              <div key={section.cat}>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-gray-600">{t('Mes expériences')}</span>
        <div className="flex-1 h-px bg-gray-800" />
      </div>
      ...
      <div className="flex gap-4 text-xs text-gray-500">
        <div className="flex items-center gap-1.5">
          ...
          <span><span className="text-white">{t('Expériences')}</span> = {t('sandgraphs nommés, état persisté, dupliable')}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <Clock size={12} className="text-gray-400" />
          <span><span className="text-white">{t('Activité')}</span> = {t('journal brut de chaque exécution (append-only)')}</span>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 gap-3 text-center">
          ...
          <div>
            <p className="text-white font-medium">{t('Aucune expérience')}</p>
            <p className="text-gray-500 text-sm mt-1">{t('Utilisez un template ci-dessus ou créez votre premier sandgraph')}</p>
          </div>
          <button
            onClick={() => createMut.mutate()}
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg transition-colors"
          >
            {t('Créer une expérience vide')}
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {graphs.map(g => (
            <GraphCard
              ...
              onDelete={() => { if (confirm(`${t('Supprimer')} "${g.name}" ?`)) delMut.mutate(g.graph_id) }}
