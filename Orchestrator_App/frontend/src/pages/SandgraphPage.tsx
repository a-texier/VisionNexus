...
const TOOLBOX_NODES = [
  ...
  { type: 'training',   category: 'app', label: 'Training',       icon: Zap,      color: 'text-blue-400 border-blue-700/50 hover:bg-blue-900/20',     defaults: { node_type: 'training', label: 'Training YOLO', yolo_version: 'yolov8', model_size: 'n', epochs: 100, batch: 16, imgsz: 640 } },
  { type: 'inference',  category: 'app', label: 'Inference / Eval', icon: Crosshair,color: 'text-cyan-400 border-cyan-700/50 hover:bg-cyan-900/20',   defaults: { node_type: 'inference', label: 'Inference / Eval', full_auto: true, task: 'tracking', tracker_mot: 'botsort', tracker_sot: 'tracking_tophat', n_targets: 1, cmd_file: '', sequence_dir: '', model_path: '', data_yaml: '', annotation_file: '', compute_metrics: false, light_render: true, save_video: true, frame_ext: 'png' } },
  ...
  { type: 'optuna',     category: 'app', label: 'Optuna HPO',     icon: Settings2,color: 'text-cyan-400 border-cyan-700/50 hover:bg-cyan-900/20',     defaults: { node_type: 'optuna', label: 'Optuna HPO', full_auto: true, n_trials: 20, optimize: ['lr0', 'mosaic', 'scale'], best_params: '', stop_on_failure: true } },
  ...
  { type: 'model',      category: 'input', label: 'Modèle YOLO',    icon: Box,      color: 'text-blue-400 border-blue-700/50 hover:bg-blue-900/20',     defaults: { node_type: 'model', label: 'Modèle YOLO', model_path: '', yolo_version: 'yolov8', model_size: 'n' } },
]
...
const STEP_ACTION_LABEL: Record<string, string> = {
  ...
  acquire: 'Acquisition (capture)…', track: 'En attente : session interactive',
}
...
const SUBSTEP_LABEL: Record<string, string> = {
  ...
  acquire: 'Acquisition', track: 'Session interactive',
}
...
function stepSettings(action: string, d: Record<string, unknown>): string {
  ...
  if (action === 'train') {
    const parts = [`${g('yolo_version', 'yolov8')}${g('model_size', 'n')}`,
      `${g('epochs', 100)} epochs`, `batch ${g('batch', 16)}`, `imgsz ${g('imgsz', 640)}`]
    if (d.lr0 != null) parts.push(`lr0=${d.lr0}`)
    return parts.join(' · ')
  }
  if (action === 'hpo') {
    const opt = (d.optimize as string[] | undefined) ?? []
    return `${g('n_trials', 20)} trials · TPE+pruning${opt.length ? ' · ' + opt.join(',') : ''}`
  }
  ...
  for (let pass = 0; pass < 2; pass++) {
    for (const edge of currentEdges) {
      ...
      if (sType === 'dataset_source' && tType === 'explorer' && src.data?.dataset_name) {
        ...
        updated = updated.map(n => n.id === tgt.id ? { ...n, data: { ...n.data, yolo_version: src.data.yolo_version, model_size: src.data.model_size, ..._rederive(n.data, 'run_label', '_run_label_src', mn, s => `best_${s}`) } } : n)
      } else if (sType === 'model' && tType === 'inference' && src.data?.model_path) {
        updated = updated.map(n => n.id === tgt.id ? { ...n, data: { ...n.data, model_path: src.data.model_path } } : n)
      }
    }
  }
  return updated
...
function SandgraphInner() {
  ...
  const addMlopsPair = useCallback(() => {
    ...
    toast.success(missing.length === 2
      ? 'Suivi MLOps activé : MLflow + DVC ajoutés'
      : `${missing[0] === 'mlflow' ? 'MLflow' : 'DVC'} ajouté — suivi MLOps complet`)
  }, [nodes, setNodes])
  ...
  useEffect(() => {
    setNodes(ns => {
      ...
      return ns.map(n => {
        ...
        return {
          ...n,
          data: { ...n.data, has_input: parentTypes.length > 0, input_types: parentTypes, input_handles: inputHandles, exec_order: order[n.id] },
        }
      })
    })
  }, [edges]) // pas besoin de nodes en dep — functional updater lit le latest state
  ...
  const isValidConnection = useCallback((conn: {
    ...
    if (wouldViolateExclusivity(tgt, targetHandle, already)) {
      toast.error('Entrées exclusives : débranchez d\'abord l\'autre port.')
      return false
    }
    return true
  }, [])
    ...
    if (conn.source && conn.target) {
      setNodes(ns => {
        ...
        if (sType === 'model' && tType === 'training') {
          const mn = _has(src.data?.model_path) ? _baseNoExt(src.data.model_path) : undefined
          return ns.map(n => n.id === conn.target
            ? { ...n, data: { ...n.data, yolo_version: src.data.yolo_version, model_size: src.data.model_size, ..._rederive(n.data, 'run_label', '_run_label_src', mn, s => `best_${s}`) } } : n)
        }
        if (sType === 'model' && tType === 'inference' && src.data?.model_path) {
          return ns.map(n => n.id === conn.target
            ? { ...n, data: { ...n.data, model_path: src.data.model_path } } : n)
        }
        return ns
      })
    }
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      setNodes(propagated)
      const stripped = propagated.map(n => ({ id: n.id, type: n.type, position: n.position, data: { ...n.data, exec_status: undefined, frontend_url: undefined, waiting_hint: undefined, waiting_next: undefined, has_input: undefined, input_types: undefined, input_handles: undefined, exec_order: undefined, current_step: undefined, progress: undefined, activity_steps: undefined, result_summary: undefined, dvc_pending: undefined } }))
    ...
    onError: (e: unknown) => toast.error(e instanceof Error ? e.message : 'Erreur lors de la sauvegarde', { duration: 6000 }),
  })
  ...
  const createMut = useMutation({
    ...
    onSuccess: g => { qc.invalidateQueries({ queryKey: ['graphs'] }); setActiveId(g.graph_id); toast.success('Expérience créée') },
  })

  // Duplicate
  const dupMut = useMutation({
    mutationFn: (id: string) => graphsAPI.duplicate(id),
    onSuccess: g => { qc.invalidateQueries({ queryKey: ['graphs'] }); setActiveId(g.graph_id); toast.success('Dupliquée') },
  })
            ...
            try {
              ...
              if (evt.type === 'done') {
                ...
                if (st !== 'failed' && st !== 'stopped') {
                  ...
                  toast.success('Pipeline terminé — les sorties sont prêtes dans le node DVC.', { duration: 9000 })
                }
    ...
    <div className="flex h-full">
      ...
      <aside className="w-40 shrink-0 bg-gray-900 border-r border-gray-800 flex flex-col">
        <div className="px-3 py-2.5 border-b border-gray-800">
          <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Nœuds</p>
          <p className="text-[10px] text-gray-600 mt-0.5">Glisser sur le canvas</p>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {([
            { cat: 'app', title: 'Applications' },
            { cat: 'input', title: 'Entrées (inputs)' },
        ...
        <div className="px-2 py-2 border-t border-gray-800">
          <p className="text-[9px] text-gray-700 leading-relaxed">Suppr = effacer · F = vue · Ctrl+Z/Y = annuler/rétablir</p>
        </div>
      </aside>
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-gray-900 border-b border-gray-800 shrink-0">
          ...
          {activeGraph && (
            renaming ? (
              ...
              <button onClick={startRename} title="Cliquer pour renommer" className="text-sm font-medium text-white hover:text-indigo-300 truncate max-w-[200px] border-b border-transparent hover:border-indigo-500 transition-colors">
                {activeGraph.name}
              </button>
            )
          )}
          ...
          {activeGraph && (
            mlops.tracking_complete ? (
              <span title="Ce graphe est suivi : DVC + MLflow presents. Chaque run est versionnable et tracé dans Insight / Lineage."
              ...
              <button onClick={addMlopsPair}
                title={`Suivi incomplet : il manque ${mlops.has_mlflow ? 'le node DVC' : 'le node MLflow'}. MLflow et DVC vont ensemble (versionner + tracer). Cliquer pour ajouter le node manquant.`}
                ...
                <Link2 size={12} /> Suivi incomplet — compléter
              </button>
            ) : (
              <button onClick={addMlopsPair}
                title="Graphe experimental (jetable). Cliquer pour activer le suivi MLOps : ajoute la paire MLflow + DVC (versioning + tracabilite + Insight + Lineage)."
                ...
                <span className="text-indigo-300">Track in MLOps</span>
              </button>
            )
          )}
          ...
          {activeId && (
            <div className="flex items-center gap-0.5">
              <button onClick={undo} title="Annuler (Ctrl+Z)" className="p-1.5 text-gray-500 hover:text-white rounded transition-colors disabled:opacity-30" disabled={historyRef.current.length === 0}>
                <Undo2 size={13} />
              </button>
              <button onClick={redo} title="Rétablir (Ctrl+Y)" className="p-1.5 text-gray-500 hover:text-white rounded transition-colors disabled:opacity-30" disabled={futureRef.current.length === 0}>
                <Redo2 size={13} />
              </button>
            </div>
          )}
          ...
          {activeId && (
            <>
              ...
              <button
                ...
                title="Auto Save : sauvegarde automatique après chaque édition (fonctionne seul, mêmes erreurs que Sauvegarder)"
              ...
              <button
                ...
                title="Auto Check : aligne/espace automatiquement les nodes Application (fonctionne seul — activez aussi Auto Save pour persister)"
                className={`flex items-center gap-1.5 px-2.5 py-1.5 text-xs rounded-lg border transition-colors ${
              ...
              {(isRunning || isWaiting) ? (
                <>
                  ...
                  <button onClick={() => stopMut.mutate()} disabled={stopMut.isPending}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-red-600 hover:bg-red-500 disabled:opacity-50 rounded-lg" title="Arrêter le pipeline">
                    {stopMut.isPending ? <Loader2 size={12} className="animate-spin" /> : <Square size={12} />}
                    Stop
                  </button>
                  <button onClick={() => resetMut.mutate()} className="p-1.5 text-gray-500 hover:text-white hover:bg-gray-800 rounded-lg" title="Réinitialiser">
                    <RotateCcw size={14} />
                  </button>
                </>
              ) : (
                <>
                  {activeGraph?.status === 'done' && (
                    <Link to="/mlops/insights" className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-emerald-300 border border-emerald-700/50 hover:bg-emerald-900/20 rounded-lg" title="Plots et journal du dernier run">
                      <BarChart3 size={12} />
                      Insights
                    </Link>
                  )}
          ...
          {!activeId && (
            <button onClick={() => createMut.mutate()} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg">
              <Plus size={12} /> Nouvelle expérience
            </button>
          )}
        ...
        <div className="flex-1 relative" onDragOver={onDragOver} onDrop={onDrop}>
          {!activeId ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-center">
              ...
              <div>
                <p className="text-white font-semibold">Aucune expérience</p>
                <p className="text-gray-500 text-sm mt-1">Créez votre premier sandgraph ou utilisez un template dans Expériences</p>
              </div>
        ...
        <>
          ...
          <div className="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[min(92vw,460px)]
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-800 bg-amber-950/30">
              ...
              <h3 className="text-sm font-semibold text-white flex-1">Chaîne terminée</h3>
            ...
            <div className="p-4 space-y-3 text-xs text-gray-300">
              <p>Toutes les étapes du graphe sont passées. Les sorties (dataset, annotations, modèle, métriques) sont prêtes.</p>
              {dvcPending ? (
                <p className="rounded-lg border border-amber-700/50 bg-amber-950/30 px-3 py-2 text-amber-200">
                  Ce run n'est <b>pas encore versionné</b>. Le nœud <b>DVC Commit</b> clignote tant que ce n'est pas fait :
                  cochez les artefacts à garder, puis créez la version.
                </p>
              ) : (
                <p className="rounded-lg border border-emerald-700/50 bg-emerald-950/30 px-3 py-2 text-emerald-200">
                  Ce run est déjà versionné dans DVC — rien d'autre à faire.
                </p>
              )}
              <div className="flex gap-2 pt-1">
                <button onClick={_openDvcNode}
                  ...
                  <GitBranch size={13} /> Ouvrir le nœud DVC
                </button>
            ...
            >
              <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-800">
                ...
                <input
                  ...
                  placeholder="Rechercher un nœud compatible…"
              ...
              <div className="max-h-72 overflow-y-auto py-1">
                {items.length === 0 ? (
                  <p className="px-3 py-4 text-xs text-gray-600 text-center">Aucun nœud compatible</p>
                ) : items.map(t => {
                  const Icon = t.icon
                  return (
                    <button
