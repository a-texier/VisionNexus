...
function analysisMeta(name: string): { title: string; desc: string } | null {
  const m = name.match(/^analysis_(.+)_(confusion|confusion_raw|pr_curve|f1_curve|results|val_pred)\.(png|jpg)$/)
  if (!m) return null
  const run = m[1]; const kind = m[2]
  const K: Record<string, [string, string]> = {
    confusion:     ['Matrice de confusion (normalisée)', 'taux vrais/faux par classe'],
    confusion_raw: ['Matrice de confusion (brute)',       'comptes bruts'],
    pr_curve:      ['Courbe Précision-Rappel',             '≈ ROC pour la détection'],
    f1_curve:      ['Courbe F1 vs seuil',                  'seuil de confiance optimal'],
    results:       ['Résultats Ultralytics',              'pertes + métriques par epoch'],
    val_pred:      ['Prédictions sur validation',          'sortie du modèle'],
  }
  const [t, d] = K[kind] ?? [kind, '']
  return { title: `${t}`, desc: `${run} · ${d}` }
}
  ...
  const body = (
    <div className="bg-gray-950 border border-gray-800 rounded-lg px-3 py-2">
      ...
      {shown != null ? (
        ...
        <div className="text-xs text-gray-600 mt-1 italic">{missing ?? 'non relié'}</div>
      )}
    </div>
  )
...
function LineageHeader(
  ...
  const forkMut = useMutation({
    ...
    onSuccess: r => {
      toast.success(`Fork créé : ${r.name}`)
  ...
  const trackMut = useMutation({
    ...
    onSuccess: r => {
      toast.success(r.added.length ? `Suivi MLOps activé (${r.added.join(' + ')} ajoutés)` : 'Déjà suivi MLOps')
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-950/40 to-gray-900 border border-indigo-800/40 rounded-xl p-4 space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <h2 className="text-sm font-semibold text-white">Identité du run</h2>
        <span title="ID unifié (orch_run_id) : la clé qui relie ce run à Git, DVC et MLflow (tag + trailer commit)."
        ...
        {forked?.run_id && (
          <button
            ...
            title="Ce graphe est un fork. Ouvrir le graphe parent."
            ...
            <GitFork size={10} /> fork de {forked.run_id.slice(0, 6)}
          </button>
        )}
        <span className="ml-auto text-[10px] text-gray-500">
          {l.committed_at ? `versionné ${new Date(l.committed_at).toLocaleString('fr')}` : 'non versionné (pas de commit DVC)'}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Action href={mlHref} icon={<FlaskConical size={13} />} label="Open MLflow Run"
          disabled={!mlHref} title={mlHref ? '' : 'Aucun run MLflow lié à ce run'} />
        ...
        <button onClick={() => openSandgraph(item.graph_id)}
          title="Ouvrir le Sandgraph qui a produit ce run"
        ...
        <button onClick={() => navigate('/mlops/lineage')}
          title="Voir ce run dans l'arbre de lineage complet"
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-indigo-700/50 text-indigo-300 hover:bg-indigo-900/20 transition-colors">
          <GitFork size={13} /> Open Lineage
        </button>
        {!isMlops && (
          <button onClick={() => trackMut.mutate()} disabled={trackMut.isPending}
            title="Ce run est experimental : activer le suivi MLOps ajoute la paire MLflow + DVC au graphe, puis committez pour versionner."
        ...
        <button
          ...
          title="Duplique ce graphe en figeant le même dataset et les mêmes annotations, et enregistre la provenance du run. Ajustez ensuite les params puis relancez."
          className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
            forkMut.isPending
              ? 'border-gray-800 text-gray-600 cursor-not-allowed'
              : 'border-indigo-700/50 text-indigo-300 hover:bg-indigo-900/20'
        ...
        <button
          ...
          title={repro?.reproducible ? '' : 'Lineage incomplet — voir la checklist ci-dessous'}
          className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
            repro?.reproducible
      ...
      {showRepipe && repro?.reproducible && (
        <div className="bg-gray-950 border border-emerald-800/40 rounded-lg p-3 text-[11px] text-gray-300 space-y-2">
          <p className="text-emerald-300">Reproduire ce run, sans ligne de commande :</p>
          <ol className="space-y-1.5 list-decimal list-inside">
            <li>
              <b>Restaurer la version des données</b> — dans l'app DVC, onglet Historique,
              cliquer « Restaurer cette version » sur le commit <span className="font-mono text-indigo-300">{commit?.slice(0, 8)}</span>.
              {dvcHistoryHref && <a href={dvcHistoryHref} target="_blank" rel="noreferrer" className="ml-1 text-indigo-300 hover:text-indigo-200">Ouvrir l'historique <ExternalLink size={10} className="inline" /></a>}
            </li>
            <li>
              <b>Récupérer les fichiers</b> — si un remote est configuré, onglet Sync → Pull
              (récupère le contenu exact du dataset {l.dataset ? <span className="font-mono">{l.dataset}</span> : ''}).
              {dvcUrl && <a href={`${dvcUrl}/sync`} target="_blank" rel="noreferrer" className="ml-1 text-indigo-300 hover:text-indigo-200">Ouvrir Sync <ExternalLink size={10} className="inline" /></a>}
            </li>
            <li>
              <b>Re-lancer à l'identique</b> — bouton <span className="text-indigo-300">« Fork this run »</span> ci-dessus
              (fige le même dataset + annotations), puis « Lancer » dans le Sandgraph.
            </li>
            <li>
              <b>Comparer le résultat</b> — au run MLflow d'origine
              {mlHref ? <a href={mlHref} target="_blank" rel="noreferrer" className="ml-1 text-indigo-300 hover:text-indigo-200">run {mlRun?.run_id?.slice(0, 8)} <ExternalLink size={10} className="inline" /></a> : <span className="font-mono"> {mlRun?.run_id?.slice(0, 8)}</span>} — mAP50 attendue {l.map50 != null ? Number(l.map50).toFixed(4) : '—'}.
            </li>
          </ol>
        </div>
      )}
        ...
        <div className="bg-gray-950 border border-gray-800 rounded-lg">
          <button onClick={() => setShowRepro(v => !v)} className="w-full flex items-center gap-2 px-3 py-2 text-left">
            ...
            <span className={`text-[11px] font-semibold ${repro.reproducible ? 'text-emerald-400' : 'text-amber-400'}`}>
              {repro.reproducible ? 'Reproducible' : 'Incomplet'}
            </span>
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold text-white">Sandgraph source</h3>
...
function InsightDetail({ item, workspaceScope }: { item: InsightSummary; workspaceScope: string | null }) {
  ...
  const regenMut = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['graph', item.graph_id] })
      toast.success('Insight régénéré')
    },
    onError: (e: Error) => toast.error(e.message),
  })
  ...
  const analysisPlots = item.plots.filter(p => analysisMeta(p))   // PNG Ultralytics (confusion, PR, F1…)
  ...
  const exportReport = async () => {
    try {
      ...
      const tblTrain = `<table><tr><th>Entraînement</th><th>mAP50</th><th>mAP50-95</th><th>epochs</th></tr>${
        trainings.map(t => `<tr><td>${t.run_name}</td><td>${t.status?.best_map50?.toFixed(4) ?? '—'}</td><td>${t.status?.best_map5095?.toFixed(4) ?? '—'}</td><td>${t.status?.total_epochs ?? '—'}</td></tr>`).join('')}</table>`
      const tblOpt = optuna.length ? `<table><tr><th>Étude</th><th>trials</th><th>best</th><th>direction</th></tr>${
      ...
      const tblDvc = dvcCommits.length ? `<table><tr><th>Commit</th><th>Sujet</th></tr>${dvcCommits.map(c => `<tr><td>${c.short}</td><td>${c.subject ?? ''}</td></tr>`).join('')}</table>` : ''
      const html = buildHtmlReport({
        title: `Rapport Insights — ${item.graph_name}`,
        subtitle: `run ${item.run_id} · généré ${new Date().toLocaleString('fr')}`,
        plotlyJs: plotlyRaw,
        sections: [
          { heading: 'Entraînement', note: 'Métriques et pertes par epoch (toutes les runs superposées).',
            html: tblTrain,
            figures: [
              ...(mapTraces.length ? [{ title: 'mAP par epoch', data: mapTraces, layout: { xaxis: { title: 'epoch' }, yaxis: { title: 'mAP' } } }] : []),
              ...(lossTraces.length ? [{ title: 'Pertes par epoch', data: lossTraces, layout: { xaxis: { title: 'epoch' }, yaxis: { title: 'loss' } } }] : []),
              ...(gainTrace.length ? [{ title: 'mAP50 finale par entraînement', data: gainTrace, layout: { yaxis: { title: 'mAP50' } } }] : []),
            ] },
          ...(optuna.length ? [{ heading: 'Optuna (HPO)', note: 'Étude d\'hyperparamètres.', html: tblOpt,
            figures: optunaTraces.length ? [{ title: 'Valeur par trial', data: optunaTraces, layout: { xaxis: { title: 'trial' }, yaxis: { title: 'valeur' } } }] : [] }] : []),
          ...(dvcCommits.length ? [{ heading: 'DVC — versions', html: tblDvc }] : []),
        ],
      })
      ...
      toast.error('Export échoué : ' + (e as Error).message)
    }
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3 bg-gray-900/50 border border-gray-800 rounded-xl px-4 py-3">
        <p className="text-[11px] text-gray-500 leading-relaxed max-w-2xl">
          <span className="text-gray-300 font-semibold">Logique du run</span> : le pipeline produit des
          données (dataset → subset → annotation), <b>optimise</b> les hyperparamètres (Optuna), <b>entraîne</b>
          le modèle final, puis l'<b>évalue</b>. Les blocs ci-dessous suivent cet ordre — courbes interactives
          (zoom, survol, toggle légende) construites depuis <span className="font-mono">insights.json</span>.
        </p>
        <div className="shrink-0 flex items-center gap-2">
          <button onClick={() => regenMut.mutate()} disabled={regenMut.isPending}
            title="Recollecte l'état réel (DVC / MLflow / modèle) et régénère cet insight. À utiliser si le versioning vient de changer."
            ...
            {regenMut.isPending ? <Loader2 size={13} className="animate-spin" /> : <RefreshCw size={13} />} Régénérer
          </button>
          <button onClick={exportReport}
            ...
            <FileText size={13} /> Exporter rapport HTML
          </button>
        </div>
      </div>
      ...
      {trainings.length > 0 && (
        <Section title="Résultats — modèles entraînés" icon={<Zap size={14} className="text-blue-400" />}>
          ...
          <div className="flex flex-wrap gap-2 text-[11px] mt-3">
            ...
            {mlflowRuns.length > 0 && (
              <span className="px-2 py-1 rounded-lg bg-emerald-900/30 border border-emerald-700/30 text-emerald-300">MLflow · {mlflowRuns.length} runs tracés</span>
            )}
      ...
      {(mapTraces.length > 0 || gainTrace.length > 0) && (
        <Section title="Entraînement — courbes par epoch" icon={<TrendingUp size={14} className="text-blue-400" />}
          note="Interactif : zoom, survol pour les valeurs, clic sur la légende pour isoler une run.">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {mapTraces.length > 0 && <ChartCard title="mAP par epoch"><Plot data={mapTraces} layout={{ xaxis: { title: 'epoch' }, yaxis: { title: 'mAP', rangemode: 'tozero' } }} /></ChartCard>}
            {lossTraces.length > 0 && <ChartCard title="Pertes par epoch (box · cls)"><Plot data={lossTraces} layout={{ xaxis: { title: 'epoch' }, yaxis: { title: 'loss', rangemode: 'tozero' } }} /></ChartCard>}
            {prTraces.length > 0 && <ChartCard title="Précision / Rappel par epoch"><Plot data={prTraces} layout={{ xaxis: { title: 'epoch' }, yaxis: { title: 'valeur', range: [0, 1] } }} /></ChartCard>}
            {gainTrace.length > 0 && <ChartCard title="mAP50 finale par entraînement"><Plot data={gainTrace} layout={{ yaxis: { title: 'mAP50', rangemode: 'tozero' } }} /></ChartCard>}
          </div>
        </Section>
      )}
      ...
      {optunaTraces.length > 0 && (
        <Section title="Optuna — historique de l'étude" icon={<Settings2 size={14} className="text-cyan-400" />}
          note="Valeur de la métrique par trial (TPE + pruning).">
          <ChartCard title="Valeur par trial"><Plot data={optunaTraces} layout={{ xaxis: { title: 'trial' }, yaxis: { title: 'valeur' } }} /></ChartCard>
        </Section>
      )}

      {/* BLOC 4 — Analyse détaillée du modèle (PNG Ultralytics) — repliable */}
      {analysisPlots.length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl">
          <button onClick={() => setShowAnalysis(v => !v)} className="w-full flex items-center gap-2 px-4 py-3 text-left">
            ...
            <span className="text-sm font-semibold text-white">Analyse détaillée du modèle ({analysisPlots.length}) — matrices, PR, F1, prédictions</span>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl">
        ...
        >
          ...
          <span className="text-sm font-semibold text-white">Journal des étapes ({steps.length}) — outputs bruts</span>
      ...
      {graphLogs.length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl">
          ...
          >
            ...
            <span className="text-sm font-semibold text-white">Logs complets ({graphLogs.length}) — blocs colorés par node</span>
            <span className="text-xs text-gray-600 ml-auto">{showLogs ? '▲' : '▼'}</span>
          </button>
          {showLogs && (
            <div className="px-4 pb-4 max-h-96 overflow-y-auto text-[11px]">
              <p className="text-[10px] text-gray-600 mb-2">Dernier run de ce graphe (identiques au journal du Sandgraph).</p>
      ...
      <p className="text-[10px] text-gray-600">
        Fichiers persistés dans le workspace : <span className="font-mono">insights/{item.graph_id}/{item.run_id}/</span> (insights.json · insights.md · plots PNG)
      </p>
    </div>
  )
}
...
export default function InsightsPage() {
  ...
  const genMut = useMutation({
    ...
    onSuccess: r => {
      ...
      toast.success(`Insights générés (${r.plots.length} plots)`)
    },
    onError: (e: Error & { response?: { data?: { detail?: string } } }) =>
      toast.error(e.response?.data?.detail ?? e.message),
  })
  ...
  const delMut = useMutation({
    ...
    onSuccess: () => {
      setSelected(null)
      qc.invalidateQueries({ queryKey: ['insights-list'] })
      toast.success('Insight supprimé')
    },
    onError: (e: Error) => toast.error(e.message),
  })
    ...
    <div className="p-6 h-full overflow-y-auto space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-gray-500">
          Plots d'évolution + journal de compréhension pour chaque run de template
        </span>
        <div className="flex-1" />
        <span className="flex items-center gap-1.5 text-[11px] text-gray-500" title="Les insights sont (re)générés automatiquement au fur et à mesure : après chaque étape significative (training, export, commit, HPO) ET à la fin du run. Régénérables/supprimables à la main.">
          <Info size={12} /> générés au fil du pipeline
        </span>
        ...
        {graphs.filter(g => (g.run_history?.length ?? 0) > 0 || g.active_run_id).slice(0, 4).map(g => (
          <button
            ...
            title={`Générer les insights du dernier run de "${g.name}"`}
          >
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 gap-3 text-center">
          ...
          <div>
            <p className="text-white font-medium">Aucun insight généré</p>
            <p className="text-gray-500 text-sm mt-1">
              Lancez un template dans le Sandgraph — les insights sont générés automatiquement à la fin du run.
              Vous pouvez aussi les générer manuellement avec les boutons ci-dessus.
            </p>
          </div>
        </div>
      ) : (
