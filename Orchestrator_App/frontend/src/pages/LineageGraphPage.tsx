    ...
    >
      {run ? (
        ...
        <>
          ...
          <p className="mt-2 text-[10px] text-gray-500">{t("déposez le jeton")}</p>
        </>
      )}
    </div>
  );
  return (
    <aside className="w-[420px] shrink-0 overflow-y-auto border-l border-gray-800 bg-gray-950 shadow-2xl">
      <div className="sticky top-0 z-10 flex items-center border-b border-gray-800 bg-gray-950 px-4 py-4">
        ...
        <h2 className="flex-1 font-semibold text-white">{t("Comparaison de runs")}</h2>
      ...
      <div className="space-y-4 p-4">
        <p className="text-xs text-gray-400">
          {t("Glissez deux jetons ici. Les éléments identiques restent discrets ; seules les différences sont accentuées.")}
        </p>
          ...
          <>
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-white">
                {t("Différences détectées")}
              </h3>
              <span className="text-[10px] text-gray-500">
                {diffs.filter((d) => d.state !== "identical").length} {t("section(s)")}
              </span>
            </div>
            ...
            <div className="rounded-lg border border-sky-800/50 bg-sky-950/20 p-3 text-[10px] text-sky-200">
              {t("Une sortie absente dans le fork n’est jamais remplacée par celle du parent.")}
            </div>
          </>
        )}
      </div>
        ...
        <div className="mb-5 flex gap-3 rounded-xl border border-amber-600/50 bg-amber-950/15 p-4">
          ...
          <div>
            ...
            <h2 className="text-sm text-white">
              {source?.label || t("Source non identifiée")}
            </h2>
            <p className="mt-1 font-mono text-[10px] text-gray-500">
              {String(source?.data.source_path || t("chemin non renseigné"))}
            </p>
          </div>
        </div>
              ...
              >
                <div className="mb-4 flex">
                  <div className="flex-1">
                    ...
                    >
                      {run.data.is_fork ? t("Run fork") : t("Run mère")}
                    </p>
                ...
                <div className="grid gap-2 sm:grid-cols-3">
                  ...
                  {!products.length && (
                    <p className="col-span-3 rounded border border-dashed border-gray-800 p-3 text-xs text-gray-500">
                      {t("Aucune production pour ce run interrompu.")}
                    </p>
                  )}
                </div>
              </section>
...
function DetailPanel({
  ...
  const forkRun = () => {
    ...
    axios
      ...
      .catch((e) => setForkError(e?.response?.data?.detail || e?.message || t("Fork impossible")))
        ...
        {node.type === "run" && (
          <>
            ...
            <button
              ...
              title={t("Duplique le graphe de ce run (meme dataset, memes annotations) et enregistre sa provenance, puis ouvre le fork.")}
...
function LineageInner() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((e) => setError(e?.message || t("Chargement échoué")));
    return () => {
      alive = false;
    };
  }, []);
      ...
      <header className="flex items-center gap-3 border-b border-gray-800 px-5 py-3">
        ...
        <div>
          <h1 className="text-base font-semibold text-white">
            {t("Lineage des expériences")}
          </h1>
          <p className="text-[10px] text-gray-500">
            {t("Run parent → fork → différences introduites → conséquences sur les sorties")}
          </p>
        </div>
        <div className="flex-1" />
        <button
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={t("Run, dataset, subset…")}
