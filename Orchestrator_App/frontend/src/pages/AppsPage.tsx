...
const APP_META: Record<string, { icon: React.ReactNode; color: string; desc: string }> = {
  ...
  inference: {
    ...
    color: 'text-sky-400',
    desc: 'Acquisition, inférence, tracking et évaluation des modèles entraînés.',
  },
  dvc: {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-96 shadow-2xl" onClick={e => e.stopPropagation()}>
        <h3 className="text-white font-semibold mb-4">Lancer {appId}</h3>
        <div className="space-y-3">
          <div>
            <label className="text-xs text-gray-400 block mb-1">Workspace de base</label>
            <input value={ws} onChange={e => setWs(e.target.value)} placeholder="Laissez vide = auto (à côté de l'orchestrateur)"
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500" />
          </div>
          <div>
            <label className="text-xs text-gray-400 block mb-1">Utilisateur</label>
            <input value={user} onChange={e => setUser(e.target.value)} placeholder="Laissez vide = user courant"
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500" />
          </div>
          <div>
            <label className="text-xs text-gray-400 block mb-1">Env conda</label>
...
export default function AppsPage() {
  ...
  const launchMut = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success('Application lancée')
  ...
  const stopMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['launcher-sessions'] }); toast('Application arrêtée') },
    onError: (e: Error) => toast.error(e.message),
  })
  ...
  const launchAllMut = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (n > 0) toast.success(`${n} app${n > 1 ? 's' : ''} lancée${n > 1 ? 's' : ''}`)
      else toast('Toutes les apps sont déjà actives')
    },
    onError: (e: Error) => toast.error(e.message),
  })
  ...
  const stopAllMut = useMutation({
    ...
    onSuccess: (data) => {
      qc.invalidateQueries({ queryKey: ['launcher-sessions'] })
      toast(`${data.stopped.length} app(s) arrêtée(s)`)
    },
    onError: (e: Error) => toast.error(e.message),
  })
    ...
    <div className="p-6 overflow-y-auto h-full space-y-6">
            ...
            <div key={appId} className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
              ...
              {isStarting && (
                <p className="flex items-center gap-1 text-[10px] text-amber-300">
                  <Loader2 size={10} className="animate-spin" /> Démarrage en cours…
                </p>
              )}
              {isFailed && (
                <div className="rounded-lg border border-red-900/70 bg-red-950/30 p-2 text-[10px] text-red-300">
                  <p className="font-semibold">Échec du démarrage</p>
                  <p className="mt-1 whitespace-pre-wrap break-words text-red-300/80">
                    {session.failure_reason || 'Le backend ne répond pas. Consultez le journal de lancement.'}
                  </p>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-4">
        <p className="text-xs text-gray-500">
          <span className="text-gray-400 font-medium">Mode indépendant</span> — vous pouvez aussi lancer chaque app manuellement depuis son launcher :
