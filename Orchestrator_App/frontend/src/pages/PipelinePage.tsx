  ...
  if (steps.length === 0) {
    return (
      <div className="flex items-center justify-center h-full text-gray-600 text-sm">
        Aucune étape — éditez le pipeline dans la bibliothèque
      </div>
    )
  }
...
export default function PipelinePage() {
    ...
    try {
      ...
      _cancelSSE = streamRun(
        ...
        (evt: RunEvent) => {
          ...
          if (evt.type === 'waiting') {
            setStatusLabel(`⏸ En attente — ${evt.step_id}`)
            // Ne pas réinitialiser — pipeline en pause, pas terminé
          }
          if (evt.type === 'done') {
            setStatusLabel(evt.status === 'success' ? '✓ Succès' : '✗ Échoué')
            resetRunState()
            invalidate(id)
            invalidateActivity()
          }
      ...
      const msg = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? 'Erreur au démarrage'
      toast.error(msg)
    }
  }
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center h-full text-gray-500 text-sm">Chargement…</div>
  }
  if (!pipeline) {
    return <div className="p-6 text-gray-500">Pipeline introuvable</div>
  }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-800 shrink-0">
        ...
        <span className="text-xs text-gray-600 font-mono">{steps.length} étapes</span>
      </div>
