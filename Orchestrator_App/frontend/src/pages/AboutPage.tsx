...
export default function AboutPage() {
  ...
  return (
    <div className="p-6 space-y-4 max-w-6xl mx-auto overflow-y-auto h-full">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-white mb-2">
              {t('Plateforme MLOps Computer Vision')}
            </h1>
            <p className="text-gray-400 leading-relaxed max-w-2xl">
              {t("Suite intégrée d'outils pour construire des pipelines de Computer Vision complets : de l'exploration de datasets jusqu'à la validation de modèles, avec supervision humaine à chaque étape critique.")}
            </p>
          </div>
        </div>
          ...
          {APPS.map(app => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-white text-sm">{t(app.name)}</h3>
              ...
              <p className="text-xs text-gray-400 leading-relaxed">{t(app.description)}</p>
      ...
      {/* CV Training Loop */}
      <Section title={t('Pipeline CV Training Loop — 13 étapes, 5 validations humaines')}>
        <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-5 overflow-x-auto">
            ...
            {WORKFLOW_STEPS.map((step, idx) => (
              <div key={step.id} className="flex items-center gap-3">
                ...
                <div className={`flex items-center gap-3 rounded-lg px-3 py-2 border flex-1 ${
                  ...
                  <span className="text-sm text-white font-medium flex-1">{t(step.label)}</span>
                  <span className={`text-xs px-2 py-0.5 rounded border shrink-0 ${APP_COLOR[step.app] || 'bg-gray-700 text-gray-300 border-gray-600'}`}>
                    {step.app}
                  </span>
                  <span className="text-xs text-gray-600 shrink-0 hidden sm:block">{t(step.desc)}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 flex gap-4 text-xs text-gray-500">
            <span className="flex items-center gap-1.5">
              <span className="text-green-400">●</span> {t('Automatique')}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="text-orange-400">⏸</span> {t('Validation humaine')}
            </span>
          </div>
        </div>
      </Section>
          ...
          {[
            ...
            <div key={section.title} className="rounded-xl border border-gray-800 bg-gray-900/50 p-4">
              <div className="flex items-center gap-2 mb-3 text-indigo-400">
                {section.icon}
                <h3 className="font-medium text-sm text-white">{t(section.title)}</h3>
              </div>
      ...
      {/* Quick start */}
      <Section title={t('Démarrage rapide')} defaultOpen={false}>
        <div className="space-y-3">
          {[
            ...
            <div key={step.n} className="flex gap-3 items-start">
              ...
              <div className="flex-1">
                <p className="text-sm text-gray-300">{t(step.text)}</p>
                {step.code && (
      ...
      {/* Raccourcis clavier */}
      <Section title={t('Raccourcis clavier — Sandgraph')} defaultOpen={false}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {[
            ...
            <div key={key} className="flex items-start gap-3 p-3 bg-gray-800/40 rounded-lg">
              <kbd className="shrink-0 px-2 py-0.5 bg-gray-700 border border-gray-600 rounded text-xs text-gray-200 font-mono whitespace-nowrap">{t(key)}</kbd>
      ...
      <div className="flex items-center gap-2 text-xs text-gray-700 justify-center pb-4">
        ...
        <span>{t('Plateforme MLOps CV — Orchestrateur v2')}</span>
      </div>
    </div>
  )
}
