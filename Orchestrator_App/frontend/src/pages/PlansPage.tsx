...
const OV_FIELDS: { key: string; label: string; ph: string }[] = [
  ...
  { key: 'basic_lr_per_img', label: 'LR par image', ph: '0.00015' },
...
export default function PlansPage() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['plans'] }); toast.success(t('Plan enregistré')) },
  ...
  const runMut = useMutation({
    ...
    onSuccess: r => { r.ok ? toast.success(t('Plan lancé')) : toast(r.message ?? t('Déjà en cours')); qc.invalidateQueries({ queryKey: ['plans'] }) },
    onError: (e: Error) => toast.error(e.message),
  })
    ...
    <div className="p-6 h-full overflow-y-auto">
      <div className="max-w-5xl mx-auto space-y-5">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-white">{t("Plans d'expériences")}</h1>
          <span className="text-xs text-gray-500">{t("planifier une suite d'expériences et la lancer d'un clic")}</span>
          <button onClick={() => createMut.mutate()}
            ...
            <Plus size={14} /> {t('Nouveau plan')}
          </button>
        </div>

        <p className="text-[12px] text-gray-500 leading-relaxed">
          {t('Chaque étape')} <b>{t('duplique un graphe de base')}</b> {t('(que vous avez construit dans le Sandgraph : pipeline complet, ou graphe « réutilisation » annotation FREE → training) et applique des')}
          <b> {t('overrides')}</b>{t(', puis lance le run et commit dans DVC. Le résultat apparaît dans le graphe de Lineage.')}
        </p>

        <div className="flex gap-5">
          {/* Liste des plans */}
          <aside className="w-52 shrink-0 space-y-1.5">
            {plans.length === 0 && <p className="text-xs text-gray-600">{t('Aucun plan. Créez-en un.')}</p>}
            {plans.map(p => (
              <button key={p.plan_id} onClick={() => setSelId(p.plan_id)}
                ...
                <p className="text-[10px] text-gray-500 mt-0.5">{p.steps.length} {t('étape(s)')}{p.last_run ? ` · ${p.last_run.status}` : ''}</p>
              </button>
            ))}
          </aside>
          ...
          <div className="flex-1 min-w-0 space-y-4">
            {!selected ? (
              <div className="text-sm text-gray-500 py-10 text-center">{t('Sélectionnez ou créez un plan.')}</div>
            ) : (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => runMut.mutate()} disabled={running || steps.length === 0}
                    ...
                    {running ? <Loader2 size={13} className="animate-spin" /> : <Play size={13} />} {t('Lancer le plan')}
                  </button>
                  <button onClick={() => { if (confirm(t('Supprimer ce plan ?'))) delMut.mutate(selected.plan_id) }}
                ...
                <div className="space-y-3">
                    ...
                    <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl p-3 space-y-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <input value={st.label} onChange={e => setStep(i, { label: e.target.value })}
                          placeholder={t("libellé de l'étape (ex. baseline)")}
                        ...
                        <select value={st.base_graph_id} onChange={e => setStep(i, { base_graph_id: e.target.value })}
                          ...
                          <option value="">{t('graphe de base…')}</option>
                  ...
                  <button onClick={() => setSteps(s => [...s, _newStep()])}
                    ...
                    <Plus size={13} /> {t('Ajouter une étape')}
                  </button>
                </div>
                  ...
                  <div className="bg-gray-950 border border-gray-800 rounded-xl p-3 space-y-2">
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span className="text-gray-200 font-semibold">
                        {t('Exécution')} {status?.status ?? selected.last_run?.status}
                        {status?.total ? ` — ${t('étape')} ${status.current}/${status.total}` : ''}
                      </span>
                    </div>
                    {results.map(r => (
                    ...
                    <p className="text-[10px] text-gray-600 flex items-center gap-1">
                      <FlaskConical size={11} /> {t("Les résultats sont navigables dans l'onglet Lineage.")}
                    </p>
                  </div>
                )}
              </>
