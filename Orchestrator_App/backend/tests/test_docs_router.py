"""Router api/docs.py (doc markdown de la page Guide) : mini app FastAPI avec
le seul router docs, sur un dossier docs temporaire."""
...
def test_app_dir_points_to_orchestrator_docs():
    assert docs.DOCS_DIR.parent.name == "Orchestrator_App"
    assert docs.MANIFEST_PATH.parts[-2:] == ("docs", "docs_manifest.json")
...
def test_list_docs_titles_and_langs(docs_client: TestClient):
    ...
    assert by_name["workflows"]["title"] == "Procédures"
    assert by_name["workflows"]["langs"] == ["en", "fr"]
    assert by_name["code-map"]["langs"] == [] and by_name["code-map"]["title"] == "code-map"


def test_get_doc_lang_fallback(docs_client: TestClient):
    data = docs_client.get("/api/docs/concepts", params={"lang": "en"}).json()
    assert (data["lang"], data["title"], data["body"]) == ("fr", "Concepts FR", "Corps\n")
