...
# Artefacts d'analyse à rapatrier par training (bundlés dans l'insight).
# GET /api/training/{run}/artifacts renvoie, par categorie, les plots declares
# par le MOTEUR du run (catalogue "artifacts" de Training_App), du prefere au
# moins prefere : un run YOLOX donne ceux de detection_metrics/yolox_plots, un
# run de plugin ses plots natifs (synthese "summary" comprise s'il en a une).
# (categorie, suffixe de fichier voulu dans la categorie, kind) ; suffixe vide
# = premier fichier de la categorie.
_ANALYSIS_WANTED = [
    ("summary",         "",                          "summary"),
    ("confusion",       "",                          "confusion"),
...
def _pick_analysis_file(names: list[str], suffix: str) -> str | None:
    """Premier fichier de la categorie, ou celui dont le nom finit par `suffix`
    (les moteurs prefixent parfois : "BoxPR_curve.png" pour "PR_curve.png")."""
    if not suffix:
        return names[0] if names else None
    return next((n for n in names if n.rsplit("/", 1)[-1].endswith(suffix)), None)
        ...
        for t in data.get("trainings", []):
            ...
            for cat, fname, kind in _ANALYSIS_WANTED:
                if kind in seen_kinds:
                    continue
                target = _pick_analysis_file(arts.get(cat) or [], fname)
                if not target:
                    continue
                try:
