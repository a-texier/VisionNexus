...
def _steps_for_node(node: dict, deps: list[str], ctx: dict | None = None,
    ...
    if ntype == "optuna":
        # AUTO : lance l'étude (TPE+pruning) via l'endpoint orchestrateur -> best params.
    ...
    if ntype == "training":
        yolo_version = data.get("yolo_version", "yolov8")
        model_size   = data.get("model_size", "n")
        # Un noeud d'entree `model` branche FIGE l'architecture (best.pt = telle version).
        for _pn in parent_nodes or []:
            if (_pn.get("data", {}).get("node_type") or _pn.get("type", "")) == "model":
                _pd = _pn.get("data", {})
                yolo_version = _pd.get("yolo_version") or yolo_version
                model_size   = _pd.get("model_size") or model_size
                break
        ...
        run_label = data.get("run_label", "") or f"{yolo_version}{model_size}"
            ...
            return [
                {
                    ...
                    "hint": (
                        ...
                        f"({yolo_version}{model_size} · {epochs} epochs). "
        ...
        _HP_KEYS = {
            ...
            "box", "cls", "dfl", "workers", "cache", "device",
            "hsv_h", "hsv_s", "hsv_v", "degrees", "translate", "scale",
            "shear", "perspective", "flipud", "fliplr", "mosaic", "mixup",
            "copy_paste", "erasing", "close_mosaic", "patience",
        }
        extra_hp = {k: data[k] for k in _HP_KEYS if k in data}
        hyperparams = {"epochs": epochs, "batch": batch, "imgsz": imgsz, **extra_hp}
        ...
        if not optuna_best_ref and parent_nodes:
            for pn in parent_nodes:
                ...
                if (pd.get("node_type") or pn.get("type", "")) == "optuna":
                    ...
                    if bp:
                        hyperparams.update({k: v for k, v in bp.items() if k in _HP_KEYS})
                    break
        ...
        return [
            {
                ...
                "depends_on": deps, "type": "task",
                "hint": (
                    f"Entraînement automatique : {yolo_version}{model_size} · {epochs} epochs. "
    ...
    if ntype == "inference":
        # Noeud UNIFIE (Inference_App = tracker + inference + evaluation + acquisition).
        #   FREE  (aucune entree) : ACQUISITION -> capture un flux, produit un dataset d'images
        #   LOCKED (entree)       : eval / inference selon data.task
        ...
        # ── FREE = acquisition (produit une sortie : dataset d'images a annoter) ──
        if not has_input:
            return [{
                "id": sid("acquire"), "label": "Acquisition (capture flux -> images)",
                "app": "Inference_App", "method": "POST", "endpoint": "/api/orchestrator/acquire",
                "params": {
                    "source":      data.get("sequence_dir", ""),
                    "name":        data.get("acq_name", "") or (data.get("label") or "acquisition"),
                    "max_frames":  int(data.get("max_frames", 200)),
                    "every":       int(data.get("every", 1)),
                    "fmt":         data.get("frame_ext", "png"),   # png (brut) | jpg (compresse)
                    "camera_name": data.get("camera_name", ""),
                },
                "depends_on": deps, "type": "task",
                "hint": ("Capture le flux (MJPEG/ZMQ/optional_format/video) et sauve les images -> "
                         "dataset importable dans Dataset Explorer / Annotation."),
        ...
        #       'tracking'  : tracker headless -> benchmark ; sans cmd_file = MOT seul ;
        #                     avec cmd_file (cmd_send) = test SOT+MOT (rejeu de clics).
        full_auto = bool(data.get("full_auto", True))
        ...
        # (`kind="detection"` = model.val() Ultralytics), AUCUN tracker (MOT/SOT = none).
        ...
        # ── TRACKING (unifié) : headless ; cmd_file -> test SOT+MOT (rejeu), sinon MOT seul ──
        tracker_mot = data.get("tracker_mot", "botsort")
        tracker_sot = data.get("tracker_sot", "tracking_tophat")
        cmd_file    = data.get("cmd_file") or data.get("clicks") or None
        mode        = "command" if cmd_file else "headless"
        return [{
            "id": sid("infer"), "label": f"Tracking {tracker_mot}" + (f"/{tracker_sot} (SOT+MOT)" if cmd_file else " (MOT)"),
            "app": "Inference_App", "method": "POST", "endpoint": "/api/orchestrator/infer",
            "params": {
                "sequence_dir": seq_input, "model_path": model_path,
                "mode": mode, "clicks": cmd_file,
...
def graph_to_pipeline(graph: dict) -> tuple[PipelineDef, dict]:
    ...
    for node in ordered:
        ...
        # Mode FREE : node sans input → expose outputs existants, aucune étape pipeline.
        # EXCEPTION inference : FREE = ACQUISITION (produit un dataset d'images) -> genere une etape.
...
def _needed_app_keys(graph: dict) -> set[str]:
    ...
    for node in graph.get("nodes", []):
        ...
        if ntype == "dvc":
            needed.add("dvc-app")
            continue
        # Inference : FREE = acquisition (produit un dataset) -> app requise meme sans arete.
        if ntype == "inference":
