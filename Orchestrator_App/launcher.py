...
def _find_app_base(app_root: Path) -> Path:
    """Full stack: traverse up to find Computer_Vision_App/../ = App/.
    Standalone zip: no Computer_Vision_App ancestor → use app root itself."""
    p = app_root
    for _ in range(5):
        if p.name == "Computer_Vision_App":
            return p.parent
        p = p.parent
    return app_root


APP_BASE = _find_app_base(APP_ROOT)
...
def _bundled_node_bin() -> "str | None":
    ...
    Prioritaire sur nvm/systeme. Cherche en remontant depuis APP_ROOT."""
    p = APP_ROOT
    for _ in range(6):
...
def find_node_bin() -> str | None:
    """Find Node.js v20 bin directory for Linux standalone deployment."""
    home = Path.home()
    candidates = [
        home / ".nvm" / "versions" / "node" / "v20.20.2" / "bin",
        home / ".nvm" / "versions" / "node" / "v20" / "bin",
        Path("/usr/local/lib/nodejs/node-v20.20.2-linux-x64/bin"),
        Path("/usr/local/bin"),
        Path("/usr/bin"),
    ]
    for d in candidates:
        if (d / "node").exists() or (d / "npm").exists():
            return str(d)
    return None
...
def main():
    parser = argparse.ArgumentParser(description="Launch orchestrator-app")
    ...
    parser.add_argument("--no-reload",     action="store_true", help="(deprecie, sans effet : le reload est OFF par defaut)")
    parser.add_argument("--reload",        action="store_true", help="Active uvicorn --reload (DEV uniquement). OFF par defaut : sur Windows le reload fait tomber tout l'arbre VisionNexus (backend+frontend) au moindre edit backend.")
    ...
    node_bin = _bundled_node_bin() or find_node_bin()
    env = {**os.environ,
           "ORCHESTRATOR_WORKSPACE": ws, "ORCHESTRATOR_USER": args.user,
           "ORCHESTRATOR_FRONTEND_PORT": str(fp),
    ...
    print("=" * 60); print(f"  orchestrator-app — {args.user}"); print("=" * 60)
    ...
    # Reload OFF par defaut (opt-in via --reload). Sur Windows le reloader uvicorn
    # fait tomber tout l'arbre VisionNexus au moindre edit backend (crashs constates).
    if args.reload: backend_cmd.append("--reload")
