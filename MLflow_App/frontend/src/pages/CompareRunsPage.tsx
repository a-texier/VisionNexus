...
export default function CompareRunsPage() {
  ...
  const handleCompare = async () => {
    if (selectedRunIds.size < 2) {
      toast.error('Sélectionnez au moins 2 runs')
      return
    }
    setComparing(true)
    try {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-semibold text-white">Comparer des runs</h1>
      </div>

      {/* Step 1 — Select experiment */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
        <h2 className="text-sm font-medium text-gray-300">1. Sélectionner une expérience</h2>
        <select
          value={selectedExpId}
      ...
      {selectedExpId && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
            <h2 className="text-sm font-medium text-gray-300">
              ...
              {selectedRunIds.size > 0 && (
                <span className="ml-2 text-xs text-indigo-400">({selectedRunIds.size} sélectionné(s))</span>
              )}
            </h2>
          ...
          {runsLoading ? (
            ...
            <div className="px-4 py-6 text-center text-gray-500 text-sm">Aucun run dans cette expérience</div>
          ) : (
          ...
          <div className="flex justify-end px-4 py-3 border-t border-gray-800">
            <button
              ...
              <BarChart2 size={15} />
              {comparing ? 'Comparaison…' : `Comparer ${selectedRunIds.size} run(s)`}
            </button>
          </div>
        </div>
      )}
      ...
      {result && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-white">3. Résultats de comparaison</h2>
          ...
          {result.all_params.length > 0 && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-800">
                <h3 className="text-sm font-medium text-gray-300">Paramètres comparés</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-gray-800">
                      <th className="text-left px-4 py-2 text-gray-500 font-medium uppercase">Paramètre</th>
