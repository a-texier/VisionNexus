      ...
      <div className="px-3 py-1.5 border-b border-white/10 text-[10px] uppercase flex items-center gap-1">
        ...
        {d.kind === "run" && (
          <>
            ...
            <button
              className="nodrag p-0.5"
              title={d.collapsed ? "Déplier" : "Replier"}
              onClick={(e) => {
                e.stopPropagation();
...
function build(
  ...
  runs.forEach((r, i) => {
    ...
    nodes.unshift({
      ...
      type: "groupbox",
      position: { x: x0, y: 220 },
      data: { label: `${fork ? "Run fork" : "Run mère"} · vue MLflow`, fork },
...
function Inner() {
  ...
  const flow = useMemo(
    () => build(payload, local, search, collapsed, orchUrl),
    [payload, local, search, collapsed, orchUrl],
  );
  ...
  return (
    <div className="h-full min-h-[600px] flex flex-col bg-gray-950">
      <header className="px-5 py-3 border-b border-gray-800 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-semibold text-white">Lineage MLflow</h1>
        ...
        <button
          ...
          aria-label={view === "graph" ? "Afficher la liste" : "Afficher le graphe"}
          title={view === "graph" ? "Afficher la liste" : "Afficher le graphe"}
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Expérience, Run ID, dataset…"
            ...
            <div className="h-full overflow-auto p-6">
              <div className="mx-auto max-w-[1450px] rounded-2xl border border-emerald-700/40 bg-emerald-950/5 p-5">
                <div className="mb-5 rounded-xl border border-amber-700/50 bg-amber-950/15 p-4">
                  <p className="text-xs font-semibold text-amber-300">Dataset source commun</p>
                  <p className="mt-1 text-sm text-white">{payload.nodes.find(n => n.type === "source_dataset")?.label || "Source inconnue"}</p>
                ...
                <div className="grid gap-5 xl:grid-cols-2">
                  {flow.runs.map(r => {
                    ...
                    return <section key={r.id} className={`rounded-2xl border-2 border-dashed p-4 ${r.data.is_fork ? "border-rose-700/50 bg-rose-950/10" : "border-sky-700/50 bg-sky-950/10"}`}>
                      <div className="flex"><div className="flex-1"><p className={r.data.is_fork ? "text-xs text-rose-300" : "text-xs text-sky-300"}>{r.data.is_fork ? "Run fork" : "Run mère"}</p><h3 className="text-sm text-white">{String(r.data.graph_name || r.label)}</h3><p className="font-mono text-[10px] text-violet-300">orch_run_id={rid}</p></div><span className="h-fit rounded bg-violet-950 px-2 py-1 text-[10px] text-violet-300">{stages.length} étape(s)</span></div>
                      <div className="mt-4 rounded-lg border border-amber-800/40 bg-amber-950/10 p-3"><p className="text-[10px] text-amber-400">Subset utilisé</p><p className="text-xs text-white">{String(r.data.subset_name || r.data.dataset || "—")} · {String(r.data.subset_image_count || "?")} images</p></div>
                      <p className="mb-2 mt-4 text-[10px] uppercase text-gray-500">Étapes MLflow et métriques</p>
                      <div className="space-y-2">{stages.map(stage => { const metrics = (stage.data.metrics || {}) as Record<string, unknown>; return <button key={stage.id} onClick={() => { const localStage = local.find(x => x.run_id === stage.data.mlflow_run_id); setSelected({ kind: "stage", label: stage.label, sub: String(stage.data.mlflow_run_id || ""), node: stage, runId: rid, graphId: String(r.data.graph_id || ""), stage: localStage }) }} className="w-full rounded-lg border border-violet-800/40 bg-violet-950/10 p-3 text-left"><p className="text-xs text-violet-200">{stage.label}</p><p className="mt-1 font-mono text-[10px] text-gray-500">{Object.entries(metrics).slice(0, 4).map(([k,v]) => `${k}=${typeof v === "number" ? v.toFixed(4) : v}`).join(" · ") || "Aucune métrique"}</p></button>})}{!stages.length && <p className="rounded border border-dashed border-gray-800 p-3 text-xs text-gray-500">Aucune étape MLflow : le fork s’est interrompu avant Training.</p>}</div>
                    </section>
                  })}
                </div>
              </div>
