...
function TransitionMenu({
  ...
  const handleTransition = async (stage: string) => {
    ...
    try {
      ...
      toast.success(`Version ${version} → ${stage}`)
      onDone()
    } catch {
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
      ...
      <div
        ...
        <span className="text-xs text-gray-500 flex-shrink-0 ml-2">
          Mis à jour : {formatTs(model.last_updated_timestamp)}
        </span>
      </div>
      ...
      {expanded && (
        <div className="border-t border-gray-800">
          <p className="px-4 py-2 text-[11px] text-gray-500 bg-gray-950/40 border-b border-gray-800/60">
            Chaque <b className="text-gray-400">version</b> = un entraînement successif du même modèle
            (même projet). Le dataset et la mAP50 relient la version à son run d'origine.
          </p>
          {versions.length === 0 ? (
            <div className="px-4 py-4 text-xs text-gray-500">Aucune version enregistrée</div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800">
                  ...
                  <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium uppercase">Créé le</th>
    ...
    <div className="p-6 space-y-5">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-500">
          ...
          <p className="text-sm">Aucun modèle enregistré</p>
        </div>
      ) : (
