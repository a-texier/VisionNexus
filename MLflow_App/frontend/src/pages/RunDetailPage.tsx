...
import { useRun, useRunArtifacts } from '../hooks/useRuns'
import {
  LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip,
  ResponsiveContainer,
} from 'recharts'
import { ArrowLeft, Activity, FileText, Package, GitCommit, Database, Workflow, Link2, Image as ImageIcon } from 'lucide-react'
...
// Plots joints au run par Training_App (dossier "plots") : ceux que le moteur
// du run declare. Affiches ici pour eviter d'aller les chercher sur le disque.
function PlotGallery({ runId }: { runId: string }) {
  const { data } = useRunArtifacts(runId, 'plots')
  const images = (data?.artifacts ?? []).filter(a => !a.is_dir && /\.(png|jpe?g)$/i.test(a.path))
  if (images.length === 0) return null
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        <ImageIcon size={16} className="text-emerald-400" />
        <h2 className="text-sm font-semibold text-white">Plots</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {images.map(a => (
          <a key={a.path} href={runsAPI.artifactUrl(runId, a.path)} target="_blank" rel="noreferrer"
             className="block bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-emerald-700/60">
            <img src={runsAPI.artifactUrl(runId, a.path)} alt={a.path} loading="lazy" className="w-full" />
            <p className="px-3 py-2 text-[11px] text-gray-400 font-mono truncate">{a.path.split('/').pop()}</p>
          </a>
        ))}
      </div>
    </section>
...
export default function RunDetailPage() {
  ...
  if (error || !run) {
    return (
      <div className="p-6">
        ...
        <div className="text-red-400 text-sm">{t('Run introuvable ou serveur MLflow non disponible.')}</div>
      </div>
    )
  }
  ...
  const tags = run.tags ?? {}
  const lineageFields = [
    { key: 'orch_run_id', label: t('Run orchestrateur'), icon: <Workflow size={12} />, val: tags['orch_run_id'] },
    { key: 'graph_id',    label: t('Graphe'),            icon: <Workflow size={12} />, val: tags['graph_id'] },
    ...
    { key: 'node_label',  label: t('Nœud'),              icon: <Workflow size={12} />, val: tags['node_label'] },
    { key: 'stage',       label: t('Étape'),             icon: <Activity size={12} />, val: tags['stage'] },
  ].filter(f => f.val)
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-3 mt-4">
          <span className="text-xs text-gray-400 bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5">
            {t('Début')} : {formatTs(run.start_time)}
          </span>
          <span className="text-xs text-gray-400 bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5">
            {t('Durée')} : {formatDuration(run.duration_ms)}
          </span>
          <span className="text-xs text-gray-400 bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5">
      ...
      {lineageFields.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[11px] text-gray-500">{t('quel code / quelles données ont produit ce run')}</span>
          </div>
      ...
      {hasHistory && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-semibold text-white">{t('Historique métriques')}</h2>
          </div>
      ...
      {Object.keys(run.metrics).length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-semibold text-white">{t('Métriques finales')}</h2>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800">
                  <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium uppercase w-1/3">{t('Paramètre')}</th>
                  <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium uppercase">{t('Valeur')}</th>
                </tr>
              </thead>
              <tbody>
                ...
                {run.artifacts.map((a, i) => (
                  <tr key={a.path} className={i > 0 ? 'border-t border-gray-800/60' : ''}>
                    ...
                    <td className="px-4 py-2.5 text-xs">
                      {a.is_dir ? (
                        <span className="text-blue-400">{t('Dossier')}</span>
                      ) : (
                        <span className="text-gray-400">{t('Fichier')}</span>
                      )}
                    </td>
