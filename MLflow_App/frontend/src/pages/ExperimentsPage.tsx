    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-md p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-white font-semibold text-base">Nouvelle expérience</h2>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-xs text-gray-500 uppercase">
            ...
            <th className="text-left px-4 py-2 font-medium" title="orch_run_id : la clé unique qui relie ce run à Git, DVC et l'orchestrateur">ID unifié</th>
            <th className="text-left px-4 py-2 font-medium">Statut</th>
            <th className="text-left px-4 py-2 font-medium">Début</th>
            <th className="text-left px-4 py-2 font-medium">Métriques finales</th>
...
export default function ExperimentsPage() {
  ...
  const handleDelete = async (exp: Experiment, e: React.MouseEvent) => {
    e.stopPropagation()
    if (!window.confirm(`Supprimer l'expérience "${exp.name}" ?`)) return
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-gray-400 bg-gray-900/50 border border-gray-800 rounded-lg px-4 py-2.5">
        <span className="text-gray-500">
          Une <b className="text-gray-300">expérience</b> = un projet (graphe MLOps) qui regroupe tous ses runs.
          Le <b className="text-gray-300">rôle</b> distingue chaque étape ; l'<b className="text-amber-300">ID unifié</b> (orch_run_id)
          relie chaque run à Git/DVC/orchestrateur ; les <b className="text-indigo-300">forks</b> sont indentés sous leur parent.
        </span>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-500">
          ...
          <p className="text-sm">Aucune expérience trouvée</p>
          <p className="text-xs text-gray-600">Créez votre première expérience ou vérifiez que MLflow est actif</p>
        </div>
      ) : (
