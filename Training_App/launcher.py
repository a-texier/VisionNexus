...
def main():
    parser = argparse.ArgumentParser(description="Launch Training_App")
    ...
    env = {**os.environ,
           "TRAINING_APP_WORKSPACE": ws,
           "TRAINING_APP_USER": args.user,
           "TRAINING_APP_FRONTEND_PORT": str(fp),
           "BACKEND_PORT": str(bp),
    ...
    print("=" * 60)
    print(f"  Training_App — {args.user}")
    print("=" * 60)
    ...
    backend_cmd = [python_exe, "-m", "uvicorn", "backend.main:app",
                   "--host", os.environ.get("CV_BIND_HOST", "127.0.0.1"), "--port", str(bp)]
